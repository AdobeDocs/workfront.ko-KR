...
* `CALENDAR_DATE`: 이 날짜는 [일별 내역] 보기에만 표시됩니다. 매일 기록 보기는 `CALENDAR_DATE`에 지정된 각 날짜에 대해 11:59 UTC에 데이터의 모습에 대한 기록을 제공합니다.
* `BEGIN_EFFECTIVE_TIMESTAMP`: 이 날짜는 이벤트 및 일일 기록 보기에 모두 있으며 레코드가 애플리케이션의 현재 값이 되는 시간을 나타냅니다.
* `END_EFFECTIVE_TIMESTAMP`: 이 날짜는 이벤트 및 일일 기록 보기에 모두 있으며 레코드가 현재 행의 값을 _부터_&#x200B;까지 다른 행의 값으로 변경한 때를 정확히 기록합니다. `BEGIN_EFFECTIVE_TIMESTAMP`과(와) `END_EFFECTIVE_TIMESTAMP`의 쿼리 사이에 을(를) 허용하려면 새 값이 없어도 이 값은 null이 아닙니다. 레코드가 여전히 유효한 경우(값이 변경되지 않음) `END_EFFECTIVE_TIMESTAMP`의 값은 2300-01-01입니다.
...
>[!NOTE]
>
>Workfront 애플리케이션의 진화하는 데이터 요구 사항을 지원하기 위해 사전 공지 없이 새 필드를 오브젝트 보기에 추가할 수 있습니다. 다운스트림 데이터 수신자가 추가되는 열을 처리할 준비가 되지 않은 경우 &quot;SELECT&quot; 쿼리를 사용하지 않도록 주의합니다.<br>
...
>[!NOTE]
>
>팀 객체 테이블에는 PROJECT, TEMPLATE 및 ADHOC의 세 가지 팀 유형이 저장됩니다. <br>
...
<table>
    ...
    <tbody>
        ...
        <tr>
             <td>SCHEMAID</td>
             <td>FK</td>
             <td>SCHEMA 테이블은 제공되지 않습니다. 이 테이블의 값은 SCHEMANAME 열에 제공됩니다. SCHEMANAME은 KPI(예: plannedRevenueRate, plannedCostRate, actualRevenue 등)를 식별합니다. 레코드가 연결되는 대상.</td>
             <td>SCHEMAID</td>
        </tr>
        <tr>
             <td>SOURCETASKID</td>
...
다음 표에서는 Data Connect의 Planning 스키마에서 사용할 수 있는 데이터 보기에 대해 설명합니다. 워크플로우 데이터 보기와 달리 Planning 데이터 보기에는 각 보기에 대한 전체 열별 설명이 포함됩니다.

Planning 데이터는 다음 보기에서 사용할 수 있습니다.

* **FIELD\_CURRENT** — Planning 레코드 유형에 구성된 모든 필드의 현재 정의입니다.
* **FIELD\_DAILY\_HISTORY** - 시간 경과에 따른 필드 구성 변경을 추적하는 데 유용한 필드 정의의 일별 스냅숏입니다.
* **FIELD\_EVENT** — 필드 정의를 수정하는 모든 이벤트의 변경 로그입니다.
* **PLANNINGRECORD\_CURRENT** — 모든 작업 영역 및 레코드 유형의 모든 Planning 레코드의 현재 상태입니다.
* **PLANNINGRECORD\_DAILY\_HISTORY** - 트렌드 분석에 유용한 Planning 레코드 상태의 일별 스냅숏입니다.
* **PLANNINGRECORD\_EVENT** — 시점 비교에 유용한 Planning 레코드를 수정하는 모든 이벤트의 변경 로그입니다.
* **RECORDTYPE\_CURRENT** — Planning 작업 영역에 구성된 모든 레코드 유형의 현재 정의입니다.
* **RECORDTYPE\_DAILY\_HISTORY** - 레코드 유형 정의의 일별 스냅숏으로, 시간 경과에 따른 레코드 유형 구성 변경을 추적하는 데 유용합니다.
* **RECORDTYPE\_EVENT** — 레코드 유형 정의를 수정하는 모든 이벤트의 변경 로그입니다.
* **REFERENCE\_CURRENT** — Planning 레코드와 Workfront(또는 다른 외부) 개체 간의 모든 개체 간 참조 연결의 현재 상태입니다.
* **WORKSPACE\_CURRENT** — 모든 Planning 작업 영역의 현재 정의입니다.
* **WORKSPACE\_DAILY\_HISTORY** - 작업 영역 정의에 대한 일별 스냅숏으로, 시간 경과에 따른 작업 영역 구성 변경을 추적하는 데 유용합니다.
* **WORKSPACE\_EVENT** — 작업 영역 정의를 수정하는 모든 이벤트의 변경 로그입니다.



>[!NOTE]
>
>다음 보기는 Planning 스키마에서 사용할 수 있지만 여기에서 자세히 설명하지는 않습니다. 표준 기록 및 이벤트 감사 필드가 추가되어 해당 CURRENT 보기와 동일한 열 패턴을 따릅니다.
>
>보기는 다음과 같습니다.
>
>* 일별\_내역 보기
>
>* 이벤트 보기(FIELD\_DAILY\_HISTORY, FIELD\_EVENT, PLANNINGRECORD\_DAILY\_HISTORY, PLANNINGRECORD\_EVENT, RECORDTYPE\_DAILY\_HISTORY, RECORDTYPE\_EVENT, WORKSPACE\_DAILY\_HISTORY 및 WORKSPACE\_EVENT)


### FIELD\_CURRENT

Workfront Planning 레코드 유형에 구성된 모든 필드의 현재 정의를 포함합니다. 각 행은 디스플레이 이름, 유형 및 JSON(구조화된 변형) 열에 저장된 유형별 구성을 포함하여 하나의 필드를 나타냅니다. 이 보기를 사용하여 PLANNINGRECORD\_CURRENT의 필드 ID를 사람이 읽을 수 있는 필드 이름 및 메타데이터로 확인할 수 있습니다.
...
모든 작업 공간 및 레코드 유형에서 Workfront Planning에서 생성된 모든 레코드의 현재 상태를 포함합니다. 각 행은 하나의 Planning 레코드를 나타내며, Planning 데이터의 동적, 스키마 유연한 특성을 반영하는 구조화된 변형(JSON) 열에 저장된 필드 값이 있습니다.
...
Workfront Planning 작업 영역에 구성된 모든 레코드 유형의 현재 정의를 포함합니다. 각 행은 표시 이름, 작업 공간 연결, 기본 필드 및 구성 메타데이터를 포함하여 하나의 레코드 유형을 나타냅니다. 이 보기를 사용하여 PLANNINGRECORD\_CURRENT 및 FIELD\_CURRENT의 레코드 유형 ID를 사람이 읽을 수 있는 이름으로 확인할 수 있습니다.
...
Workfront Planning 레코드와 Workfront(또는 기타 외부) 객체 간의 모든 교차 객체 참조 연결의 현재 상태를 포함합니다. 각 행은 하나의 참조 연결을 나타냅니다. PLANNINGRECORD\_CURRENT.REFERENCE\_IDS와 함께 이 보기를 사용하여 연결된 외부 개체를 해결합니다.
...
모든 Workfront Planning 작업 공간에 대한 현재 정의를 포함합니다. 각 행은 이름, 계층 및 소유권 메타데이터를 포함하여 하나의 작업 영역을 나타냅니다. 이 뷰를 사용하여 PLANNINGRECORD\_CURRENT 및 RECORDTYPE\_CURRENT의 작업 공간 ID를 사람이 읽을 수 있는 작업 공간 이름으로 확인할 수 있습니다.
