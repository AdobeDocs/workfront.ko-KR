...
title: 계산된 사용자 정의 필드 및 계산된 열
description: Adobe Workfront에서 여러 필드를 합산하고 해당 합산 값을 새 필드에 표시하려면 사용자 정의 폼에서 계산된 사용자 정의 필드를 만들거나 뷰의 계산된 열을 만들 수 있습니다.
author: Courtney
...
# 계산된 사용자 정의 필드 및 계산된 열

Adobe Workfront에서 여러 필드를 합산하고 해당 합산 값을 새 필드에 표시하려면 다음을 생성할 수 있습니다.
...
* 뷰의 계산된 열\
  보기에서 계산을 사용하는 방법에 대한 자세한 내용은 [텍스트 모드에 대한 일반적인 사용 개요](../../../reports-and-dashboards/reports/text-mode/understand-common-uses-text-mode.md#use-text-mode-in-views) 문서의 [보기에서 텍스트 모드 사용](../../../reports-and-dashboards/reports/text-mode/understand-common-uses-text-mode.md) 섹션을 참조하십시오.

텍스트 모드를 사용하여 계산된 필드와 계산된 열을 모두 작성하지만 작성하기 위한 구문은 다릅니다. 계산된 필드 및 계산된 열을 빌드하는 방법은 위에 나열된 문서를 참조하십시오. 계산된 사용자 정의 필드 및 열과 같이 계산된 데이터 식에 사용되는 다양한 구문에 대한 자세한 내용은 이 문서의 [계산된 사용자 정의 필드 및 계산된 사용자 정의 열의 구문](/help/quicksilver/reports-and-dashboards/reports/calc-cstm-data-reports/calculated-custom-fields-calculated-columns.md#syntax-of-calculated-custom-fields-vs-calculated-custom-columns) 섹션을 참조하십시오.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>Workfront 인터페이스에 표시되는 필드의 이름을 사용합니다.</p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>예: </b></span></span>계산된 사용자 지정 필드에 사용된 필드 이름의 예: <code>Planned Completion Date</code>.</p> </td> 
   <td> <p>Workfront 데이터베이스에 나타나는 개체 또는 필드의 이름을 사용합니다. 개체와 필드의 이름이 컴파운드 이름인 경우 소문자나 낙타식 이름의 철자를 사용합니다. </p> <p>데이터베이스에 나타나는 모든 Workfront 개체 및 필드의 인벤토리는 <a href="../../../wf-api/general/api-explorer.md" class="MCXref xref">API 탐색기</a>를 참조하십시오. </p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>예: </b></span></span>계산된 사용자 지정 보고 요소에 사용되는 필드 이름의 예: <code>plannedCompletionDate</code>.</p> </td> 
  </tr> 
  <tr> 
   <td>필드 이름을 괄호나 중괄호로 묶습니다.</td> 
   <td> <p>필드 이름을 <code>valuefield </code>행에 사용할 때는 대괄호나 괄호로 묶지 마십시오.</p> <p>필드 이름을 <code>valueexpression</code> 줄로 사용할 때는 중괄호로 묶으십시오.</p> </td> 
  </tr> 
  <tr> 
   <td>마침표로 필드 구분</td> 
   <td> <p><code>valuefield</code>줄에서 필드를 사용할 때는 필드를 콜론으로 구분합니다.</p> <p>필드를 <code>valueexpression</code>줄에 사용할 때는 필드를 마침표로 구분하십시오.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
자세한 내용은 [사용자 지정 양식 필드에서 정보 편집](/help/quicksilver/workfront-basics/work-with-custom-forms/edit-custom-forms.md)을 참조하세요.

## 뷰에서 계산된 열을 사용하는 경우

* 보고서에서 실시간 데이터를 사용하려는 경우.

  보고서가 실행되거나 뷰가 적용될 때 계산되므로 계산된 뷰는 항상 신선합니다.
