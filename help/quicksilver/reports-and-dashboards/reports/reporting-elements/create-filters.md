...
<p>참고: 필터용 베타 빌더는 보고서에서 사용할 수 없습니다.
</td>
</tr>
</tbody>
</table>
...
## 표준 빌더에서 필터 만들기 또는 편집 {#create-filter-in-standard-builder}

다음과 같은 방법으로 목록 및 보고서에서 필터를 만들 수 있습니다.
...
## 베타 빌더에서 필터 만들기 또는 편집

다양한 인터페이스를 사용하여 필터를 만들 때에는 다음 사항을 고려하십시오.

* 위의 표에 나열된 영역에 대한 표준 필터 인터페이스를 찾는 동일한 위치에서 베타 빌더를 찾을 수 있습니다.
* 베타 옵션을 사용할 수 있는 표준 및 베타 빌더 인터페이스 간을 앞뒤로 전환할 수 있습니다.
* 한 영역에서 Beta 빌더를 활성화하면 사용 가능한 모든 영역에 대한 기본 경험입니다. 예를 들어 프로젝트 목록에서 Beta 빌더를 활성화하면 목록에서 작업 및 문제 필터를 만드는 기본 경험이 됩니다.
* 다음과 같은 방법으로 베타 빌더 인터페이스를 사용하여 필터를 만들 수 있습니다.
...
* 저장된 필터는 원래 빌드했던 경험에 관계없이 두 빌더에서 모두 사용할 수 있습니다. 예를 들어 표준 빌더를 사용하여 필터를 만든 경우 Beta 빌더 인터페이스에서도 필터를 찾아 수정할 수 있습니다.

   >[!TIP]
   >
   >필터가 적용되지 않을 때 모든 목록 항목이 표시되므로 &quot;모두&quot; 필터는 Beta 빌더에 포함되지 않습니다. 클릭 **모두 지우기** 빌더 오른쪽 상단에서 클릭하여 활성 필터를 지우고 모든 항목을 표시합니다. If **모두 지우기** 이 흐리게 표시되어 있으면 필터가 적용되지 않습니다.

* 표준 및 베타 빌더는 AND 및 OR 연산자를 결합하는 여러 문 필터를 작성할 때 구문은 약간 다릅니다. 따라서 빌더에서 다른 빌더로 전환할 때 이러한 필터가 다르게 표시될 수 있습니다.

   >[!INFO]
   >
   >다음 시나리오가 있습니다.
   >
   >1. Beta 빌더를 사용하여 다음 구문을 사용하는 필터를 만듭니다.
   >
   >   `(A OR B) AND C`
   >
   >1. 에 설명된 대로 표준 빌더로 돌아가서 표준 빌더 구문을 사용하여 필터를 편집합니다 [표준 빌더에서 필터 만들기 또는 편집](#create-filter-in-standard-builder) 섹션에 자세히 설명되어 있습니다. 표준 빌더의 구문은 다음과 같이 필터 구문을 표시합니다.
   >
   >   `A AND C`
   >   `OR`
   >   `B AND C`
   >
   >1. 표준 인터페이스에서 필터를 변경합니다.
   >1. 다시 베타 빌더로 전환합니다. 위에 설명된 대로 필터 문은 표준 빌더에서 지원되는 논리에 따라 표시됩니다.

   >
   >   필터는 Beta 빌더 인터페이스에 다음과 같이 표시됩니다.
   >  
   >   `A AND C`
   >   `OR`
   >   `B AND C`
   > 
   >   이 문제는 표준 인터페이스에서 필터가 수정되었기 때문에 발생합니다.

Beta 빌더 인터페이스를 사용하여 필터를 만듭니다.

1. 필터를 만들거나 사용자 지정할 필터가 포함된 목록으로 이동합니다.
1. 을(를) 클릭합니다. **필터** 아이콘 ![필터 아이콘](assets/filter-nwepng.png)를 활성화한 후 **베타 설정** ![베타 설정](assets/beta-toggle-white-on-existing-filters.png) 베타 빌더에 액세스하려면 기본적으로 비활성화되어 있습니다.

   다음으로, 필요한 경우 베타 계약에 동의합니다. Beta 빌더가 활성화되면 한 번만 동의하면 됩니다.

   베타 필터 빌더 인터페이스가 열립니다.

   >[!TIP]
   >
   >베타 빌더를 활성화하면 필터 빌더 인터페이스의 헤더가 파란색으로 변경됩니다. Beta Builder 인터페이스를 활성화하면 Workfront이 사용 가능한 모든 영역에 대해 활성화됩니다.
...
1. 수정자 드롭다운 메뉴를 눌러 수정자를 선택합니다. 기본 수정자는 &quot;같음&quot;입니다.
   ...
   >[!TIP]
   >
   >필터를 빌드하면 결과가 목록에 즉시 나타납니다. 필터 패널이 목록을 포함하는 경우 필터를 닫아서 디스플레이를 볼 수 있습니다. 입력한 정보는 패널을 다시 열 때 Beta 빌더에 유지됩니다.
...
1. (선택 사항) **텍스트 모드** 텍스트 모드를 사용하여 필터를 계속 빌드하려면 다음을 수행하십시오.
   ...
   >[!TIP]
   >
   >텍스트 모드에서만 지원되는 필터를 수정해야 하는 경우 Beta 빌더 인터페이스를 사용하고 텍스트 모드를 사용해야 하는 경우에만 필터를 최대한 사용하는 것이 좋습니다.
...
1. 클릭 **종료 텍스트 모드** 베타 빌더 인터페이스로 돌아갑니다.

   >[!WARNING]
   >
   >일부 텍스트 모드 문은 Beta 빌더 또는 표준 인터페이스에서 지원되지 않습니다. 이러한 유형의 문을 만든 경우 텍스트 모드를 종료하면 경고 메시지가 생성됩니다.
