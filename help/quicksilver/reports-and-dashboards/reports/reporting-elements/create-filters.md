...
1. 클릭 **필터 추가** 다른 필드를 선택하려면 필터 문에 필터링 기준을 추가하십시오.
...
1. (선택 사항) **필터 그룹 추가** 다른 필터링 기준 세트를 추가합니다. 집합 사이의 기본 연산자는 입니다. **및**. 변경할 연산자를 클릭합니다. **또는**.
...
1. (선택 사항) 필터를 즐겨찾기에 추가 목록으로 이동하려면 필터 서랍의 필터 위로 마우스를 가져간 후 **즐겨찾기** 아이콘 ![즐겨찾기 아이콘](assets/favorites-icon-small.png).
...
1. 클릭 **필터 저장** 을 클릭하여 필터를 만들거나 선택한 필터를 변경 내용으로 바꿉니다.

   또는

   클릭 **새 필터로 저장** 을 눌러 선택한 필터에서 필터를 생성합니다.
