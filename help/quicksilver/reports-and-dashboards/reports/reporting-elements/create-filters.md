...
다음과 같은 방법으로 목록 및 보고서에 필터를 만들 수 있습니다.
...
## Beta 빌더에서 필터 만들기 또는 편집

서로 다른 인터페이스를 사용하여 필터를 만들 때는 다음 사항을 고려하십시오.

* 위의 표에 나열된 영역에 대한 표준 필터 인터페이스를 찾을 때와 동일한 위치에서 Beta 빌더를 찾을 수 있습니다.
* Beta 옵션을 사용할 수 있는 표준 과 Beta 빌더 인터페이스 간을 전환할 수 있습니다.
* 한 영역에서 Beta 빌더를 활성화한 후에는 이 빌더를 사용할 수 있는 모든 영역에 대한 기본 경험이 됩니다. 예를 들어 프로젝트 목록에서 Beta 빌더를 활성화하는 경우, 목록에서 작업 및 문제 필터를 빌드하는 기본 경험입니다.
* Beta 빌더 인터페이스를 사용하여 다음과 같은 방법으로 필터를 만들 수 있습니다.
...
* 저장된 필터는 원래 빌드하는 데 사용한 경험에 관계없이 두 빌더에서 모두 사용할 수 있습니다. 예를 들어 표준 빌더를 사용하여 필터를 만든 경우 Beta 빌더 인터페이스에서도 필터를 찾아 수정할 수 있습니다.

   >[!TIP]
   >
   >필터가 적용되지 않으면 모든 목록 항목이 표시되므로 &quot;모든&quot; 필터는 Beta 빌더에 포함되지 않습니다. 클릭 **모두 지우기** 빌더의 오른쪽 상단에서 활성 필터를 지우고 모든 항목을 표시합니다. If **모두 지우기** 는 흐리게 표시되어 적용된 필터가 없습니다.

* 표준 및 Beta 빌더는 AND와 OR 연산자를 결합하는 여러 문 필터를 작성할 때 약간 다른 구문을 사용합니다. 따라서 이러한 필터는 한 빌더에서 다른 빌더로 전환할 때 다르게 표시될 수 있습니다.

   >[!INFO]
   >
   >다음 시나리오가 있습니다.
   >
   >1. Beta 빌더를 사용하여 다음 구문을 사용하는 필터를 생성합니다.
   >
   >   `(A OR B) AND C`
   >
   >1. 표준 빌더로 다시 전환하고 다음에 설명된 대로 표준 빌더의 구문을 사용하여 필터를 편집합니다. [표준 빌더에서 필터 만들기 또는 편집](#create-filter-in-standard-builder) 이 문서의 섹션. 표준 빌더 구문은 다음과 같이 필터 문을 표시합니다.
   >
   >   `A AND C`
   >   `OR`
   >   `B AND C`
   ...
   >1. Beta 빌더로 다시 전환합니다. 필터 문은 위에서 설명한 대로 표준 빌더에서 지원되는 논리에 따라 표시됩니다.
   ...
   >   `B AND C`
   > 
   >   이 문제는 필터가 표준 인터페이스에서 수정되었기 때문에 발생합니다.
...
1. 다음을 클릭합니다. **필터** 아이콘 ![필터 아이콘](assets/filter-nwepng.png)을 클릭한 다음 을(를) 활성화합니다. **Beta 설정** ![Beta 설정](assets/beta-toggle-white-on-existing-filters.png) 를 클릭하여 beta 빌더에 액세스합니다. 기본적으로 비활성화되어 있습니다.
   ...
   필요한 경우 Beta 계약에 동의합니다. 한 번만 동의하면 Beta 빌더가 활성 상태로 유지됩니다.

   그러면 Beta 필터 빌더 인터페이스가 열립니다.

   >[!TIP]
   >
   >Beta 빌더를 활성화하면 필터 빌더 인터페이스의 헤더가 파란색으로 변경됩니다. Beta 빌더 인터페이스를 활성화하면 Workfront은 사용 가능한 모든 영역에서 해당 인터페이스를 활성화 상태로 유지합니다.
...
1. 수정자 드롭다운 메뉴를 눌러 수정자를 선택합니다. 기본 수정자는 &quot;같음&quot;입니다.
   ...
   >[!TIP]
   >
   >필터를 빌드하면 결과가 목록에 즉시 나타납니다. 필터 패널이 목록을 덮는 경우 필터 패널을 닫고 디스플레이를 볼 수 있습니다. 입력한 정보는 패널을 다시 열 때 Beta 빌더에 남아 있습니다.
...
1. (선택 사항) **텍스트 모드** 을 클릭하여 텍스트 모드를 사용하여 필터를 계속 빌드합니다.
   ...
   >[!TIP]
   >
   >텍스트 모드에서만 지원되는 필터를 수정해야 하는 경우 Beta 빌더 인터페이스를 사용하고 텍스트 모드만 사용하여 가능한 한 많은 필터를 빌드하는 것이 좋습니다.
...
1. 클릭 **텍스트 모드 종료** 를 클릭하여 베타 빌더 인터페이스로 돌아갑니다.

   >[!WARNING]
   >
   >일부 텍스트 모드 문은 Beta 빌더 또는 표준 인터페이스에서 지원되지 않습니다. 이러한 유형의 문을 작성했을 때 텍스트 모드를 종료하면 경고 메시지가 생성될 수 있습니다.
