...
description: Adobe Workfront에서 다른 사용자와 보고서를 공유할 때 보고서 보기를 사용자 지정하여 특정 정보를 더 쉽게 읽거나 눈에 잘 띄도록 하는 것이 좋습니다.
author: Courtney
...
Adobe Workfront에서 다른 사용자와 보고서를 공유할 때 보고서 보기를 사용자 지정하여 특정 정보를 더 쉽게 읽거나 눈에 잘 띄도록 하는 것이 좋습니다.
...
보고서 보기에서 조건부로 열 서식을 지정하여 보고서 표시 방식에 영향을 주는 규칙을 설정할 수 있습니다. 이러한 조건이나 규칙이 충족되면 특수 형식이 적용됩니다.

예를 들어 작업의 완료율이 20% 미만이면 백분율 번호를 굵게, 빨간색 텍스트 및 노란색 배경색으로 표시하여 필드를 강조 표시할 수 있습니다.

조건부 서식의 뷰를 사용하면 다음 작업을 수행할 수 있습니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>필터, 보기, 그룹화에 대한 액세스 편집</p> <p>보고서, 대시보드, 달력에 대한 액세스 권한을 편집하여 보고서의 보기 편집</p>
   </td> 
  </tr> 
  <tr> 
   <td role="rowheader">개체 권한</td> 
    <td> <p>보고서에 대한 권한 관리를 통해 보고서에서 보기를 생성하거나 편집합니다</p> <p>보기에 대한 권한 관리</p></td> 
   </td> 
  </tr> 
 </tbody> 
</table>
...
조건부 서식을 보기에 추가하려면 먼저 보고서를 만들어야 합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td>열의 값이 표시되는 형식을 선택합니다. 열 필드의 값에 따라 날짜, 숫자 또는 통화 표시 방법을 설정할 수 있습니다. 일부 열에는 이 옵션이 표시되지 않습니다.</td> 
     </tr> 
     <tr> 
      <td role="rowheader"><strong>대시보드에 있을 때 이 열 표시</strong></td> 
      <td>보고서를 대시보드에 배치할 때 열을 표시하려면 이 필드를 선택합니다. 대시보드 외부의 보고서를 보면 항상 열이 표시됩니다.</td> 
     </tr> 
    </tbody> 
   </table>
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td>색상 선택기를 사용하여 텍스트 배경색을 선택합니다.</td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      ...
      <td> <p>이 열의 실제 값 대신 사용자 지정 레이블을 표시하려면 이 옵션을 선택합니다. 제공된 필드의 값 대신 표시할 텍스트를 지정합니다.</p> <p><b>중요</b></p> <p><strong>텍스트 표시</strong>를 선택하면 이 열에서 텍스트를 인라인 편집하는 기능이 비활성화됩니다.<br>또한 Previous 열에는 기본 제공 논리를 포함하므로 값을 변경할 수 없습니다.</p> </td> 
     </tr> 
     <tr> 
...
1. **규칙 추가**&#x200B;를 클릭합니다.\
   ...
   **예 1**
   ...
   먼저 프로젝트가 작성 상태에 있을 때 텍스트 색상이 보라색이고 굵게 표시되는 규칙을 만들 수 있습니다. 그런 다음 작업의 이름이 비어 있지 않을 때 텍스트 색상은 빨간색으로 기울임꼴로 표시되고 배경색은 녹색으로 표시된다는 두 번째 규칙을 만듭니다. 이 예제에서 다음 오류가 발생합니다.
   ...
   **예 2**
   ...
   프로젝트 계획 완료 날짜 열에서 전체 행에 영향을 주는 규칙을 만들어 프로젝트가 취소된 경우(예: 프로젝트 상태가 작동 중지인 경우) 배경을 회색으로 바꿉니다. 그런 다음 프로젝트 계획 완료 날짜가 오늘(프로젝트가 늦음을 의미함)보다 작을 때 배경을 빨간색으로 바꾸는 열 규칙을 만듭니다. 이 예제에서 취소된 프로젝트에 늦은 완료 날짜가 있는 경우 행의 다른 셀이 회색이더라도 해당 셀은 빨간색으로 표시됩니다. 이 형식을 수정하려면 다음을 수행합니다.

   * 계획된 완료 날짜의 서식을 편집하고, 지연 프로젝트의 빨간색 배경에 대한 열 규칙을 삭제합니다.
   * 행 규칙과 같은 서식의 열 규칙을 추가합니다(프로젝트 상태가 데드 인 경우 회색 배경).
   * 늦은 프로젝트의 빨간색 배경에 대해 열 규칙을 다시 추가합니다.
   * 규칙 및 보기를 저장할 때 빨간색 배경이 취소된 프로젝트에 적용되지 않습니다.

1. **저장**&#x200B;을 클릭합니다.
1. **저장 및 닫기**&#x200B;를 클릭합니다.\
   지정된 조건이 충족되면 보고서에서 형식 변경 사항이 표시됩니다.
