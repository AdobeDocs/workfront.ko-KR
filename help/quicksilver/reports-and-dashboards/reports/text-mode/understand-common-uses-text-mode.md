...
title: 텍스트 모드에 대한 일반적인 사용 개요
description: 텍스트 모드에 대한 일반적인 사용 개요
author: Nolan
...
보고서 및 보고서 요소에서 텍스트 모드를 사용하여 보고 기능을 확장할 수 있습니다. 텍스트 모드 버전을 사용하여 보다 복잡한 계산된 사용자 지정 필드를 작성할 수도 있습니다. 텍스트 모드에 대한 자세한 내용은 [텍스트 모드 개요](../../../reports-and-dashboards/reports/text-mode/understand-text-mode.md).

이 문서에서는 Adobe Workfront에서 보고 또는 계산된 사용자 지정 필드 기능을 확장하기 위해 텍스트 모드를 사용해야 하는 경우에 대한 몇 가지 일반적인 예를 간략하게 설명합니다. 자세한 예제 목록은 다음을 참조하십시오.
...
다른 Workfront 고객이 식별했을 수 있는 추가 실제 텍스트 모드 예를 보려면 [텍스트 모드 보고 토론](https://one.workfront.com/s/topic/0TO0z000000cdHmGAI/text-mode-reporting?tabset-21363=3) Adobe Workfront One에서 탭하거나 [Workfront 커뮤니티](https://one.workfront.com/s/community) (로그인해야 함).  

## 목록 및 보고서에서 텍스트 모드를 사용할 수 있는 인스턴스

보고서 및 목록 빌더를 사용하여 보기, 필터 및 그룹화를 만드는 것이 좋습니다. 그러나 텍스트 모드를 사용하여 보고서와 목록을 향상시킬 수 있는 경우도 있습니다.

Workfront에서 다음을 달성하려는 경우 텍스트 모드를 사용할 수 있습니다.

* 사용자 정의 양식에 사용자 정의 계산된 사용자 정의 필드를 작성합니다.\
   계산된 사용자 지정 필드에 대한 자세한 내용은 [계산된 사용자 지정 필드에서 텍스트 모드 사용](#use-text-mode-in-calculated-custom-fields) 섹션에 자세히 설명되어 있습니다.
* Report Builder에서 사용할 수 있는 것 이상의 필터, 보기 및 그룹화를 개선합니다. 필터, 보기 및 그룹화에 텍스트 모드를 사용하는 방법에 대한 자세한 내용은 이 문서의 다음 섹션을 참조하십시오.

   * [보기에서 텍스트 모드 사용](#use-text-mode-in-views)
   * [필터에서 텍스트 모드 사용](#use-text-mode-in-filters)
   * [그룹화 시 텍스트 모드 사용](#use-text-mode-in-groupings)

* 사용자 정의 프롬프트를 생성합니다. 텍스트 모드를 사용해서만 사용자 정의 프롬프트를 생성할 수 있습니다.

   사용자 정의 프롬프트 작성에 대한 자세한 내용은 [보고서에 프롬프트 추가](../../../reports-and-dashboards/reports/creating-and-managing-reports/add-prompt-report.md).

## 계산된 사용자 지정 필드에서 텍스트 모드 사용 {#use-text-mode-in-calculated-custom-fields}

텍스트 모드를 사용하여 계산된 사용자 지정 필드를 사용자 지정 양식에 추가할 수 있습니다.

사용자 지정 양식에 계산된 사용자 지정 필드를 추가하는 방법에 대한 자세한 내용은 [사용자 지정 양식 만들기 또는 편집](../../../administration-and-setup/customize-workfront/create-manage-custom-forms/create-or-edit-a-custom-form.md).

텍스트 모드에서 계산된 사용자 지정 필드를 만드는 방법에 대한 자세한 내용은 [사용자 지정 양식에 계산된 데이터 추가](../../../administration-and-setup/customize-workfront/create-manage-custom-forms/add-calculated-data-to-custom-form.md).

예를 들어 항목이 진행 중으로 표시된 시점의 시간 및 날짜 스탬프를 표시하는 계산된 사용자 지정 필드를 추가할 수 있습니다. 다른 상태에 대해 이 계산을 사용할 수 있습니다.

자세한 내용은 [계산된 사용자 지정 필드 예: 사용자 지정 양식에 상태 타임스탬프 표시](../../../reports-and-dashboards/reports/calc-cstm-data-reports/example-status-timestamp-in-calculated-field.md).
...
뷰에서 텍스트 모드를 사용하여 뷰에 표시할 수 있는 필드와 개체에서 확장할 수 있습니다.
...
필터를 빌드할 때 텍스트 모드를 사용하여 필터링할 수 있는 필드 및 개체에서 확장할 수 있습니다.

필터에 텍스트 모드를 사용하는 가장 일반적인 이유에 대한 예는 다음 문서를 참조하십시오.
...
* 섹션 [개체 계층 구조에서 여러 수준에 걸쳐 있는 텍스트 모드 필터의 예](../../../reports-and-dashboards/reports/text-mode/create-complex-text-mode-filters-using-exists-statements.md#examples) 기사 [EXISTS 문을 사용하여 복잡한 텍스트 모드 필터 만들기](../../../reports-and-dashboards/reports/text-mode/create-complex-text-mode-filters-using-exists-statements.md)
* 섹션 [누락된 객체에 대해 복잡한 텍스트 모드 필터 만들기](../../../reports-and-dashboards/reports/text-mode/create-complex-text-mode-filters-using-exists-statements.md#missing-object-filters) 기사 [EXISTS 문을 사용하여 복잡한 텍스트 모드 필터 만들기](../../../reports-and-dashboards/reports/text-mode/create-complex-text-mode-filters-using-exists-statements.md)
...
그룹화를 작성할 때 텍스트 모드를 사용하여 목록 및 보고서에서 그룹화할 수 있는 필드 및 개체에서 확장할 수 있습니다.
