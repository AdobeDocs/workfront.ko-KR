...
* EXISTS 필터에서 참조할 수 있는 세 가지 개체가 있습니다.
   ...
   * 원본 개체와 Target 개체를 연결하는 개체입니다. 서로 직접 연결되어 있지 않은 경우 연결 개체입니다.
...
* 문을 연결하려면 연결 개체의 개체 코드를 사용해야 합니다.\
   ...
   API 탐색기에 대한 자세한 내용은 [API 탐색기](../../../wf-api/general/api-explorer.md).

* 원본 개체와 Target 개체가 직접 연결되어 연결 개체가 없을 경우 연결 개체 대신 Target 개체의 개체 코드를 사용할 수 있습니다.
* 동일한 개체(Target 개체)에서 여러 필드(Target 필드)를 참조할 수 있으며, 이 경우 AND로 필드를 참조하는 줄을 연결해야 합니다.\
   Target 개체에 속한 두 개 이상의 필드에 대한 필터링 예는 [예제 4: 여러 필드로 필터링: Portfolio 소유자 이름 및 Portfolio 정렬 스코어카드 ID별 작업](#example-4-filter-by-multiple-fields-tasks-by-portfolio-owner-name-and-portfolio-alignment-scorecard-id) 이 문서의 섹션.
...
필터의 예는 다음을 참조하십시오 [예제 1: Portfolio 소유자 이름별로 문제 필터링](#example-1-filter-for-issues-by-portfolio-owner-name) 이 문서의 섹션.

객체 계층의 여러 레벨에 걸쳐 적용되는 필터를 생성하려면 다음과 같이 하십시오.

1. 필터의 개체를 식별합니다. 이 객체를 원래 객체라고 합니다.\
   예를 들어, 문제
1. 필터링 기준으로 사용할 필드를 식별합니다. 이 개체를 Target 개체에 속한 Target 필드라고 합니다.\
   예를 들어 Portfolio(Target 개체)에 속하는 ownerID 필드(Target 필드)입니다.
1. (조건부) 원래 개체(Issue)와 Target 필드(ownerID)가 직접 연결되어 있지 않으면 세 번째 개체인 원본 개체와 원본 개체를 연결하는 연결 개체(Project)를 찾아야 합니다. 연결 객체에는 원본 객체의 필드 또는 참조 탭(원본 객체에 표시된 연결 필드)에서 참조하는 하나 이상의 필드가 있어야 하며 연결 객체의 필드 또는 참조 탭에 표시되는 Target 객체와의 연결 필드도 있어야 합니다. 연결 개체에 표시되는 Target 개체에 대한 연결 필드(또는 연결 개체에 표시되는 연결 필드)는 Target 필드와 일치해야 합니다.\
   예를 들어, (프로젝트) ID(원래 오브젝트에 표시된 연결 필드)는 문제(원래 오브젝트)에서 참조됩니다. (Portfolio) ownerID(Target 개체에 대한 연결 필드)가 프로젝트(연결 개체)의 필드 탭에 표시됩니다. Portfolio ownerID 는 Target 개체 (Portfolio)의 필드입니다. 연결 개체의 연결 필드는 Target 필드와 일치합니다.\
   ![portfolio_id_in_the_project_api_object.PNG](assets/portfolio-id-in-the-project-api-object-350x88.png)

1. API 탐색기를 사용하여 **개체 코드** 링크 개체(프로젝트)의\
...
항상 를 사용해야 합니다. *존재함* 문 및 이 필터를 빌드하는 텍스트 모드 인터페이스입니다.
...
1. 필터의 개체를 식별합니다. 이 객체를 원래 객체라고 합니다.\
   예: 매개변수 또는 사용자 정의 필드.
1. 필터링 기준으로 사용할 필드를 식별합니다. 이 개체를 Target 개체에 속한 Target 필드라고 합니다.\
   예를 들어 Category(Target 개체)에 속하는 categoryID 필드(Target 필드)입니다.
1. 원본 개체(매개 변수)와 Target 필드(categoryID)는 서로 직접 연결되어 있지 않으므로 세 번째 개체인 연결 개체(범주 매개 변수)를 찾아야 합니다. 연결 객체에는 원본 객체의 필드 또는 참조 탭(원본 객체에 표시된 연결 필드)에서 참조하는 하나 이상의 필드가 있어야 하며 연결 객체의 필드 또는 참조 탭에 표시되는 Target 객체와의 연결 필드도 있어야 합니다. 연결 개체에 표시되는 Target 개체에 대한 연결 필드(또는 연결 개체에 표시되는 연결 필드)는 Target 필드와 일치해야 합니다.\
   예를 들어, Parameter(Original Object)에서 Category 매개 변수(원본 개체에 표시된 연결 필드)의 ID를 참조합니다. parameterID(Target 개체에 필드 연결)는 범주 매개 변수(연결 개체)의 필드 탭에 표시됩니다. 연결 개체에 표시되는 Target 개체에 대한 연결 필드는 Target 필드와 일치합니다.
...
1. 다음 코드를 **보고서에 대한 필터 규칙 설정** 위의 일반 코드를 바꿀 영역:
   ...
   >* Target 개체는 Portfolio 상태입니다.
   >* 연결 개체는 프로젝트입니다.
   >* 연결 개체에서 참조된 Target 개체에 대한 Target 필드 및 연결 필드는 ownerID입니다.
...
1. 다음 코드를 **보고서에 대한 필터 규칙 설정** 위의 일반 코드를 바꿀 영역:
   ...
   >* parameterID는 연결 개체 테이블과 Target 개체 테이블 모두에 있으므로 Target 개체에 대한 연결 필드는 parameterID입니다.
...
1. 다음 코드를 **보고서에 대한 필터 규칙 설정** 위의 일반 코드를 바꿀 영역:
   ...
   >* 연결 개체가 없으므로 Target 개체의 개체 코드 HOUR을 사용해야 합니다.
   >* Target 개체에 대한 연결 필드는 원래 개체에 표시되는 ownerID입니다. 연결 개체가 없습니다.
   >* 원래 객체에 표시되는 연결 필드는 Target 객체에 표시되는 시간의 ID(연결 객체 누락)입니다.
...
텍스트 모드 인터페이스를 사용하여 Target 개체에 있는 두 개 이상의 필드를 참조하는 필터를 빌드할 수 있습니다. 이 경우 Target 필드를 참조하는 필터 문은 AND로 연결해야 합니다.
...
1. 다음 코드를 **보고서에 대한 필터 규칙 설정** 영역:
   ...
   >* Target 개체에 대한 연결 필드는 Portfolio의 ID입니다.
