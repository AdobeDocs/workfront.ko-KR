...
필터를 작성할 때 표준 보고 인터페이스를 사용하여 최대 2가지 관계 수준 내에서 필터 개체에 연결된 다른 개체를 참조할 수 있습니다.

예를 들어, 문제 필터의 Portfolio ID를 참조하여 표준 인터페이스를 사용하여 특정 포트폴리오와 연결된 프로젝트의 문제만 표시할 수 있습니다. 이 경우 포트폴리오는 문제에서 2단계 떨어져 있습니다.

하지만 표준 인터페이스를 사용하여 문제 필터에서 Portfolio 소유자를 참조하여 소유자가 특정 사용자인 포트폴리오와 연결된 프로젝트의 문제만 표시할 수는 없습니다. Portfolio 소유자 이름 필드에 액세스하려면 텍스트 모드를 사용해야 합니다. 이 필드는 문제로부터 3단계 떨어져 있습니다.

![포트폴리오 소유자 아이콘 문제](assets/issue-to-portfolio-owner-sraight-line-icons-350x83.png)
...
객체 계층의 여러 레벨에 걸쳐 있는 필터 또는 누락된 객체에 대한 필터를 생성할 때 다음 사항을 고려하십시오.
...
1. (Conditional) If the Original Object (Issue) and the Target Field (ownerID) are not directly connected to each other, you must find a third object, a Linking Object (Project) that connects them. 연결 객체에는 원본 객체의 필드 또는 참조 탭(원본 객체에 표시된 연결 필드)에서 참조하는 하나 이상의 필드가 있어야 하며 대상 객체로의 연결 필드도 있어야 합니다. 연결 개체에 표시되는 대상 개체로의 연결 필드(또는 연결 개체에 표시되는 연결 필드)는 대상 필드와 일치해야 합니다.
...
1. API 탐색기를 사용하여 연결 개체(프로젝트)의 **개체 코드**&#x200B;를 식별하십시오.\
   예를들어, Project의 개체 코드는 PROJ입니다.\
   ![project_objCode_in_the_API.PNG](assets/project-objcode-in-the-api-350x84.png)

1. 원본 객체에 대한 필터를 만듭니다.\
   예를 들어 문제 필터를 만듭니다.\
...
1. 새 필터의 텍스트 모드 인터페이스에 다음 공식 예제를 붙여 넣고 예제 텍스트를 올바른 개체 및 필드로 바꿉니다.
   ...
   위에서 식별한 필드를 사용하는 예제는 이 문서의 [예 1: Portfolio 소유자 이름별 문제 필터링](#example-1-filter-for-issues-by-portfolio-owner-name) 섹션을 참조하십시오.

1. **필터 저장**&#x200B;을 클릭합니다.

## 누락된 개체에 대한 복잡한 텍스트 모드 필터 만들기
...
1. **보고서에 대한 필터 규칙 설정** 영역에 다음 코드를 붙여넣어 위의 일반 코드를 바꿉니다.
   ...
   >* 대상 개체는 범주입니다.
   >* 연결 개체는 범주 매개 변수입니다.
   >* 연결 개체의 개체 코드는 CTGIA입니다.
   >* 대상 객체에 대한 연결 필드는 parameterID입니다. parameterID는 연결 객체 테이블과 대상 객체 테이블 모두에 존재하기 때문입니다.
   >* 원래 개체에 표시되는 연결 필드는 범주 매개 변수의 ID입니다.
...
1. **보고서에 대한 필터 규칙 설정** 영역에 다음 코드를 붙여넣어 위의 일반 코드를 바꿉니다.
   ...
   >[!NOTE]
   >
   >* 원본 개체 는 보고서의 개체입니다. 사용자
   >* 대상 개체는 시간입니다.
   >* 사용자 및 시간은 Workfront 데이터베이스에서 직접 연결되므로 이 예제에서는 연결 개체가 필요하지 않습니다.
   >* 연결 개체가 없으므로 대상 개체의 개체 코드 HOUR을 사용해야 합니다.
   >* 대상 객체에 대한 연결 필드는 ownerID입니다(원본 객체에 표시되고 연결 객체가 누락됨).
   >* 원본 개체에 표시되는 연결 필드는 대상 개체에 표시되는 ID(시간 단위)입니다. 연결 개체가 없습니다.
   >* EXISTS:A:entryDate 문은 대상 개체(시간)를 정의하는 필드를 나타내며 일반 필터 문과 동일한 구문을 사용합니다. 이렇게 하면 특정 기간(이 경우 이전 주) 동안 시간을 기록하지 않은 사용자만 표시할 수 있습니다.
