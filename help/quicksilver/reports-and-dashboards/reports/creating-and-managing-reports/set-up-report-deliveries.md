...
보고서가 정의된 일정에 따라 사용자에게 자동으로 전달되도록 예약하거나 보고서를 수동으로 한 번만 보낼 수 있습니다. Adobe Workfront에서 보고서를 보내면 사용자는 별도의 첨부 파일로 Workfront 보고서가 포함된 이메일을 받습니다.
...
* 지정된 보고서에 대해 최대 10개의 반복 보고서 납품을 예약할 수 있습니다.
* 보고서를 작성한 경우에만 보고서가 전달되도록 예약할 수 있습니다. 작성하지 않은 보고서를 보내야 하는 경우 수동으로 보낼 수 있습니다.

## 내보내기 제한

Workfront에서 보고서가 표시되는 방식과 수동 내보내기, 배달된 보고서 또는 API를 통해 내보내는 방식에 영향을 주는 몇 가지 크기 제한이 있습니다.

* **10MB 파일 크기:** 배달되도록 예약된 내보낸 모든 보고서의 파일 크기 제한. 이메일에 첨부된 내보낸 파일이 5MB보다 큰 경우 첨부된 내보낸 보고서 대신 파일을 다운로드할 수 있는 링크가 이메일로 전송됩니다.

  >[!NOTE]
  >
  >10MB보다 큰 Excel .xlsx 파일은 전자 메일을 생성하지 않습니다. 수동으로 보고서를 이 형식으로 내보낼 수 있습니다. 보고서 내보내기에 대한 자세한 내용은 [데이터 내보내기](../../../reports-and-dashboards/reports/creating-and-managing-reports/export-data.md)를 참조하세요.

* **50,000개의 행:** .pdf 및 탭으로 구분된 파일에 대해 보고서 내보내기에서 허용되는 데이터 행 수입니다.
   ...
   * 프로젝트에 대한 활용률 정보 익스포트

     프로젝트의 사용률 정보 내보내기에 대한 자세한 내용은 [리소스 사용률 보고서 개요](../../../reports-and-dashboards/reports/using-built-in-reports/resource-utilization-report.md)를 참조하십시오.

* **65,530개의 하이퍼링크:** 하이퍼링크가 65,530개를 초과하는 문서에 Excel에서 지정한 제한입니다. 이러한 문서는 수동으로 내보내거나 게재된 보고서에서 전송할 때 열 수 없습니다. Excel 문서에는 200개의 데이터 행만 있을 수 있지만 문서 내에 65,530개가 넘는 링크가 있으면 문서가 열리지 않습니다. 이 제한은 지원되는 다른 형식이 아닌 Excel 파일에만 적용됩니다.
* **256열**: 열이 256개를 초과하는 문서에 Excel에서 지정한 제한입니다. 이러한 문서는 수동으로 내보내거나 게재된 보고서에서 전송할 수 없습니다. 이 제한은 지원되는 다른 형식이 아닌 Excel 파일에만 존재합니다.

한도를 초과하여 데이터를 내보내려고 하면 내보내기에서 필요한 데이터를 모두 받지 못할 수 있습니다. 대신 이 제한 범위 내에서 수정된 보고서가 생성됩니다.

또한 실행하는 데 60분 이상 걸리는 보고서가 중단됩니다.

제한에 관한 문제 또는 문제가 있는 경우 Workfront 기술 지원 센터에 문의하십시오.
...
* 브라우저에서 보고서를 볼 때 보고서의 타임스탬프와 형식은 브라우저 설정에 정의된 대로 브라우저의 로케일과 시간대와 일치합니다.
* 보고서가 전자 메일로 전송되면 Workfront 프로필에 지정된 사용자 로케일 및 시간대와 일치하는 타임스탬프 및 형식으로 보고서가 전달됩니다.\
  Workfront의 사용자 로캘 및 시간대에 대한 자세한 내용은 [사용자 프로필 편집](../../../administration-and-setup/add-users/create-and-manage-users/edit-a-users-profile.md) 문서를 참조하세요.

## 특수 보기가 있는 보고서 {#reports-with-a-special-view}

보고서에 특수 보기를 적용하면 Workfront의 보고서 [세부 정보] 탭에 특수 보기가 표시됩니다.

특수 보기가 있는 보고서 전달을 예약하면 세부 정보 탭이 특수 보기 대신 보낸 이메일 첨부 파일로 전달됩니다.
...
>[!NOTE]
>
>보고서에 Matrix 탭이 있고 기본 탭도 있는 경우에는 Matrix 탭에 해당 보고서가 표시됩니다.
...
Workfront에서 보고서를 보내면 사용자는 보고서가 첨부된 이메일을 별도의 첨부 파일로 받습니다.

* [제목, 첨부 파일 이름 및 보고서 제목](#subject-line-attachment-name-and-report-title)
* [타임스탬프](#timestamps)
* [브랜딩](#branding)
* [서식 지정](#formatting)
* [링크](#links)
...
파일 형식이 .pdf인 경우에만 첨부된 파일에 타임스탬프가 표시됩니다. 타임스탬프는 첨부된 파일의 바닥글에 있습니다.
...
Workfront 관리자가 Workfront 인스턴스에 사용자 정의된 브랜딩을 추가한 경우 .pdf 형식으로 보내지는 보고서에는 개인 맞춤화된 로고도 포함됩니다.

다른 모든 형식으로 전송된 보고서는 로고로 개인화할 수 없습니다.

Workfront 인스턴스 브랜딩에 대한 자세한 내용은 [Adobe Workfront 인스턴스 브랜딩](../../../administration-and-setup/customize-workfront/brand-workfront/brand-your-workfront-instance.md) 문서를 참조하십시오.
...
특수 보기로 보고서를 받는 방법에 대한 자세한 내용은 [특수 보기로 보고서](#reports-with-a-special-view) 문서를 참조하세요.\
...
다음을 작성하여 보고서를 전달하도록 구성했는지 여부를 확인할 수 있습니다.

* 목록 또는 보고서에 대한 보고서의 보고서 개체에 대한 **보기**: 보고서 목록 또는 보고서에 대한 보고서에 보기를 만들고 다음 열을 보기에 추가합니다.\
  *예약된 보고서 이름\
...
* **보고서 개체에 대한 필터**: 보고서 목록 또는 보고서에 다음 문으로 필터를 만듭니다. *예약된 보고서 ID가 비어 있지 않습니다*.\
  이렇게 하면 목록이나 보고서에 예약된 보고서만 표시됩니다.\
