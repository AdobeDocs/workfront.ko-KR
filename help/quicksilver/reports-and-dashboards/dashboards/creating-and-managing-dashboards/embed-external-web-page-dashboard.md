...
>[!IMPORTANT]
>
>* 보안상의 이유로 일부 웹 사이트에서는 웹 페이지를 iframe으로 포함할 수 없습니다. 대시보드에 포함하려는 웹 페이지에서 이를 허용하지 않는 경우 대시보드에 페이지가 표시되지 않습니다. 그러나 대시보드의 이름을 눌러 외부 페이지에 액세스할 수 있습니다.\
>![빈 외부 페이지 보고서](assets/qs-empty-external-page-report-350x165.png)\
>소유하고 있는 웹 사이트에 포함할 수 있도록 허용하려면 웹 관리자와 함께 **X-프레임 옵션** 설정을 조정하세요. 자세한 내용은 [X-프레임 옵션](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Frame-Options)을 참조하세요.
>
>
...
>
>대체 해결 방법으로 [대시보드에 보고서 추가](/help/quicksilver/reports-and-dashboards/dashboards/creating-and-managing-dashboards/add-report-dashboard.md)에 설명된 대로 목록 보고서를 대시보드에 포함하는 것이 좋습니다.
...
1. Workfront에 표시할 페이지의 URL을 찾아 주소 표시줄에 있는 URL을 복사합니다.
   ...
   >[!NOTE]
   >
   >Workfront 개체에 대한 URL을 공유하는 경우 일부 URL은 시간이 경과함에 따라 만료됩니다. 예를 들어 문서 URL은 열린 후에 만료됩니다. 이는 보안 수단으로 구성되며, 설계상 비 정적 URL로 간주되어 공유해서는 안 됩니다.
...
1. 외부 페이지에 대한 다음 정보를 지정합니다.
   ...
   * **URL**: 이전에 복사한 URL을 이 필드에 붙여 넣습니다.
      ...
      * 특정 웹 사이트에 대한 세션 정보를 포함하는 템플릿 URL입니다.\
        예: *https://localhost/?session={!$$SESSION}*
외부 페이지를 표시하려면 지정된 웹 사이트에 로그인해야 합니다.\
        Workfront에서 SessionID를 가져오는 방법에 대한 자세한 내용은 [API 기본 사항](../../../wf-api/general/api-basics.md)을 참조하세요.\
        Workfront 관리자는 보안상의 이유로 외부 페이지에서 세션 정보를 사용하지 못하도록 시스템 환경 설정을 구성할 수 있습니다. 이 경우 외부 페이지가 대시보드에 로드되지 않습니다.\
        시스템 보안 기본 설정에 대한 자세한 내용은 [시스템 보안 기본 설정 구성](../../../administration-and-setup/manage-workfront/security/configure-security-preferences.md)을 참조하세요.\
...
1. **대시보드 세부 정보** 상자의 **레이아웃 선택/보고서 추가** 영역에서 업데이트할 외부 페이지를 찾은 다음 그 위에 마우스를 가져간 다음 **편집** 아이콘을 클릭합니다.\
