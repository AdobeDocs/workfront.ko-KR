...
1. 첫 번째 (검색) 모듈을 마우스 오른쪽 버튼으로 클릭하고 를 선택합니다. **모듈 삭제**.
...
1. 필터 필드에서 을(를) 선택합니다 **새 레코드만**.
1. [출력] 상자에서 `ID`, `Name`, 및 `Project ID`.
1. 클릭 **확인** 모듈 설정을 저장합니다.
...
1. 실행 시나리오 필드에서 을(를) 선택합니다 **일정한 간격으로**.
...
* 웹후크에 대한 자세한 내용은 [의 인스턴트 트리거(웹후크) [!DNL Adobe Workfront Fusion]](/help/quicksilver/workfront-fusion/webhooks/instant-triggers-webhooks.md).
