...
title: XML
description: XML 앱을 사용하면 XML &gt;를 통해 XML 형식의 텍스트를 구문 분석할 수 있습니다 XML 모듈을 구문 분석하고 번들로 변환하여 다른 모듈에서 데이터를 사용할 수 있도록 합니다. XML &gt;를 통해 번들을 XML 형식의 텍스트로 변환할 수도 있습니다 XML 모듈 만들기
author: Becky
...
다음 [!UICONTROL XML] 앱을 사용하면 [!UICONTROL XML] > [!UICONTROL XML 구문 분석] 모듈로 변환하여 다른 모듈에서 데이터를 사용할 수 있도록 합니다. 또한 번들을 [!UICONTROL XML] > [!UICONTROL XML 만들기] 모듈
...
다음 [!UICONTROL XML] > [!UICONTROL XML 구문 분석] 모듈은 XML 형식의 텍스트를 구문 분석하고 XML에서 추출된 모든 정보를 포함하는 단일 번들을 출력합니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>데이터 구조는 XML의 구조를 설명하여 다음 모듈의 매핑 패널에서 모듈의 출력을 사용할 수 있도록 합니다.</p> <p>구문 분석하려는 XML 샘플이 있는 경우 이 샘플을 사용하여 데이터 구조를 생성할 수 있습니다.</p> 
    <ol> 
     ...
     <li value="4">클릭 <strong>[!UICONTROL Save]</strong>.</li> 
     <li value="5">데이터 구조가 성공적으로 생성되었는지 확인합니다.</li> 
     <li value="6"> <p>을(를) 클릭합니다. <strong>[!UICONTROL Save]</strong> 단추를 클릭하여 데이터 구조를 저장합니다.</p> <p>2-5단계를 건너뛰고 빈 데이터 구조를 제공할 수 있습니다. 데이터 구조가 비어 있으면 모듈이 적어도 한 번 이상 실행될 때까지 매핑 패널에서 모듈의 출력을 사용할 수 없습니다.</p> </li> 
  ...
  <tr> 
   ...
   <td>숫자가 텍스트(문자열) 값으로 유지되도록 하려면 이 옵션을 활성화합니다. 그렇지 않으면 숫자가 숫자 값으로 캐스팅됩니다.</td> 
  </tr> 
  <tr> 
   <td role="rowheader"> <p>[!UICONTROL XML]</p> </td> 
   <td> <p>구문 분석할 XML 형식 텍스트를 입력하거나 매핑합니다.</p> <p>수식을 사용하는 경우 결과 값 유형이 [!UICONTROL Text] 데이터 유형인지(또는 자동으로 을) 강제 적용할 수 있는지 확인하십시오. </p> <p> <img src="assets/if-you-use-a-formula-350x164.png" style="width: 350;height: 164;"> </p> <p>결과 값 유형이 [!UICONTROL Buffer](이진 데이터)이면 <code>toString()</code> 함수를 사용하여 텍스트 데이터 유형으로 변환합니다. 자세한 내용은 <a href="../../workfront-fusion/mapping/type-coercion.md" class="MCXref xref">형식 강제 적용 [!DNL Adobe Workfront Fusion]</a> 및 <a href="../../workfront-fusion/mapping/item-data-types.md" class="MCXref xref">[!UICONTROL Adobe Workfront Fusion]의 항목 데이터 유형</a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!INFO]
>
>**예:** URL에서 XML 파일을 다운로드하고 해당 컨텐츠를 구문 분석하려면:
>
>1. 새 시나리오를 만듭니다.
...
기본적으로 [!UICONTROL XML] > [!UICONTROL XML 구문 분석] 모듈은 속성을 특수 컬렉션에 추가합니다 `_attributes` 이러한 속성을 갖는 노드의 하위로. 노드가 텍스트 노드이고 특성이 있는 경우 두 개의 특수 속성이 추가됩니다. `_attributes` 속성에 대해 `_value` 를 입력합니다.
...
다음 [!UICONTROL XML] > [!UICONTROL XML 만들기] 모듈은 번들을 XML 형식 텍스트로 변환합니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td role="rowheader"> <p>[!UICONTROL 데이터 구조]</p> </td> 
   <td> <p>데이터 구조는 결과 XML의 구조를 설명합니다. 만들려는 XML의 샘플이 있는 경우 이 샘플을 사용하여 데이터 구조를 생성할 수 있습니다.</p> 
    <ol> 
     <li value="1">을(를) 클릭합니다. <strong>[!UICONTROL Add]</strong> 버튼을 클릭합니다.</li> 
     <li value="2">을(를) 클릭합니다. <strong>[!UICONTROL Generator]</strong> 버튼을 클릭합니다.</li> 
     <li value="3">XML 샘플을 복사하여 샘플 데이터 필드에 붙여넣습니다.</li> 
     <li value="4">을(를) 클릭합니다. <strong>[!UICONTROL Save]</strong> 버튼을 클릭합니다.</li> 
     <li value="5">데이터 구조가 성공적으로 생성되었는지 확인합니다.</li> 
     <li value="6">클릭 <strong>[!UICONTROL Save]</strong> 를 눌러 데이터 구조를 저장합니다.</li> 
    </ol> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL 루트 요소 이름]</td> 
   <td>XML의 루트 요소의 이름을 입력합니다. 기본값은 입니다. <code>root</code>.</td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL Doctype SYSTEM ID]</td> 
   <td>에 사용할 파일 이름을 입력합니다.<code> !DOCTYPE SYSTEM</code> 선언</td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL Doctype PUBLIC ID]</td> 
   <td>에 사용할 파일 이름을 입력합니다.<code> !DOCTYPE PUBLIC</code> 선언</td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL Strip Xml 선언]</td> 
   <td>XML 선언을 제거하려면 이 옵션을 사용합니다 <code>&lt;?xml ... ?&gt;</code> 및 <code>&lt;!DOCTYPE ... &gt;</code>XML 루트 요소와 해당 컨텐츠만 그대로 둡니다.</td> 
  </tr> 
 </tbody> 
</table>
...
복잡한 노드(다른 노드를 포함할 노드)에 속성을 추가하려면 이름을 사용하는 컬렉션을 추가해야 합니다 `_attributes` 참조하십시오. 이 컬렉션은 노드 특성에 매핑됩니다. 텍스트 노드에 속성을 추가하려면(예: `<node attr="1">abc</node>`), 컬렉션을 추가해야 합니다 `_attributes` 속성 및 텍스트 속성에 대해 `_value` 사용자 지정 데이터 구조의 이 노드에 대한 노드 값입니다.
...
## 문제 해결: 에서 데이터를 매핑할 수 없습니다. [!UICONTROL XML 구문 분석] 모듈

데이터 구조가 올바르게 정의되어 있는지 확인합니다. 또는 빈 데이터 구조를 사용하고 모듈을 한 번 이상 실행하여 XML 입력을 처리할 수도 있습니다.
