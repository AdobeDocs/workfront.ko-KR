...
title: 텍스트 구문 분석기
description: Text parser 도구를 사용하여 다른 곳에서 사용할 텍스트를 구문 분석할 수 있습니다 [!DNL Adobe Workfront Fusion] 시나리오 모듈입니다. 텍스트 파서는 연결할 필요가 없습니다.
author: Becky
...
다음을 사용할 수 있습니다. [!UICONTROL 텍스트 파서 도구] 기타 항목에 사용할 텍스트 구문 분석하기 [!DNL Adobe Workfront Fusion] 시나리오 모듈입니다. 다음 [!UICONTROL 텍스트 구문 분석기] 는 연결할 필요가 없습니다.
...
## [!UICONTROL 텍스트 구문 분석기] 모듈 및 해당 필드

를 구성할 때 [!UICONTROL 텍스트 구문 분석기] 모듈, [!DNL Adobe Workfront Fusion] 아래 나열된 필드를 표시합니다. 모듈의 굵은 제목은 필수 필드를 나타냅니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>이 옵션을 활성화하면 제목 태그로 둘러싸인 텍스트(예: &lt;h2&gt; &lt;/h2&gt;)을 대문자 텍스트로 변환합니다.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
다음 [!UICONTROL 패턴 일치] 모듈을 사용하면 지정된 텍스트에서 검색 패턴과 일치하는 문자열 요소를 찾고 추출할 수 있습니다. 이 모듈에서는 정규 표현식(regex 또는 regexp라고도 함)을 사용합니다.
...
* 메타문자의 전체 목록은 다음을 참조하십시오. [정규 표현식](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions) MDN 웹 문서에서.
* 정규 표현식을 만드는 방법에 대한 자습서를 보려면 다음을 권장합니다 [RegexOne](https://regexone.com/).
* 정규 표현식으로 실험하려면 다음을 수행하는 것이 좋습니다. [정규 표현식 101](https://regex101.com/) 웹 사이트입니다. 왼쪽 패널에서 ECMAScript(JavaScript) 버전 을 선택합니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td>[!UICONTROL Pattern] </td> 
   <td> <p>정규 표현식 패턴을 입력합니다. </p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>예: </b></span></span> <code>[+-]?(\d+(\.\d+)?|\.\d+)([eE][+-]?\d+)?</code> 제공된 텍스트에 있는 모든 숫자를 추출합니다.</p> <p>참고:  <p>패턴에는 괄호 안에 캡처 그룹이 하나 이상 있어야 합니다. <code>()</code>. 패턴에 캡처 그룹이 없으면 출력 번들은 비어 있습니다.</p> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>이 옵션을 활성화하여 시작 및 끝 메타문자(<code>^</code> 및 <code>$</code>)는 전체 입력 문자열의 시작 또는 끝뿐만 아니라 각 줄의 시작 또는 끝과 일치합니다.</p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL Singleline]</td> 
   <td>이 옵션을 활성화하여 마침표(.)가 줄바꿈 문자(<code>\n</code>).</td> 
  </tr> 
  <tr> 
...
데이터 스크래핑 도구가 REST API를 제공하는 경우 유니버설을 통해 연결할 수 있습니다 [[!UICONTROL HTTP] 모듈](../../workfront-fusion/apps-and-their-modules/http-modules/http-modules-1.md) 및 [웹훅](../../workfront-fusion/apps-and-their-modules/webhooks-updated.md) 모듈.
