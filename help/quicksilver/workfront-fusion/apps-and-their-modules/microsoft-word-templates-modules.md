...
다음에서 [!DNL Adobe Workfront Fusion] 시나리오에서는 다음을 사용하는 워크플로를 자동화할 수 있습니다 [!DNL Microsoft Word Templates]을 여러 타사 응용 프로그램 및 서비스에 연결할 수 있습니다.
...
예를 들어 다음 코드를 사용할 수 있습니다 [!DNL Microsoft Word] 템플릿:
...
A [!DNL Microsoft Word] 템플릿이 일반적입니다. [!DNL Microsoft Word] 데이터를 병합하거나 채우는 위치와 방법을 결정하는 특수 태그가 텍스트에 포함된 문서(.docx 파일). 태그는 다음 세 가지 유형이 있습니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>바꿀 텍스트의 시작 부분을 표시할 문자를 입력합니다. </p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>예: </b></span></span>입력 <code>[[</code> 이 텍스트와 유사한 텍스트를 바꾸려면 다음을 수행합니다. <code>[[replace_me]]</code></p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader"> <p>[!UICONTROL 바꿀 텍스트의 끝 구분 기호]</p> </td> 
   <td> <p>바꿀 텍스트의 끝을 표시할 문자를 입력합니다. </p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>예: </b></span></span>입력 <code>]]</code> 이 텍스트와 유사한 텍스트를 바꾸려면 다음을 수행합니다. <code>[[replace_me]]</code></p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>이는 다음과 같은 컬렉션의 배열이어야 합니다.</p> 
    <ul> 
     ...
     <li>항목 <code>entry </code>다음 항목의 컬렉션을 포함합니다. <code>key </code>및 <code>value</code></li> 
     <li>항목 <code>key </code>태그의 이름을 포함합니다.</li> 
     <li>항목 <code>value </code>태그의 값 포함</li> 
    </ul> 
...
### [!UICONTROL 데이터 배치로 문서 채우기] {#fill-a-document-with-a-batch-of-data}
...
데이터 항목이 배열로 나오는 경우에도 를 사용하여 이 모듈을 사용할 수 있습니다. *반복자* 배열의 콘텐츠를 일련의 번들로 변환하는 모듈입니다.
