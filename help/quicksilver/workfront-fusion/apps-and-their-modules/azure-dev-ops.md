...
description: 다음에서 [!DNL Adobe Workfront Fusion] 시나리오에서는 다음을 사용하는 워크플로를 자동화할 수 있습니다 [!DNL Azure DevOps]을 여러 타사 응용 프로그램 및 서비스에 연결할 수 있습니다.
author: Becky
...
다음에서 [!DNL Adobe Workfront Fusion] 시나리오에서는 다음을 사용하는 워크플로를 자동화할 수 있습니다 [!DNL Azure DevOps]을 여러 타사 응용 프로그램 및 서비스에 연결할 수 있습니다.
...
사용 [!DNL Azure DevOps] 모듈, 다음 항목이 있어야 합니다. [!DNL Azure] DevOps 계정.

## 연결 [!DNL Azure DevOps] 끝 [!DNL Workfront Fusion] {#connect-azure-devops-to-workfront-fusion}

1. 추가 [!DNL Azure DevOps] 을 시나리오에 연결합니다.
1. 클릭 **[!UICONTROL 추가]** 다음 옆에 [!UICONTROL 연결] 필드.
1. 다음에서 [!UICONTROL 연결 유형] 필드, 선택 **[!DNL Azure DevOps]**.

   >[!IMPORTANT]
   >
   >다음 [!UICONTROL [!DNL Azure DevOps] (모든 범위 요청)] 연결 유형은 가까운 시일 내에 더 이상 사용되지 않습니다. 따라서 사용하지 않는 것이 좋습니다.
...
1. 다음 필드를 채웁니다.
   ...
   <table style="table-layout:auto">
      ...
      <tr>
            <td>[!UICONTROL 조직]</td>
            <td>을(를) 생성한 조직의 이름을 입력합니다 [!DNL Azure DevOps] 응용 프로그램.</td>
        </tr>
    </table>

1. 클릭 **[!UICONTROL 계속]** 연결 설정을 완료하고 시나리오 만들기를 계속합니다.

## [!UICONTROL Azure 개발 운영] 모듈 및 해당 필드

를 구성할 때 [!DNL Azure DevOps] 모듈, [!DNL Workfront Fusion] 아래 나열된 필드를 표시합니다. 이와 함께 추가 [!DNL Azure DevOps] 앱이나 서비스의 액세스 수준 등에 따라 필드가 표시될 수 있습니다. 모듈의 굵은 제목은 필수 필드를 나타냅니다.
...
이 인스턴스 트리거 모듈은에서 레코드가 추가, 업데이트 또는 삭제될 때 시나리오를 실행합니다. [!UICONTROL Azure 개발 운영].
...
<table style="table-layout:auto">
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>모듈에 대한 웹후크를 선택하거나 추가합니다.</p> <p>트리거 모듈의 웹후크에 대한 자세한 내용은 <a href="../../workfront-fusion/webhooks/instant-triggers-webhooks.md" class="MCXref xref">의 인스턴트 트리거(웹후크) [!DNL Adobe Workfront Fusion]</a>.</p> <p>웹후크를 만드는 방법에 대한 자세한 내용은 <a href="../../workfront-fusion/apps-and-their-modules/webhooks-updated.md" class="MCXref xref">웹훅</a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
이 작업 모듈에서는 다음을 위한 사용자 지정 인증 호출을 만들 수 있습니다. [!DNL Azure DevOps] API. 이렇게 하면 다른 사용자가 수행할 수 없는 데이터 흐름 자동화를 만들 수 있습니다 [!DNL Azure DevOps] 모듈.
...
<table style="table-layout:auto">
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>연결에 대한 자세한 내용 [!DNL Azure DevOps] 계정 위치: [!DNL Workfront Fusion], 참조 <a href="#connect-azure-devops-to-workfront-fusion" class="MCXref xref">연결 [!DNL Azure DevOps] [!UICONTROL Workfront Fusion]으로</a> 이 문서에서.</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL 기본 URL]</td> 
   <td> <p>에 연결하는 데 사용하는 기본 URL을 선택하거나 매핑합니다. [!DNL Azure DevOps] account</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr data-mc-conditions=""> 
   ...
   <td>버전 선택 또는 매핑 [!DNL Azure DevOps] 이 API 호출을 위해 연결하려는 API입니다. 버전을 선택하지 않으면 [!DNL Workfront Fusion] 에 연결 [!DNL Azure DevOps] API 버전 5.1.</td> 
  </tr> 
  <tr> 
...
이 작업 모듈은 의 단일 레코드에서 데이터를 읽습니다. [!DNL Azure DevOps].
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>프로젝트를 읽을지 작업 항목을 읽을지 선택</p> 
    <ul> 
     <li> <p><strong>[!UICONTROL 프로젝트]</strong>: 읽을 프로젝트를 선택합니다.</p> </li> 
     <li> <p><strong>[!UICONTROL 작업 항목]</strong>: 읽고자 하는 작업 항목이 포함된 프로젝트를 선택한 다음 작업 항목 유형을 선택합니다.</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
    ...
    <ul> 
     <li> <p><strong>[!UICONTROL 프로젝트]</strong> </p> <p>다음 필드를 채웁니다.</p> 
      <ul> 
       ...
       <li> <p><strong>[!UICONTROL 가시성]</strong>: 프로젝트를 공개 또는 비공개로 할지 선택합니다. 비공개 프로젝트와 상호 작용하려면 사용자에게 조직에 로그인하고 프로젝트에 대한 액세스 권한이 부여되어야 합니다. 조직에 로그인하지 않은 사용자가 공개 프로젝트를 볼 수 있습니다.</p> </li> 
       <li> <p><strong>[!UICONTROL Version control]</strong>: 프로젝트를 사용할지 여부를 선택합니다 [!DNL Git] 또는 버전 제어용 [!UICONTROL Team Foundation Version Control(TFCV)]을 참조하십시오.</p> </li> 
       <li> <p><strong>[!UICONTROL 작업 항목 프로세스]</strong>: 프로젝트에 사용할 작업 프로세스를 선택합니다. 옵션은 [!UICONTROL Basic], [!UICONTROL Scrum], [!UICONTROL Capability Maturity Model Integration(CMMI)] 및 [!UICONTROL Agile]입니다.</p> <p>에 대한 자세한 내용 [!DNL Azure DevOps] 프로세스, 참조 <a href="https://docs.microsoft.com/en-us/azure/devops/boards/work-items/guidance/choose-process?view=azure-devops&amp;tabs=basic-process">프로세스 선택</a> 다음에서 [!DNL Azure DevOps] 설명서입니다.</p> </li> 
      </ul> </li> 
     <li> <p><strong>[!UICONTROL 작업 항목]</strong> </p> <p>다음 필드를 채웁니다.</p> 
      <ul> 
       <li> <p><strong>[!UICONTROL 프로젝트]</strong>: 작업 항목을 만들 프로젝트를 선택합니다.</p> </li> 
       <li> <p><strong>[!UICONTROL 작업 항목 유형]</strong>: 만들려는 작업 항목의 유형을 선택합니다.</p> </li> 
       <li> <p><strong>[!UICONTROL 기타 필드]</strong>:이 필드에 주어진 속성에 대해 작업 항목에 지정할 값을 입력합니다. 사용 가능한 필드는 작업 항목 유형에 따라 다릅니다.</p> </li> 
      </ul> </li> 
    </ul> </td> 
  </tr> 
 </tbody> 
...
이 작업 모듈은 의 특정 유형에 있는 모든 작업 항목을 [!DNL Azure DevOps] 프로젝트.
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>다음을 실행할 최대 작업 항목 수를 입력하거나 매핑합니다. [!DNL Workfront Fusion] 는 한 실행 주기 동안 를 반환합니다.</td> 
  </tr> 
 </tbody> 
</table>
