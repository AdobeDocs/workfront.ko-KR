...
[!UICONTROL 도구] 모듈은 앱 목록 또는 [!UICONTROL 도구] 아이콘 ![](assets/tools-icon-small.png) 화면 맨 아래에.
...
예를 들어, 연락처 또는 지정된 이메일 주소(예: )로 전송되도록 예약된 기타 목록에 대해 이 모듈을 사용할 수 있습니다 [!UICONTROL 이메일] >[!UICONTROL 이메일 보내기], 또는 [!DNL Gmail] >[!UICONTROL 이메일 보내기] 또는 를 간단한 미리 알림으로 사용하여 언제든지 트리거할 수 있습니다.
...
이 모듈은 변수가 가 설정된 경로와 다른 경로에 설정된 경우에도 시나리오의 어느 위치에나 설정된 변수를 읽을 수 있습니다. [!UICONTROL 여러 변수 가져오기] 모듈이 있습니다. 유일한 요구 사항은 [!UICONTROL 도구] > [!UICONTROL 변수 설정] 또는 [!UICONTROL 도구] > [!UICONTROL 여러 변수 설정] 모듈은 다음 작업 전에 실행됩니다. [!UICONTROL 도구] > [!UICONTROL 여러 변수 가져오기] 모듈. 모듈이 실행되는 순서에 대한 자세한 내용은 [의 라우터 모듈 [!DNL Adobe Workfront Fusion]](../../workfront-fusion/modules/router-module.md).
...
>[!INFO]
>
>**예:**  다음은 를 사용할 수 있는 방법입니다 [!UICONTROL 설정]/[!UICONTROL (여러) 변수 가져오기] 모듈:
>
>* 다른 경로에서도 나중에 사용할 수 있도록 계산된 값을 저장합니다. 이 기능은 값이 여러 모듈에 사용되고 값을 계산하는 수식이 너무 복잡한 경우에 유용합니다.
>* 수식을 디버깅합니다. 모듈에 사용된 수식이 올바른 결과를 제공하지 않는 경우 수식을 복사하여 [!UICONTROL 변수 설정] 관련 모듈 앞에 삽입하는 모듈입니다. 다음 작업 후 모듈 연결 해제 [!UICONTROL 변수 설정] 모듈을 지정하고 시나리오를 실행합니다. 확인 [!UICONTROL 변수 설정] 모듈의 출력, 공식을 조정 또는 단순화하고 시나리오를 다시 실행한 다음 문제가 해결될 때까지 계속 실행합니다.


#### [!UICONTROL 변수 가져오기]

이 모듈은 이전에 만든 값을 [!UICONTROL 변수 설정] 또는 [!UICONTROL 여러 변수 설정] 모듈.

이 모듈은 변수가 가 설정된 경로와 다른 경로에 설정된 경우에도 시나리오의 어느 위치에나 설정된 변수를 읽을 수 있습니다. [!UICONTROL 변수 가져오기] 모듈이 있습니다. 유일한 요구 사항은 [!UICONTROL 도구] > [!UICONTROL 변수 설정] 또는 [!UICONTROL 도구] > [!UICONTROL 여러 변수 설정] 모듈은 다음 작업 전에 실행됩니다. [!UICONTROL 도구] > [!UICONTROL 변수 가져오기] 모듈. 모듈이 실행되는 순서에 대한 자세한 내용은 [의 라우터 모듈 [!DNL Adobe Workfront Fusion]](../../workfront-fusion/modules/router-module.md).
...
>1. 모듈의 설정 **[!UICONTROL 값 재설정]** 필드: 절대 안 함
>1. 홀수 값에 대한 경로를 설정합니다. 다음과 같은 계수 수학 함수를 사용하여 이 경로에 대한 필터를 설정합니다. `1`:
>
>   ![](assets/odd-350x459.png)
>
>  **참고**: 다음 사항을 변경하는 것을 잊지 마십시오 [!UICONTROL 다음과 같음] 기본값의 연산자 [!UICONTROL 텍스트] 연산자 - [!UICONTROL 숫자] 연산자.
>
>1. 동일한 계수 수학 함수를 사용하여 짝수 값에 대한 경로 설정 `0`:
>
>증가 함수는 시나리오가 실행될 때마다 1을 추가합니다. 필터는 증가를 확인하고 해당 값에 따라 작동하여 이메일이 균일하게 배포되도록 합니다.

#### [!UICONTROL 여러 변수 설정]

이 모듈은 경로의 다른 모듈에서 매핑할 수 있는 변수를 만듭니다. 변수를 [!UICONTROL 변수 가져오기] 또는 [!UICONTROL 여러 변수 가져오기] 시나리오에서 모든 경로에 대한 모듈입니다.
...
이 함수는 예를 들어 를 낮추려는 경우 유용합니다 [!DNL target] 서비스 서버는 대량 SMS 또는 이메일을 보낼 때 사람의 동작을 모방하거나 로드합니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>집계된 출력을 그룹화할 표현식을 정의합니다. 이 표현식은 하나 이상의 매핑된 항목을 포함할 수 있습니다. 그런 다음 집계된 데이터는 이 표현식의 값을 사용하여 그룹으로 구분됩니다. 각 그룹은 키(평가된 표현식)와 값(집계된 값)이 있는 별도의 번들로 출력됩니다. 키는 후속 모듈에서 필터로 사용할 수 있습니다.</p> </td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>[!UICONTROL Group by]</p> </td> 
   <td> <p>집계된 출력을 그룹화할 표현식을 정의합니다. 이 표현식은 하나 이상의 매핑된 항목을 포함할 수 있습니다. 그런 다음 이 표현식의 값을 사용하여 집계된 데이터가 그룹으로 분리됩니다. 각 그룹은 키(평가된 표현식)와 값(집계된 값)이 있는 별도의 번들로 출력됩니다. 키는 후속 모듈에서 필터로 사용할 수 있습니다.</p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 빈 집계 후 처리 중지]</td> 
...
>[!INFO]
>
>**예:** 텍스트 집계기를 사용하여 단일 번들에 더 많은 값(예: 고객 이름 또는 메모)을 삽입하고 이메일 본문 또는 이메일 제목의 모든 값이 포함된 이메일을 보낼 수 있습니다.
  ...
  <tr> 
   ...
   <td> <p>정규 표현식을 사용하려면 이 옵션을 활성화합니다. 모듈은 정확한 일치가 아닌 정규 표현식을 기반으로 사례를 결정합니다.</p> 
    <div> 
     ...
     <ul> 
      <li>메타문자의 전체 목록은 다음을 참조하십시오. <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions">정규 표현식</a> MDN 웹 문서에서.</li> 
      <li>정규 표현식을 만드는 방법에 대한 자습서를 보려면 다음을 권장합니다 <a href="https://regexone.com/">RegexOne</a>.</li> 
      <li>정규 표현식으로 실험하려면 다음을 수행하는 것이 좋습니다. <a href="https://regex101.com/">정규 표현식 101</a> 웹 사이트입니다. 왼쪽 패널에서 ECMAScript(JavaScript) 버전 을 선택합니다.</li> 
     </ul> 
    </div> </td> 
  </tr> 
  <tr> 
