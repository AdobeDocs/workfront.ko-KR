...
<pre>{<br>    "data": [<br>        {<br>            "percentComplete": 0,<br>            "status": "CUR",<br>            "priority": 2,<br>            "name": "새 프로젝트",<br>            "ID": "4c7c08b20000002de5ca1ebc19edf2d5" <br>        } <br>    ] <br>}</pre>

>[!NOTE]
>
>브라우저의 주소 표시줄을 통해 GET 요청을 실행할 때 sessionID를 요청의 일부로 포함할 필요가 없습니다.

PUT, POST 및 DELETE 요청에 대해 특수 보안이 추가되었습니다. 데이터베이스에 쓰거나 데이터베이스에서 삭제하는 모든 요청은 **sessionID=abc123** 는 URI에 포함됩니다. 다음 예제는 DELETE 요청을 검색하는 방법을 보여 줍니다.
...
1. URL을 /attask/api/v15.0/project/search로 변경합니다.\
   페이지를 찾을 수 없습니다.
1. 단어 바꾸기 *검색* 로그인?사용자 이름=admin&amp;password=user를 사용하여 사용자 이름 및 암호 대체 *admin* 및 *사용자\
   *이 세션은 브라우저에 쿠키로 저장되며 후속 GET 요청마다 다시 기술할 필요가 없습니다.

1. URL을 다음으로 변경 **/attask/api/v15.0/project/search**.
1. 제공된 응답에 주목합니다.
...
/attask/api/v15.0/project?id=... 요청이 와 동일합니다. `/attask/api/v15.0/project/...` 요청.
...
<pre>GET /attask/api/v15.0/task/search?name=Planning<br>&amp;name_Mod=contains<br>또는(&amp;O):1:portfolio:name=FixedAssets<br>또는(&amp;O):1:portfolio:name_Mod=eq<br>또는(&amp;O):1:assignedTo:name=Steve<br>또는(&amp;O):1:assignedTo:name_Mod=cicontains<br>또는(&amp;O):2:parent:name=최종 작업<br>또는(&amp;O):2:parent:name_Mod=eq
</pre>
...
<pre>{<br>    "name": "중요한 문제",<br>    "ID": "4c78285f00000908ea8cfd66e084939f",<br>    "owner": {<br>        "title": "Operations Specialist"<br>        "phoneNumber": "555-1234",<br>        "name": "관리자",<br>        "ID": "4c76ed7a0000054c172b2c2d9f7f81c3" <br>    } <br>}</pre>
...
기본 결과 수 쿼리 제한을 재정의하고 200개의 결과를 허용하려면 다음을 포함할 수 있습니다. `$$LIMIT=200` 다음 예와 같이 쿼리에서 필터링합니다.
<pre>GET /attask/api/v15.0/project/search?$$LIMIT=200</pre>

시스템의 다른 테넌트에 대한 신뢰성과 성능을 보장하기 위해 쿼리당 허용되는 최대 결과 제한은 2000개입니다. 더 큰 제한을 지정하려고 하면 `IllegalArgumentException` 오류 메시지입니다. 

따라서 큰 데이터 세트에 대해 페이지 번호를 매긴 응답을 사용하는 것이 좋습니다. 반환해야 하는 첫 번째 결과를 지정하려면 `$$FIRST` 필터. 예를 들어 다음 요청은 쿼리에 대한 결과 201-250을 반환합니다.
<pre>GET /attask/api/v15.0/project/search?$$FIRST=200&amp;$$LIMIT=50</pre>

위의 예에서 `$$FIRST=200` 201번째 결과를 반환합니다. `$$FIRST=0` 는 첫 번째 결과를 반환합니다. $$FIRST 값을 결과를 반환하기 전에 건너뛸 결과 수로 간주하는 것이 도움이 될 수 있습니다.

결과에 페이지가 올바르게 매겨졌는지 확인하려면 정렬 매개 변수를 사용하십시오. 이렇게 하면 결과를 같은 순서로 반환할 수 있으므로 페이지 매김이 반복되거나 결과를 건너뛰지 않습니다. 예를 들어 개체 ID를 사용하여 정렬하려면 `ID_Sort=asc`.
...
<pre>POST /attask/api/v15.0/document?updates={<br>    이름: aFileName,<br>    핸들: abc...123, (파일 업로드의 핸들)<br>    docObjCode: PROJ, (또는 TASK, OPTASK 등)<br>    objID: abc...123,<br>    currentVersion:{version:v1.0,fileName:aFileName}<br>}</pre>
