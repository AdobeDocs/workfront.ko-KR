...
기본 결과 수 쿼리 제한을 무시하고 200개의 결과를 허용하려면 다음을 포함할 수 있습니다 `$$LIMIT=200` 다음 예와 같이 쿼리의 필터입니다.
<pre>GET /attask/api/v15.0/project/search?$$LIMIT=200</pre>

시스템의 다른 테넌트에 대한 안정성 및 성능을 보장하기 위해 쿼리당 허용되는 최대 결과 제한은 2000개의 개체입니다. 더 큰 제한을 지정하려고 하면 `IllegalArgumentException` 오류 메시지. 

따라서 큰 데이터 세트에 대해 페이지 매김된 응답 사용을 고려해 보는 것이 좋습니다. 반환할 첫 번째 결과를 지정하려면 `$$FIRST` 필터. 예를 들어 다음 요청은 쿼리에 대한 결과 201-250을 반환합니다.
<pre>GET /attask/api/v15.0/project/search?$$FIRST=200&amp;$$LIMIT=50</pre>

위의 예에서 `$$FIRST=200` 201번째 결과를 반환합니다. `$$FIRST=0` 첫 번째 결과를 반환합니다. 따라서 결과를 반환하기 전에 건너뛸 결과 수로 $$FIRST 값을 고려하는 것이 도움이 될 수 있습니다.

결과가 올바르게 페이지 매김되었는지 확인하려면 정렬 매개 변수를 사용하십시오. 이렇게 하면 페이지 매김이 결과를 반복하거나 건너뛰지 않도록 결과가 동일한 순서로 반환될 수 있습니다. 예를 들어 개체 ID를 사용하여 정렬하려면 `ID_Sort=asc`.
