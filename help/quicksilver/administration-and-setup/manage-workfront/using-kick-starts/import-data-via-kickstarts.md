...
킥스타트 템플릿을 내보내면 빈 Excel 스프레드시트 통합 문서가 표시됩니다. 이 문서의 후속 절차에서는 통합 문서를 사용자 정보로 채운 다음 다시 Workfront으로 가져옵니다.

킥스타트 템플릿을 내보내려면 다음을 수행하십시오.

1. 다음을 클릭합니다. **메인 메뉴** 아이콘 ![](assets/main-menu-icon.png) Adobe Workfront의 오른쪽 상단에서 을(를) 클릭한 다음 **설정** ![](assets/gear-icon-settings.png).

1. 클릭 **시스템** > **데이터 내보내기(킥스타트)**.

1. 클릭 **추가 옵션**&#x200B;을(를) 클릭한 후 포함할 정보 유형을 선택합니다.
...
1. 클릭 **다운로드**.
1. 계속 [데이터로 스프레드시트 템플릿 채우기](#populate-the-spreadsheet-template-with-your-data) 템플릿을 사용자 정보로 채웁니다.
...
>
>* 이러한 필드는 어떤 방식으로든 삭제하거나 수정하지 마십시오. 예를 들어, 순서나 이름을 변경하지 마십시오.
>* 모든 필드를 굵은 글꼴로 표시된 열 머리글로 채웁니다. 필수 필드를 나타냅니다.
>
...
1. (선택 사항) 사용자 지정 데이터를 추가하려면:
   ...
   * 해당 사용자 정의 필드에 대해 각 새 열의 이름을 다음과 같이 지정합니다. **DE:[Workfront에 표시되는 사용자 정의 필드의 이름]**.
...
>[!TIP]
>
>대부분의 사람들은 MM/DD/YYYY 형식(예: 07/10/2022)을 사용하는 것이 가장 쉽다고 생각합니다.

Workfront에서도 시간 값을 날짜의 일부로 허용합니다(예: 07/10/2022 01:30 또는 07/10/2022 1:00 PM).

날짜에서 시간을 생략할 경우 Workfront은 다음 중 하나를 수행합니다.

* 오전 12시를 가정합니다. 예상 날짜 결과를 보려면 시스템 시간대가 사용자의 시간대와 일치해야 합니다.
...
* **(프로젝트 가져오기)**
  ...
  위에서 설명한 예에 대해 속성 이름 대체를 수행하려면 다음을 변경합니다 **집합 ID** 열 머리글에서 다음으로 **#setGroupID **** 이름**. 그런 다음 이름별로 각 프로젝트의 그룹을 참조할 수 있습니다.
