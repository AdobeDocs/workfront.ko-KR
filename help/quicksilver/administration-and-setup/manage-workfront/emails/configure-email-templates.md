...
이메일 템플릿에는 미리 알림 알림이 시작되면 사용자에게 전송되는 메시지가 포함됩니다.\
...
1. **새 전자 메일 서식 파일** 상자에서 다음 정보를 지정하십시오.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td>템플릿을 연결할 개체 유형을 지정합니다. 다음 개체 중에서 선택합니다.
      <ul>
      <li>프로젝트</li>
      <li>작업</li>
      <li>문제</li>
   ...
   필수 필드이며 기본적으로 프로젝트로 설정되어 있습니다.</td>
     ...
     <tr> 
      ...
      <td>이메일 템플릿, 용도 및 의도한 대상자에 대한 자세한 내용을 추가합니다.</td> 
     </tr>
   ...
   <tr> 
     ...
     <tr> 
      ...
      <td> <p>이메일 메시지 콘텐츠에 대한 텍스트를 추가합니다.</p> <p>이 문서의 <a href="#add-html-formatting-to-an-email-template" class="MCXref xref">전자 메일 템플릿에 HTML 서식 추가</a> 섹션에 설명된 대로 전자 메일 콘텐츠에 HTML 서식을 사용할 수 있습니다.</p> </td> 
     </tr> 
    </tbody> 
   </table>
...
### Workfront 개체에 대한 링크 {#link-to-workfront-objects}

`$$` 와일드카드를 사용하여 전자 메일 생성기에 특정 개체와 연결된 데이터베이스에서 값을 검색하도록 지시하여 Workfront 필드에 대한 링크를 포함할 수 있습니다.

예를 들어 작업이 시작하려고 하는 작업의 피할당자에게 알리는 알림용 이메일 본문은 다음 구조를 따를 수 있습니다.
...
* 보고서의 텍스트 모드 보기 내에서 찾은 `valuefield` 값을 사용합니다. 텍스트 모드 값에 대한 자세한 내용은 [텍스트 모드 개요](../../../reports-and-dashboards/reports/text-mode/understand-text-mode.md)를 참조하십시오.

`heading` 값은 전자 메일 본문에 표시하려는 개체의 이름일 수 있습니다.

### HTML을 사용하여 사용자 지정 필드에 연결 {#link-to-custom-fields-with-html}

`$$` 와일드카드를 사용하여 사용자 및 사용자 지정 필드에 대한 링크를 포함하여 전자 메일 생성기에 개체와 연결된 데이터베이스에서 값을 검색하도록 지시할 수 있습니다. 데이터베이스 속성 참조의 양쪽에 있어야 합니다.

예를 들어 다음 텍스트를 HTML으로 추가하면 지정된 사용자의 이름이 작업과 연결된 미리 알림 알림에 추가됩니다.
...
* [지연 프로젝트 알림(예)](#late-project-reminder-notification-example)
...
![Project에서 전자 메일이 늦게 됨](assets/project-became-late-email.png)

#### 작업 또는 문제 시작 알림 {#task-or-issue-about-to-start-reminder-example}

예정된 작업 또는 문제에 대한 미리 알림 알림을 만들 수도 있습니다.

다음 코드는 작업 및 문제 알림 알림에 사용할 전자 메일 템플릿에 포함될 수 있으며, 이 알림은 작업 또는 문제의 계획된 시작 날짜 이전에 며칠씩 전송됩니다.

`<your domain>`을(를) 중괄호 없이 회사의 Workfront 도메인으로 바꾸십시오.

문제 전자 메일에 사용하려면 작업 항목에 대한 링크의 `/task/view.` 값을 `/issue/view`(으)로 변경하십시오.
