...
Adobe Workfront 관리자는 모든 작업, 문제 또는 기타 할당된 작업 항목이 만기이거나 늦거나 계획된 완료 일자 근처에 있을 때 이메일 알림을 트리거하기 위한 자동 미리 알림을 설정할 수 있습니다.

이러한 설정을 구성한 후에는 사용자가 자동 미리 알림을 비활성화할 수 없습니다. 내 설정 영역에서 사용자의 알림 설정에 관계없이 자동 미리 알림이 전송됩니다.

지연 알림의 경우 작업 또는 문제가 완료될 때까지 이메일이 매일 밤 전송됩니다. 즉, 작업 또는 문제가 완료되지 않은 동안 사용자는 매일 알림을 받게 됩니다.
...
자동 미리 알림은 Workfront 작업 및 문제에 대한 설정 수준 미리 알림 기능이며 오브젝트 수준 미리 알림 기능과 별개입니다. 자동 미리 알림과 미리 알림의 차이에 대한 자세한 내용은 [자동 미리 알림과 미리 알림](/help/quicksilver/administration-and-setup/tips-tricks-and-troubleshooting/auto-reminders-vs-reminder-notifications.md)을 참조하세요.

증명 및 증명 결정 또한 자동 미리 알림으로 처리되지 않으며 별도의 미리 알림 프로세스를 따르십시오. 증명 및 증명 결정 미리 알림에 대한 자세한 내용은 [증명 알림 및 미리 알림](/help/quicksilver/workfront-proof/wp-emailsntfctns/wp-emails-and-notifications.md)의 문서를 참조하십시오.

## 자동 미리 알림 사용 시 고려 사항

자동 미리 알림을 사용할 때는 다음 사항을 고려하십시오.

* 지연 이메일은 항목당 별도의 이메일이 아니라 수신자 당 매일 하나의 다이제스트 이메일로 전송됩니다.
* 자동 미리 알림을 활성화하면 항목의 지연 기간에 관계없이 기한이 이미 지난 문제나 작업이 다음의 기한 경과 다이제스트 이메일에 표시될 수 있습니다.
* 미리 알림은 현재/활성 상태인 프로젝트에만 적용됩니다.
* 자동 미리 알림 설정의 &quot;일&quot;은 경과일 또는 달력 시간이 아닌 일정 인식 업무일을 나타냅니다.
