---
title: 미리 알림 설정
description: 미리 알림 은 지정된 기준에 따라 사용자에게 전송되는 이메일을 생성합니다. 미리 알림 은 사용자에게 작업, 문제, 프로젝트 또는 작업표에 대해 수행해야 하는 작업을 알려 줍니다.
...
미리 알림 은 지정된 기준에 따라 사용자에게 전송되는 이메일을 생성합니다. 미리 알림 은 사용자에게 작업, 문제, 프로젝트 또는 작업표에 대해 수행해야 하는 작업을 알려 줍니다.

미리 알림 알림을 생성한 후 사용자는 이를 프로젝트, 작업, 문제 및 작업표와 같은 작업 항목에 수동으로 연결할 수 있습니다. 자세한 내용은 [개체에 알림 첨부](/help/quicksilver/workfront-basics/using-notifications/attach-reminder-notification-object.md)를 참조하십시오.
...
미리 알림 이메일을 사용자 정의하려면 이메일 템플릿을 만들어 미리 알림 알림에 첨부해야 합니다.

전자 메일 템플릿을 만드는 방법에 대한 자세한 내용은 [전자 메일 템플릿 구성](../../../administration-and-setup/manage-workfront/emails/configure-email-templates.md)을 참조하세요.
...
1. 드롭다운 목록에서 미리 알림 알림과 연결할 개체 유형을 클릭합니다.

   예를 들어 작업표에 미리 알림을 첨부하려면 **작업표**&#x200B;를 클릭하세요.

1. 표시되는 **새 알림** 상자에서 다음 정보를 지정하십시오.
   ...
   <p>미리 알림이 타임시트를 대상으로 하는 경우 사용 가능한 옵션은 종료 날짜, 시작 날짜 또는 마지막 업데이트 날짜와 관련이 있습니다. 작업표에 대한 미리 알림 메시지는 작업표 종료, 시작 및 마지막 업데이트 날짜의 시간 기록을 고려합니다. 작업표는 시작 날짜(오전 12:00)의 자정에 시작하고 종료 날짜(오후 11:59)의 자정 바로 전에 끝납니다.</p>

   <p><b>메모</b></p>
      <p>타임시트 미리 알림은 24시간마다 한 번만 배포됩니다.</p> <p>24시간 내에 여러 미리 알림을 설정하면 Workfront에서는 해당 알림에 포함된 모든 미리 알림과 함께 하나의 알림 이메일을 보냅니다.</p>
      <p>예를 들어 기한이 되기 10시간 전, 2시간 전, 1시간 전을 트리거하도록 세 개의 미리 알림을 구성하는 경우, 세 개의 미리 알림이 같은 날 동안 발생할 경우 모두 동일한 알림에 결합됩니다.</p> <p>단, 기한 전에 26시간 동안 미리 알림 알림을 설정하고 1시간 동안 미리 알림 알림을 설정하는 경우 사용자는 두 개의 알림을 별도로 수신하게 됩니다. </p>
     ...
     <tr> 
      ...
      <td> <p>미리 알림을 예약하려면 조건을 선택하십시오. 기준 선택을 충족하지 않으면 미리 알림이 예약되지 않습니다.</p> <p>4단계에서 선택한 객체 유형에 따라 다음 기준 옵션을 사용할 수 있습니다.</p> 
       <ul> 
        ...
        <li><strong>전자 메일 템플릿:</strong> 드롭다운에서 미리 알림에 첨부할 전자 메일 템플릿을 선택합니다.<br>전자 메일 템플릿을 만드는 방법에 대한 자세한 내용은 <a href="../../../administration-and-setup/manage-workfront/emails/configure-email-templates.md" class="MCXref xref">전자 메일 템플릿 구성</a>을 참조하세요.</li> 
       </ul> </td> 
     </tr> 
     <tr> 
      ...
      <li>할당 대상</li>
      <li>작성자</li>
      <li>프로젝트 팀(프로젝트 팀의 모든 사용자가 알림 메시지를 수신함)</li>
      <li>종속 작업 담당자(종속 작업에 지정된 사용자가 미리 알림을 받습니다)</li>
      <li>프로젝트 소유자</li>
      <li>할당 대상(작업 또는 문제에 할당된 사용자는 알림 메시지 수신)</li>
      <li>타임시트 소유자</li>
      <li>타임시트 승인자</li>
...
예를 들어 미리 알림이 프로젝트의 계획된 완료 일자 1시간 후에 트리거되도록 설정된 경우 미리 알림이 설정된 시간과 지금 사이에 해당 시간이 경과해야 합니다. 알림 메시지가 활성화되기 전에 계획된 완료 날짜가 지났던 모든 프로젝트는 알림을 트리거하지 않습니다.

미리 알림 알림을 수동으로 트리거하려면 다음을 수행하십시오.

{{step-1-to-setup}}

1. Workfront 왼쪽 아래 모서리에서 **시스템** > **진단**&#x200B;을 클릭합니다.
