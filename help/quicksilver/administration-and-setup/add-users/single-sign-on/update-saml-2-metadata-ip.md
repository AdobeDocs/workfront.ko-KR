...
1. 왼쪽 패널에서 를 확장합니다. **ADFS 2.0,** 확장 **트러스트 관계.**

1. 다음을 클릭합니다. **신뢰 당사자 트러스트** 폴더를 삭제합니다.
1. Workfront에서 사용하도록 이전에 구성한 신뢰 당사자 트러스트를 선택한 다음 오른쪽 패널에서 을 클릭합니다.**페더레이션 메타데이터에서 업데이트**.
1. (조건부) 이 옵션이 흐리게 표시되면(신뢰 당사자 트러스트가 이전에 메타데이터 파일을 사용하여 구성되었음을 의미함) 다음 단계를 완료합니다.
   ...
   1. 클릭 **설정을 편집합니다.**
   1. 클릭 **구성 편집**&#x200B;을 선택한 다음 을 선택합니다. **SAML 2.0** 다음에서 **유형** 드롭다운 목록입니다.

   1. 다음을 복사합니다. **메타데이터 URL**: 다음과 유사해야 합니다.

      `https://<yourdomain>.my.workfront.com/sso/downloadSAML2MetaData`

   1. ADFS 서버에서 이전에 구성한 신뢰 당사자 트러스트를 마우스 오른쪽 단추로 클릭한 다음 **속성.**
   1. 다음을 클릭합니다. **모니터링** 탭을 클릭한 다음 Workfront에서 복사한 URL을 **신뢰 당사자의 페더레이션 메타데이터 URL** 필드.

   1. 다음 옵션을 선택합니다. **신뢰 당사자 모니터링** 및 **신뢰 당사자 자동 업데이트**.

   1. 클릭 **좋아.**
   1. Workfront에서 사용하도록 이전에 구성한 신뢰 당사자 트러스트를 선택한 다음 오른쪽 패널에서 을 클릭합니다. **페더레이션 메타데이터에서 업데이트합니다.**

1. 클릭 **확인** ADFS 2.0에서 지원하지 않는 페더레이션 메타데이터의 일부 콘텐츠에 대한 메시지를 무시합니다.
...
1. 제대로 작동하는지 확인하려면 **이벤트 뷰어** adfs2.0 로그에서 다음 정보를 찾으려면:
...
1. 왼쪽 패널에서 를 확장합니다. **ADFS 2.0**, 다음 확장 **트러스트 관계**.
...
1. Workfront에서 사용하도록 이전에 구성한 신뢰 당사자 트러스트를 선택한 다음 오른쪽 패널에서 을 클릭합니다. **페더레이션 메타데이터에서 업데이트**.
   ...
   1. Workfront의 설정 영역에서 **메타데이터 URL** Workfront Single Sign-On 설치 화면에서 다음을 수행합니다.

      다음에 대한 정보에 액세스하려면 **메타데이터 URL**:

      1. 클릭 **설정** 전역 탐색 막대에 있는 Adobe Workfront의 오른쪽 상단 모서리 근처.
      1. 클릭 > **시스템** > **SSO(단일 인증)**.
      1. 클릭 **설정을 편집합니다.**
      1. 클릭 **구성 편집**&#x200B;을 선택한 다음 을 선택합니다. **SAML 2.0** 다음에서 **유형** 드롭다운 목록입니다.
      1. 다음을 복사합니다. **메타데이터 URL**: 다음과 유사해야 합니다.
...
1. 클릭 **확인** ADFS 2.0에서 지원하지 않는 페더레이션 메타데이터의 일부 콘텐츠에 대한 메시지를 무시합니다.
1. 클릭 **업데이트** 페더레이션 메타데이터 업데이트를 완료합니다.

Workfront 로그인 자격 증명을 사용하여 기본 로그인 화면을 통해 Workfront에 액세스할 수 있는 사용자(의 각 사용자 프로필 페이지에서 구성할 수 있음) **액세스** 섹션)에서 다음 URL로 이동하여 Workfront 사용자 이름 및 암호를 사용하여 로그인할 수 있습니다. `https://<yourdomain>.my.workfront.com/Workfront/login.cmd`.
