  ...
  <tr> 
   ...
   <td> <p>다음 중 하나가 있어야 합니다.</p> 
    <ul> 
     ...
     <li> <p><b>액세스 편집 액세스로</b> <b>구성된 액세스 수준의 사용자</b> 설정, <b>설정 미세 조정</b> <img src="assets/gear-icon-in-access-levels.png">에서 <b>만들기</b> 및 두 개의 <b>사용자 관리</b> 옵션 중 하나 이상이 활성화되어 있습니다. </p> <p>이 두 옵션 중 사용자 관리자(그룹 사용자)</b>가 활성화된 경우 <b>사용자가 구성원인 그룹의 그룹 관리자여야 합니다.</p> </li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table>
...
1. 둘 이상의 사용자를 선택한 다음 편집 아이콘 ![편집 아이콘을](assets/edit-icon.png) 클릭합니다.

1. **사용자 편집** 상자가 나타나면 다음 중 원하는 옵션을 변경합니다.
     ...
     <tr> 
      ...
      <td> 
       <ul> 
        ...
        <p>사용자에게 액세스 수준을 할당할 때 자신의 액세스 수준과 같거나 낮은 수준을 할당할 수 있습니다. 예를 들어 액세스 수준이 Planner인 경우 관리자 액세스 수준을 할당할 수 없습니다. </p>
        ...
        <p><b>참고</b>: 이 필드에서 사용할 수 있는 레이아웃 템플릿 목록은 액세스 권한에 따라 다릅니다.
          <ul>
           ...
           <p>그룹 수준 레이아웃 템플릿에 대한 자세한 내용은 레이아웃 템플릿</a> 만들기 및 관리를 참조하십시오<a href="../../../administration-and-setup/customize-workfront/use-layout-templates/create-and-manage-layout-templates.md" class="MCXref xref">.</p>
           </li>
          </ul></p> 
          </li> 
       </ul> </td> 
     ...
     <tr> 
      ...
      <td> 
       <ul> 
        ...
        <li><b>다른 팀</b>: 사용자는 여러 팀에 속할 수 있습니다. </li> 
        <li> <p><b>홈 그룹:</b> 적절한 그룹 그룹을 선택하여 사용자를 홈 그룹으로 할당합니다. 이렇게 하면 사용자 간에 그룹 간에 공유되는 개체에 액세스할 수 있습니다.</p> <p><b>참고</b>: 이것은 필수 필드입니다. 사용자는 홈 그룹에 연결되지 않을 수 없습니다.</p> <p>다음 상황에서만 사용자에게 그룹을 할당할 수 있습니다.</p> 
         <ul> 
   ...
   <li>
       <b>작업 시간</b>: 사용자가 오버헤드를 포함하지 않고 실제 작업에 사용할 수 있는 FTE(Full Time Equivalent) 시간의 백분율을 나타냅니다. 작업 시간은 최대 1의 10진수여야 하며 0일 수 없습니다. 예를 들어 실제 작업의 20% 가용성은 0.2입니다.
   ...
   Workfront은 설정 영역의 리소스 관리 기본 설정에 따라 사용자의 가용성을 계산합니다. 자세한 내용은 Configure Resource Management preferences(리소스 관리 기본 설정</a> 구성)를 참조하십시오<a href="../../set-up-workfront/configure-system-defaults/configure-resource-mgmt-preferences.md">.

   <b>팁</b>

   작업 시간 값을 1로 설정하여 사용자 전체를 풀타임으로 사용하는 프로젝트 관련 작업에 사용할 수 있음을 나타냅니다.
   </li>

   <li><b>비활성화 예약</b>: 일정 시간 후에 사용자가 비활성화되도록 예약하려면 이 확인란을 선택합니다.</li> 
       <li><b>예약된 비활성화 날짜</b>: 사용자가 비활성화되는 날짜입니다. 사용자 비활성화 예약에 대한 자세한 내용은 사용자</a> 비활성화 또는 다시 활성화의 <a href="../../../administration-and-setup/add-users/create-and-manage-users/deactivate-a-user.md" class="MCXref xref">사용자 비활성화</a> 예약 섹션을 <a href="../../../administration-and-setup/add-users/create-and-manage-users/deactivate-a-user.md#scheduling-users-for-deactivation" class="MCXref xref">참조하십시오.</li> 
       <li> <p><b>기본 역할</b>: 사용자가 Workfront에서 보유하는 기본 작업 역할입니다. 사용자가 할당된 모든 작업 및 문제도 기본적으로 이 작업 역할에 할당됩니다. 작업 역할은 리소스 관리에서 필수적입니다. 작업 역할에 대한 자세한 내용은 <a href="../../../administration-and-setup/set-up-workfront/organizational-setup/create-manage-job-roles.md" class="MCXref xref">작업 역할 만들기 및 관리</a>를 참조하세요.</p> <p>관리 사용자 액세스 권한이 있는 플랜 라이선스가 있거나 Workfront 관리자인 경우에만 이 필드를 업데이트할 수 있습니다. 관리 사용자 액세스 권한이 있는 사용자를 설정하는 방법에 대한 자세한 내용은 사용자에게</a> 액세스 권한 부여를 참조하세요<a href="../../../administration-and-setup/add-users/configure-and-grant-access/grant-access-other-users.md" class="MCXref xref">.</p> </li> 
       <li>(조건부) 기본 역할을</b><b> 선택한 <b>경우 FTE 가용성</b> 백분율 필드가 표시됩니다. 이 작업 역할에 할당되는 사용자 일정의 시간 백분율을 지정합니다. 기본 역할에 대한 FTE 가용성 비율의 기본값은 100%입니다.</li> 
       ...
       <li> <p>모든 역할에 대한 모든 FTE 가용성 백분율의 합은 100%여야 합니다. FTE 가용성의 각 비율은 리소스 플래너의 사용자당 각 역할에 대한 가용 시간을 계산합니다. 사용자당 각 역할에 대한 사용 가능한 시간은 사용자의 사용 가능한 시간에 따라 다릅니다.</p> <p>사용자 사용 가능 시간은 Workfront 관리자가 리소스 관리 환경 설정(Resource Management Environment Preferences)에서 FTE를 계산하기 위해 선택한 방법에 따라 Workfront에서 계산합니다.</p> <p>사용자 가용성의 계산에 대한 자세한 내용은 <a href="../../../resource-mgmt/resource-planning/calculate-hours-fte-for-users-roles-resource-planner.md" class="MCXref xref">리소스 플래너에서 사용자 및 역할에 대한 시간 및 FTE 계산 개요</a>를 참조하십시오.</p> <p>자원 관리 환경 설정 구성에 대한 자세한 내용은 자원 관리 환경 설정</a> 구성을 참조하십시오<a href="../../../administration-and-setup/set-up-workfront/configure-system-defaults/configure-resource-mgmt-preferences.md" class="MCXref xref">.</p> </li> 
       </ul> </p> </li> 
       <li> <p><b>일정</b>: 일정을 사용자와 연결합니다. 사용자의 일정은 사용자에게 할당된 작업의 타임라인 일정을 계산합니다.</p> <p>Workfront 관리자 또는 그룹 관리자는 일정을 만들어야 사용자와 연결할 수 있습니다.</p> <p>시스템 수준 또는 그룹 일정을 선택하여 선택한 사용자에게 할당합니다.</p> <p>시스템 수준 및 그룹 일정에 대한 자세한 내용은 <a href="../../../administration-and-setup/set-up-workfront/configure-timesheets-schedules/create-schedules.md" class="MCXref xref">일정 만들기</a>를 참조하십시오.</p> <p><b>중요</b>: Workfront에서는 리소스 가용성 계산 설정이 사용자의 일정으로 설정된 경우에만 사용자의 일정을 사용합니다. 리소스 관리에 사용되는 일정에 영향을 주는 리소스 가용성 계산 설정에 대한 자세한 내용은 <a href="../../../administration-and-setup/set-up-workfront/configure-system-defaults/configure-resource-mgmt-preferences.md" class="MCXref xref">리소스 관리 환경 설정 구성</a>을 참조하십시오.</p> </li> 
       ...
       <li> <p><b>사용 가능한 시간 유형</b>: 사용자 사용 가능한 시간 유형을 선택합니다. 이러한 시간 유형은 사용자가 시간을 기록할 수 있는 Workfront의 모든 곳에서 볼 수 있습니다. 사용자 프로젝트 수준과 사용자 수준에서 사용할 수 있는 시간 유형만 볼 수 있습니다.</p> 
   ...
   Workfront에서 일정을 만드는 방법에 대한 자세한 내용은 <a href="../../set-up-workfront/configure-timesheets-schedules/create-schedules.md">일정 만들기</a>를 참조하십시오.
       ...
       <li> <p><b>리소스 풀</b>: 사용자를 리소스 풀과 연결합니다.</p> <p><b>참고</b>: 선택한 모든 사용자에게 공통된 리소스 풀만 이 필드에 나타납니다. 선택한 사용자에게 공유 리소스 풀이 없는 경우 이 필드는 비어 있습니다. 이 필드가 비어 있으면 여기에서 지정한 자원 풀이 개별 자원 그룹을 덮어씁니다.</p> 
       ...
       <li><b>사용자 지정 Forms</b>: 기존 사용자 사용자 지정 양식을 사용자와 연결합니다. 사용자 지정 양식을 만들어야 사용자 양식과 연결할 수 있습니다. 활성 사용자 지정 양식만 목록에 표시됩니다. 사용자 지정 양식 만들기에 대한 자세한 내용은 사용자 지정 양식</a> 만들기 를 참조하십시오<a href="/help/quicksilver/administration-and-setup/customize-workfront/create-manage-custom-forms/form-designer/design-a-form/design-a-form.md">.</li> 
       <li><b>댓글</b>: 제공된 필드에 댓글을 입력합니다. 선택한 모든 사용자는 인앱 알림와 댓글이 포함된 이메일 알림 받게 됩니다. 사용자 프로필의 업데이트 탭 에 댓글이 표시됩니다.</li> 
       </ul> </td> 
     </tr> 
    </tbody> 
   </table>

1. (선택 사항) **Custom Forms** 섹션에서 Recalculate Custom Expressions **옵션을 선택하여**&#x200B;선택한 사용자에게 첨부된 사용자 지정 양식의 모든 계산된 사용자 지정 필드가 최신 상태인지 확인합니다.
