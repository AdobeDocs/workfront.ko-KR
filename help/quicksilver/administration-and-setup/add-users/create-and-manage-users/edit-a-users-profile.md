...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td><p>사용자 프로필을 편집하려면: 모든 Workfront 또는 워크플로 패키지</p>
       <p>비율 속성 및 원가율 재정의를 설정하려면: Workflow Ultimate</p></td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>다음 중 하나가 있어야 합니다.</p> 
    ...
    <p>사용자에 대한 청구 및 비용 요금을 편집하려면 청구 요금 편집 및 비용 요금 편집 세부 조정 옵션도 활성화되어 있어야 합니다.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
* **전자 메일 주소**: 사용자의 전자 메일 주소도 Workfront의 사용자 이름입니다. 이 필드는 대/소문자를 구분하므로 고유해야 합니다. 사용자가 10분 내에 고유하지 않은 이메일 주소를 3번 추가하려고 하면 reCAPTCHA 응답이 나타납니다.
  ...
  전자 메일 도메인을 사용하고 목록에 없는 전자 메일 도메인을 입력하면 사용자에게 전자 메일 알림이 전송되지 않습니다. 허용 목록에 추가하다에 대한 자세한 내용은 [전자 메일 구성](/help/quicksilver/administration-and-setup/get-started-wf-administration/configure-your-email-allowlist.md)을 참조하십시오.

  조직이 Adobe Admin Console으로 마이그레이션되었으므로 Workfront에서 사용자의 이메일 주소를 편집할 수 없습니다. 사용자의 이메일 주소는 Adobe Admin Console에서 설정됩니다.
...
* **연락처 정보**: 사용자의 전화 번호(**전화 번호**, **내선 번호**, **휴대폰 번호** 필드) 및 주소(**주소**, **도시**, **상태**, **우편 번호**, **국가** 필드).

  연락처 정보 섹션의 **국가** 필드는 국가 코드 값(예: US, GB, IN)만 허용합니다.
...
>[!NOTE]
>
>&quot;문서를 업로드할 때 증명을 자동으로 생성&quot; 옵션이 제거되었습니다.
...
* **비활성화 날짜**: 사용자가 비활성화되는 날짜와 시간입니다. 비활성화에 대한 사용자 예약에 대한 자세한 내용은 [사용자 비활성화 또는 다시 활성화](/help/quicksilver/administration-and-setup/add-users/create-and-manage-users/deactivate-a-user.md)에서 [비활성화에 대한 사용자 예약](/help/quicksilver/administration-and-setup/add-users/create-and-manage-users/deactivate-a-user.md#schedule-users-for-deactivation)을 참조하십시오.
...
* **비용 비율 재정의가 허용됨**: 프로젝트에서 이 사용자에 대한 비용 비율 재정의를 허용하려면 이 옵션을 설정하십시오. 자세한 내용은 [프로젝트 수준에서 사용자 비용 비율 재정의](/help/quicksilver/manage-work/projects/project-finances/override-user-cost-rates.md)를 참조하십시오.
...
비율 속성은 작업 역할 및 사용자와 같이 비율이 존재하는 Workfront 영역에서 지원됩니다. 사용자 프로필에 속성이 적용되면 해당 할당이 자동으로 올바른 비율로 확인됩니다.

1. **새 집합 추가**&#x200B;를 클릭합니다.
1. 속성 값을 선택합니다.

   Workfront 관리자가 사용 가능한 속성을 설정합니다. 여기에는 에이전시, 위치, 비용 부서, 고객 또는 조직에 필요한 기타 속성이 포함될 수 있습니다.

1. (선택 사항) 유효 일자가 있는 다른 속성 세트를 추가하려면

   1. **새 집합 추가**&#x200B;를 클릭합니다.
   1. 현재 특성 집합에 대한 **종료 날짜**&#x200B;를 선택하십시오.

      다음 집합의 **시작 날짜**&#x200B;가 자동으로 추가됩니다.

   1. 다음 속성 세트에 대한 속성 값을 추가합니다. 이 값은 시작 날짜에 적용됩니다.
