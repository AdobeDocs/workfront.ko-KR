...
1. **추가 제한 설정**&#x200B;을 클릭한 다음 액세스 수준에 대해 다음 제한 중 하나를 설정합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr>
      <td role="rowheader">사용자가 브랜드에 액세스하도록 허용</td> 
...
1. <span class="preview">(선택 사항) 이 액세스 수준을 가진 사용자가 특정 필드를 보지 못하도록 제한하려면 제한된 필드 추가 섹션에서 필드를 추가하십시오.</span>

   <span class="preview">이 섹션에서 네이티브 필드와 사용자 지정 필드를 모두 검색할 수 있습니다. 제한된 필드는 20개로 제한됩니다.</span>

   <span class="preview">액세스 수준이 사용자에게 기본 액세스 수준으로 할당되거나 비즈니스 프로필을 통해 할당되면 필드가 제한됩니다. 비즈니스 프로필에 대한 자세한 내용은 [비즈니스 프로필 개요](/help/quicksilver/administration-and-setup/add-users/create-and-manage-users/business-profiles.md).</span>를 참조하십시오.

   <span class="preview">제한된 필드는 시스템의 사용자에게 계속 표시되지만 공백으로 나타나거나 실제 데이터 대신 해당 없음으로 표시됩니다.</span>
