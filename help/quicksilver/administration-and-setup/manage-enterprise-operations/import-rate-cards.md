...
* 사용자 지정 속성(에이전시, 비용 센터 등) 다양할 수 있습니다. 정확한 요구 사항은 시스템 관리자에게 문의하십시오.
...
1. 각 행의 비율에 대한 정보를 입력합니다.
   ...
   * **특성**(선택 사항/사용자 지정): 마지막 열(에이전시, 위치, 비용 센터 등) 고객 구성에 따라 다른 비율 속성입니다. 사용자 정의 가능한 필드이며 고객 환경에 따라 다를 수 있습니다.

     예: Agency = &quot;1: Agency,&quot; Location = &quot;Chicago,&quot; Cost Center = &quot;22: Cost Center&quot;

### RSALS(Rate Card Alias) 탭 입력

이 탭의 모든 별칭을 만들고 나열합니다. 각 행은 하나의 별칭을 나타냅니다.

비율 카드가 프로젝트에 첨부된 경우 내부 작업 역할 이름 대신 자리 표시자 할당, 경비 및 보고서와 같은 정보에 별칭이 표시됩니다. 단일 속도 카드 내에서 각 작업 역할 및 속성 조합에 대해 하나의 별칭만 존재할 수 있습니다.

별칭이 시스템에 추가되지만 이 탭의 정보를 기반으로 작업 역할에 연결되지 않습니다.

속도 카드 가져오기 템플릿 파일의 ![RSALS 탭](assets/rsals-tab-rate-card-import.png)

1. 각 행에 별칭의 이름을 입력합니다.

   행당 하나의 별칭 이름(작업 역할 별칭, 비인적 자원 범주 별칭 또는 경비 유형 별칭)만 입력합니다.

### RCRMET_RTCRD_RSALS(비율 카드 메타데이터) 탭을 채웁니다.

이 탭에서는 특정 요금 카드에 대한 리소스와 별칭 간의 연결을 정의할 수 있습니다.

속도 카드 가져오기 템플릿 파일의 ![RCRMET_RTCRD_RSALS 탭](assets/rcrmet-tab-rate-card-import.png)

1. 각 행에 대한 정보를 입력합니다.

   * **속도 카드**(필수): 리소스 및 별칭이 속한 속도 카드의 이름 또는 시퀀스 번호입니다. 요금 카드는 RTCRD 탭에 나열되어 있어야 합니다.

     순번의 경우: 비율 카드가 RTCRD 탭(2행)에 처음으로 나열한 카드인 경우 1을 입력합니다. 두 번째 문자인 경우 2를 입력합니다.

   * **작업 역할**(경비 유형 및 비인적 자원 범주가 사용되지 않는 경우 필수): 별칭이 연결된 작업 역할입니다. 작업 역할 이름 또는 작업 역할 ID일 수 있습니다. 가져오기를 통해 둘 다 인식됩니다.

     예: Designer 또는 _68c0234e00000541dd8c0757723daa68_

   * **경비 유형**(작업 역할 및 비인적 자원 범주가 사용되지 않는 경우 필수): 별칭이 연결된 경비 유형입니다. 경비 유형 이름 또는 경비 유형 ID일 수 있습니다. 가져오기를 통해 둘 다 인식됩니다.

     예: Travel 또는 _68c0234e00000541dd8c0757723daa68_

   * **비인적 자원 범주**(작업 역할 및 경비 유형이 사용되지 않는 경우 필수): 별칭이 연결된 비인적 자원 범주입니다. 카테고리 이름 또는 카테고리 ID일 수 있습니다. 가져오기를 통해 둘 다 인식됩니다.

     예: 카메라 또는 _68c0234e00000541dd8c0757723daa68_

     >[!IMPORTANT]
     >
     >**작업 역할**, **경비 유형** 및 **비인적 리소스 범주** 열 세 개를 모두 입력할 수 없습니다. 하나는 필수입니다.

   * **리소스 별칭**: RSALS 탭에 입력한 별칭입니다.
...
1. 파일을 대화 상자로 끌어다 놓거나 **Excel 파일 선택**&#x200B;을 클릭하여 컴퓨터에서 파일을 찾습니다.
1. **가져오기 시작**&#x200B;을 클릭합니다.
...
동일한 Excel 템플릿을 사용하여 기존 요금 카드의 요금을 업데이트하고 이러한 변경 사항을 Workfront에 업로드할 수 있습니다.

RATE_RTCRD(Rates Setup) 탭만 기존 비율을 업데이트하는 데 필요합니다.

>[!NOTE]
>
>기존 요금 카드의 요금을 업로드하면 요금 카드의 현재 작업 역할과 요금을 모두 덮어씁니다.
>
>예를 들어, 기존 비율 카드에 비율이 있는 작업 역할이 5개 있고 Excel 파일에 작업 역할이 1개인 경우 업로드한 후 비율 카드에 작업 역할이 1개 있습니다. 다른 5개의 작업 역할과 해당 비율을 비율 카드에 유지하려면 Excel 파일에 포함해야 합니다.
...
1. 왼쪽 패널에서 [!UICONTROL **카드 평가**]&#x200B;를 클릭합니다.
1. **새 속도 카드**&#x200B;를 클릭한 다음 **속도 카드 업데이트 가져오기**&#x200B;를 클릭합니다.
1. 파일을 대화 상자로 끌어다 놓거나 **Excel 파일 선택**&#x200B;을 클릭하여 컴퓨터에서 파일을 찾습니다.
1. **가져오기 시작**&#x200B;을 클릭합니다.
...
1. 파일에 문제가 있는 경우 오류 메시지가 나타납니다. **문제 보기**&#x200B;를 클릭하여 별도의 화면에서 문제를 봅니다.
   ...
   Workfront에 요금 카드 업데이트가 존재하기 전에 Excel 파일의 문제를 수정하고 다시 가져와야 합니다.
