...
* 다음 알림은 사용자 지정 새로 고침 샌드박스 환경에서 사용할 수 없습니다.
...
비즈니스 규칙은 Workfront 미리보기 및 샌드박스 환경 새로 고침에서 의도적으로 제외됩니다. 대부분의 데이터와 달리 비즈니스 규칙은 사용자와 유사하게 환경별 구성으로 처리됩니다. 각 환경은 목적에 맞는 고유한 규칙 세트(예: 테스트, 개발 또는 프로덕션)를 유지할 수 있습니다.

새로 고침에 비즈니스 규칙을 포함하면 이러한 구성을 덮어쓰게 되어 관리자가 비프로덕션 환경에서 의도적으로 설정한 규칙이 중단될 수 있습니다.

환경 간에 비즈니스 규칙을 이동해야 하는 경우 시간 및 범위를 제어하면서 구성 객체를 선택적으로 전송할 수 있는 환경 프로모션 을 사용합니다. 자세한 내용은 [Workfront 환경 간 개체 이동 개요](/help/quicksilver/administration-and-setup/set-up-workfront/workfront-testing-environments/environment-promotion-in-wf.md)를 참조하십시오.
