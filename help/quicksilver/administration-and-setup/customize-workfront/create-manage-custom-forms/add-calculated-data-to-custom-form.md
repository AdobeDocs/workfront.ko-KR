...
1. 다음에서 **필드 추가** 탭을 클릭하고 **계산됨**.
   ...
   오른쪽의 표시 영역에 필드가 표시됩니다 *12345*. 이는 사용자 정의 양식을 만들거나 편집하는 동안 필드가 계산된 사용자 정의 필드임을 알려주는 표시기에 불과합니다. 양식이 오브젝트에 첨부되고 사용자가 양식을 작성하는 경우 필드에 계산 결과가 표시되고 *12345* 표시기.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td>필드에 대한 레이블을 입력합니다. 이는 사용자가 사용자 정의 양식을 사용할 때 표시되는 것입니다. 필드 <b>이름</b>보고서에서 Workfront에 의해 참조되며 자동으로 채워집니다.</td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      ...
      <td> <p>필드의 결과를 저장하고 표시할 형식입니다.</p> <p>필드가 수학 계산에 사용될 경우 항상 <strong>숫자</strong> 또는 <strong>통화</strong> 포맷. 숫자 또는 통화를 선택하면 0으로 시작하는 숫자가 자동으로 잘립니다.</p> 
...
1. 계속 진행 [계산된 사용자 정의 필드에 대한 계산을 작성합니다](#build-the-calculation-for-your-calculated-custom-field) 이 문서에서.
...
1. 클릭 **최대화** 을(를) 열려면 **계산 편집기** 계산을 빌드합니다.
   ...
   두 가지(서식 유형이 있는 텍스트 필드 및 설명 텍스트)를 제외하고 계산에서 모든 유형의 사용자 정의 필드를 참조할 수 있습니다. 사용자 지정 필드 유형에 대한 자세한 내용은 [사용자 정의 양식에 사용자 정의 필드 추가](../../../administration-and-setup/customize-workfront/create-manage-custom-forms/add-a-custom-field-to-a-custom-form.md)

1. 큰 텍스트 상자를 클릭한 다음 **표현식** 및 **필드** 계산에 추가할 수 있습니다.
...
이렇게 하면 사용자 정의 양식의 개체(작업)에서 작업(프로젝트)과 관련된 다음 개체에 액세스할 수 있습니다. 여기에서 프로젝트(포트폴리오)에 대한 다음 관련 오브젝트에 액세스한 다음 포트폴리오(소유자)에 대한 다음 관련 오브젝트에 액세스할 수 있습니다.

### 사용자 정의 필드를 참조하기 위한 이름 구문

계산된 사용자 정의 필드에서 다른 사용자 정의 필드를 참조할 때 Workfront 사용자 인터페이스에 표시되는 대로 표시되는 필드의 이름을 입력해야 합니다.
...
>[!NOTE]
>
>자동 완성 필드의 구문은 를 추가해야 하므로 다른 유형의 필드에 대한 구문과 약간 다릅니다. `:name` 끝에 있습니다.
>
...
변수에 대한 자세한 내용은 다음을 참조하십시오 `$$OBJCODE,` 참조 [와일드카드 필터 변수](../../../reports-and-dashboards/reports/reporting-elements/understand-wildcard-filter-variables.md).
