   ...
   1. **최대화**&#x200B;를 클릭하여 계산 편집기를 열고 계산을 빌드합니다.
      ...
      * 계산에서 사용자 지정 필드를 참조하도록 하려면 필드 이름을 중괄호로 묶고 중괄호 안에 `DE:`을(를) 추가해야 합니다.
        ...
        **점이 있는 항목 구분**
        ...
        >[!NOTE]
        >
        >내부 조회 필드의 구문은 다른 필드 형식의 구문과 약간 다릅니다. 끝에 `:name`을(를) 추가해야 하기 때문입니다.
        >
        >예를 들어 &quot;경영 스폰서&quot;라는 사용자 정의 내부 조회 필드에서 선택한 옵션을 참조하려면 다음을 입력합니다.
        >
        >`{DE:Executive sponsor}.{name}`
        >
        >자동 완성 필드를 사용하는 동일한 예제의 경우 다음을 입력합니다.
        >
        >`{DE:Executive sponsor:name}`
