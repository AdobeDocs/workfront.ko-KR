...
1. 검색 상자를 사용하거나 **계산됨** 섹션을 사용하여 필요한 계산된 필드를 찾은 다음 사용자 정의 양식에서 표시할 필드를 드래그합니다.
...
1. 화면 왼쪽에서 **계산됨** 캔버스의 섹션으로 드래그합니다.
...
1. 다음에서 **계산** 상자, 계산 빌드를 시작합니다.
   1. 클릭 **최대화** 계산 편집기를 열고 계산을 빌드합니다.</p>
...
+++ **를 확장하여 계산된 사용자 정의 필드에 필요한 구문을 확인합니다.**
      ...
      * 계산에서 기본 제공 필드를 참조하도록 하려면 필드 이름을 중괄호로 묶어야 합니다.
         ...
         다음 위치로 이동 [Workfront API 탐색기](https://developer.adobe.com/workfront/api-explorer/) 계산에 사용할 수 있는 필드 이름을 식별합니다.
