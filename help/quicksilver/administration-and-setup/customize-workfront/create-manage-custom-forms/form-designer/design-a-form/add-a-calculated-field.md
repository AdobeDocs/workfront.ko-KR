...
description: 사용자 지정 양식이 개체에 첨부된 경우 기존 데이터를 사용하는 계산된 사용자 지정 필드를 추가하여 새 데이터를 생성할 수 있습니다.
author: Courtney
...
사용자 지정 양식이 개체에 첨부된 경우 기존 데이터를 사용하는 계산된 사용자 지정 필드를 추가하여 새 데이터를 생성할 수 있습니다.
...
>[!IMPORTANT]
>
>계산된 표현식의 변경으로 인해 개체의 필드 값이 오래된 상태가 될 수 있습니다. 이러한 필드에서 항상 최신 계산을 보려면 다음 중 하나를 수행하십시오.
>
...
1. 검색 상자를 사용하거나 **계산된 지표** 섹션에 필요한 계산된 필드를 찾은 다음 사용자 지정 양식에 표시할 필드를 드래그합니다.
...
1. 화면 왼쪽에서 **계산된 지표** 캔버스의 섹션으로 드래그합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <li> <p>도구 설명으로 사용자는 필드 위로 마우스를 가져가면 볼 수 있습니다. 도구 설명에 표시할 텍스트를 여기에 추가합니다.</p> <p>해당 사용자가 도구 설명에 있는 수식을 보지 못하게 하려면 해당 수식에 혼동을 줄 수 있습니다.</li> 
       </ul> </td> 
     </tr> 
     <tr> 
      <td role="rowheader">포맷</td> 
      <td> <p>필드 결과를 저장 및 표시할 형식입니다.</p> <p>필드를 수학 계산에 사용하는 경우 항상 <strong>숫자</strong> 또는 <strong>통화</strong> 형식 지정 번호 또는 통화를 선택하면 0으로 시작하는 숫자가 자동으로 잘립니다.</p> 
...
1. 에서 **계산** 상자에서 계산 작성을 시작합니다.
   1. 클릭 **최대화** 계산 편집기를 열고 계산을 작성합니다.</p>
계산은 일반적으로 표현식으로 시작되고, 사용자 지정 양식이 개체에 첨부된 경우 참조할 필드가 포함된 괄호가 옵니다.

      각 필드는 중괄호로 둘러싸야 합니다. 필드의 이름을 입력하기 시작하면 시스템이 제안을 하고 하나를 선택하여 계산에 삽입할 수 있습니다.

+++ **계산된 사용자 지정 필드에 필요한 구문을 보려면 를 확장하십시오**
      ...
      >[!NOTE]
      >
      >현재, 사용자 지정 양식이 첨부할 개체에 참조할 필드의 이름을 입력하기 시작할 때만 제안 사항을 제공합니다. 상위 개체의 필드는 권장되지 않습니다.
      ...
      * 계산 시 기본 제공 필드를 참조하려면 필드 이름을 중괄호로 묶어야 합니다.
         ...
         로 이동합니다 [Workfront API 탐색기](https://developer.adobe.com/workfront/api-explorer/) 를 사용하여 계산에 사용할 수 있는 필드 이름을 식별합니다.
      ...
      * 계산 시 사용자 지정 필드를 참조하려면 필드의 이름을 중괄호로 둘러싸며 앞에 `DE:` 괄호 안에 있어야 합니다.
         ...
         **마침표로 항목 구분**
         ...
         이에 따라 다음 내용이 결정됩니다. 사용자 지정 양식(작업)의 개체에서 작업과 관련된 다음 개체(프로젝트)에 액세스할 수 있습니다. 여기에서 프로젝트(포트폴리오)에 대한 다음 관련 개체에 액세스한 다음 포트폴리오 개체(소유자)에 대해 정의된 필드를 참조할 수 있습니다

         **사용자 지정 필드 참조용 이름 구문**

         계산된 사용자 지정 필드에서 다른 사용자 지정 필드를 참조하는 경우 Workfront 사용자 인터페이스에 표시되는 필드의 이름을 입력해야 합니다.
         ...
         **다중 개체 사용자 지정 양식의 계산된 사용자 지정 필드**

         다중 개체 사용자 지정 양식에서 선택한 개체 유형은 양식의 계산된 사용자 지정 필드에서 참조되는 하나 이상의 필드와 호환되어야 합니다. 개체와 호환되지 않는 필드는 양식에 해당 없음 을 표시합니다.

         계산된 필드에 모든 개체 유형에 대한 올바른 결과가 표시되는지 확인하려면 `$$OBJCODE` 각 객체 유형에 대한 계산을 정의하려면

         >[!INFO]
         >
         >**예:**
         >
         >프로젝트, 작업 및 문제와 함께 작동하도록 구성된 사용자 지정 양식에서는 다음 공식을 사용하여 개체 유형을 표시할 수 있습니다.
         >
         >`IF($$OBJCODE="PROJ","This is a project",IF($$OBJCODE="TASK","This is a task","This is an issue"))`
         >
         >프로젝트에서 필드에 &quot;This is a project&quot;가 표시되고 작업의 경우 &quot;This is a task&quot;가 표시되고 문제에 대해서는 &quot;This is a issue&quot;라고 표시됩니다.
         ...
         와 같은 변수에 대한 자세한 정보 `$$OBJCODE,` 참조 [와일드카드 필터 변수](/help/quicksilver/reports-and-dashboards/reports/reporting-elements/understand-wildcard-filter-variables.md).
...
+++ **을 확장하여 유용한 팁 보기**
      ...
      > 
      >* 계산의 표현식을 마우스로 가리키면 설명, 사용 방법을 보여주는 예제 및 문서에 있는 추가 정보에 대한 &quot;추가 정보&quot; 링크가 표시됩니다 [계산된 데이터 표현식](/help/quicksilver/reports-and-dashboards/reports/calc-cstm-data-reports/calculated-data-expressions.md).
   ...
   <col> 
    ...
    <tbody> 
     <tr> 
      ...
      <td>표시 논리 를 추가하여 양식을 채울 때 이전 다중 선택 필드(드롭다운, 확인란 또는 라디오 단추)에서 사용자가 선택한 하나 이상의 항목을 기준으로 계산된 필드가 표시되는지 여부를 결정할 수 있습니다. <!-- For more information, see <a href="Need to add link for new article when it's written" class="MCXref xref">Add display logic and skip logic to a custom form</a>.--> <p>이 기능은 하나 이상의 확인란, 라디오 단추 또는 드롭다운 필드가 양식의 계산된 사용자 지정 필드 앞에 있는 경우에만 사용할 수 있습니다. </p> <p>계산된 사용자 지정 필드에 논리 건너뛰기 를 사용할 수 없습니다.</p> </td> 
     </tr> 
     <tr> 
