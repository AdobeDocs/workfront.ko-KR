...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>작업 역할, 비율 카드 및 할당에 대한 사용자 정의 양식을 만들려면: Workflow Ultimate</p>
      <p>기타 지원되는 모든 개체(모든 Workfront 또는 워크플로 패키지)에 대한 사용자 정의 양식을 만들려면</p> </td> 
  </tr>  
  <tr> 
...
1. 사용자 정의 양식을 첨부할 개체 유형을 선택한 다음 **계속**&#x200B;을 클릭합니다.
   ...
   ![개체 유형 선택](assets/new-custom-form-select-objects-new-spectrum-icons.png)

   +++ 를 확장하여 사용자 정의 양식을 지원하는 오브젝트 목록을 봅니다.
   ...
   * 팀
   ...
   Workflow Ultimate 패키지를 사용하는 경우 다음 개체에 대한 사용자 정의 양식을 만들 수도 있습니다.
...
1. (선택 사항) 더 많은 개체에 첨부할 수 있도록 폼에 개체 형식을 추가하려면 폼 디자이너 헤더에서 **개체 형식**&#x200B;을 클릭합니다. 추가할 객체 유형을 선택하고 양식에서 삭제할 객체 유형을 선택 취소합니다.
...
### 필드 API 이름 및 레이블에 대한 참고 사항 {#notes-on-field-names-and-labels}
...
* 모든 필드에는 API 이름이 필요합니다. 이 이름은 보고서, 홈 및 API 상호 작용과 같은 Workfront 전체의 다양한 영역에 사용자 정의 필드를 추가할 때 시스템이 이 필드를 식별하는 방법입니다. 필드나 위젯을 처음 구성하는 경우 레이블을 입력하면 API 이름 필드가 자동으로 채워져 일치합니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.

* 각 사용자 정의 필드 API 이름은 조직의 Workfront 인스턴스에서 고유해야 합니다. 이렇게 하면 다른 사용자 정의 양식에 대해 이미 만들어진 파일을 다시 사용할 수 있습니다.

* 그렇게 할 수도 있지만 본인이나 다른 사용자가 Workfront에서 사용자 정의 양식을 사용하기 시작한 후에는 이 API 이름을 변경하지 않는 것이 좋습니다. 이 필드를 선택하면 이제 Workfront의 다른 영역에서 참조될 수 있는 사용자 지정 필드가 시스템에서 더 이상 인식되지 않습니다.

  예를 들어 사용자 정의 필드를 보고서에 추가하고 나중에 해당 API 이름을 변경하면 Workfront은 보고서에서 이를 인식하지 못하며 새 이름을 사용하여 보고서에 다시 추가하지 않으면 제대로 작동하지 않습니다.

* API 이름은 기본적으로 읽기 전용입니다. API 이름을 편집할 수 있게 하려면 **API 이름** 옆에 있는 **편집** 아이콘을 클릭합니다. 그런 다음 확인 상자에 **confirm**&#x200B;을 입력하고 **필드 이름 바꾸기**&#x200B;를 클릭합니다.

* 기본 제공 Workfront 필드에 이미 사용된 API 이름은 입력하지 않는 것이 좋습니다.

* Workfront의 다른 영역에서 필드를 사용할 때 오류를 방지하기 위해 사용자 지정 필드 API 이름에 마침표/점 문자를 사용하지 않는 것이 좋습니다.

* 다음 특수 문자는 사용자 지정 필드 레이블 및 API 이름에서 지원되지 않습니다.
...
* **서식 있는 텍스트**: 사용자가 필드에 여러 줄의 텍스트를 입력하고 굵게, 기울임꼴, 밑줄, 글머리 기호, 번호 매기기, 아래 첨자 및 위 첨자, 하이퍼링크, 블록 따옴표, 머리글 및 표로 텍스트 서식을 지정할 수 있습니다. 문자 길이는 15,000자로 제한되어 있어 텍스트와 서식을 지정할 수 있는 공간이 넓습니다.

  >[!NOTE]
  >
  >서식 있는 텍스트 필드 유형이 텍스트를 서식 있는 필드 유형으로 바꿉니다. 오른쪽의 필드 옵션에서 **서식 있는 텍스트로 변환** 단추를 클릭하여 서식 있는 필드가 있는 기존 텍스트를 서식 있는 텍스트로 빠르게 변환할 수 있습니다.
...
1. 화면 오른쪽에서 추가하려는 사용자 정의 필드 유형에 사용할 수 있는 옵션을 구성합니다.
   ...
   <table>
    ...
    <tr>
     <td>API 이름</td>
    <td><p>(필수) 이 이름은 시스템이 필드를 식별하는 방법입니다. 위젯을 처음 구성하는 경우 레이블을 입력하면 API 이름 필드가 일치되도록 자동으로 채워집니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p>
    ...
    <tr>
      <td>재무 권한 유형</td>
      <td><p>이 사용자 정의 필드를 보거나 편집하려면 먼저 사용자가 보유해야 하는 재무 권한 유형을 선택하십시오. 통화 형식을 선택해야 합니다.</p>
      <ul>
...
날짜 필드에는 사용자가 날짜 및 시간을 선택할 수 있는 달력이 표시됩니다.

날짜 필드를 추가하려면:

1. 화면 왼쪽의 **새 필드** 탭에서 **날짜**&#x200B;를 찾아 캔버스의 섹션으로 끌어서 놓습니다.
...
1. 화면 오른쪽에서 사용자 정의 필드에 대한 옵션을 구성합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 이름</td> 
      <td> <p>(필수) 이 이름은 시스템이 필드를 식별하는 방법입니다. 위젯을 처음 구성하는 경우 레이블을 입력하면 API 이름 필드가 일치되도록 자동으로 채워집니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p> <p>자세한 내용은 <a href="design-a-form.md#notes-on-field-names-and-labels">필드 이름 및 레이블에 대한 참고</a>을 참조하세요.</p> </td> 
     </tr>
     <tr> 
      <td role="rowheader">지침</td> 
     ...
     <tr> 
      ...
      <td><p>필드에 날짜와 함께 시간을 표시하려면 이 옵션을 선택합니다.</p></td>
     </tr> 
     <tr>
      <td>활성</td>
      <td><p>이 옵션은 기본적으로 켜져 있습니다.<p><p>필드를 비활성으로 설정하면 보고서, 필터 및 보기에서 제외되며 사용자 정의 양식 필드 라이브러리에서 더 이상 사용할 수 없습니다.</p></td>
...
외부 조회 필드는 외부 API를 호출하고 드롭다운 필드에 값을 옵션으로 반환합니다. 사용자 정의 양식이 첨부된 오브젝트로 작업하는 사용자는 외부 조회 필드가 단일 필드인지 다중 선택 필드인지에 따라 드롭다운에서 이러한 옵션 중 하나 이상을 선택할 수 있습니다. 외부 조회 필드는 목록 및 보고서에서도 사용할 수 있습니다.
...
1. 화면 오른쪽에서 사용자 정의 필드에 대한 옵션을 구성합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 이름</td> 
      <td> <p>(필수) 이 이름은 시스템이 필드를 식별하는 방법입니다. 필드를 처음 구성할 때 레이블을 입력하면 API 이름 필드가 자동으로 채워집니다. 그러나 레이블 및 API 이름 필드는 동기화되지 않으므로 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p>
      <p>자세한 내용은 <a href="design-a-form.md#notes-on-field-names-and-labels">필드 이름 및 레이블에 대한 참고</a>을 참조하세요.</p> </td>
     </tr> 
      <td role="rowheader">지침</td> 
      <td> <p>사용자 정의 필드에 대한 추가 정보를 입력합니다. 사용자가 사용자 정의 양식을 작성할 때 물음표 아이콘 위로 마우스를 가져가 여기에 입력하는 정보가 포함된 도구 설명을 볼 수 있습니다.</p> </td> 
     ...
     <tr> 
      ...
      <p>다른 필드에 종속된 외부 조회 필드의 경우({fieldName} 구문 사용), API에서 반환되는 옵션은 다른 필드에 입력한 문자열 또는 값과 일치하는 옵션으로 제한됩니다. (이 기능은 목록 및 보고서에서 지원되지 않습니다.)</p></li>
      <li>{referenceObject}.{fieldName} - 필드가 개체의 일부입니다. 이 구문은 사용자 정의 표현식과 유사합니다. (예: portfolioID={project}.{portfolioID})</li></ul>
...
1. 화면 왼쪽의 **새 필드** 탭에서 **내부 조회** 또는 **다중 선택 내부 조회**&#x200B;를 찾아 캔버스의 섹션으로 끌어서 놓습니다.
1. 화면 오른쪽에서 사용자 정의 필드에 대한 옵션을 구성합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td><p>필드와 연결할 객체 유형을 선택합니다.</p> <p><strong>적용</strong> 또는 <strong>저장 및 닫기</strong>를 클릭하면 필드의 개체 유형을 변경할 수 없습니다.</p>
      <p><strong>메모:</strong></p>
      <ul><li>Workfront 관리자가 Workfront 사용자 인터페이스에서 포트폴리오, 프로그램 또는 프로젝트 이름을 사용자 지정한 경우 개체에 대한 기본 Workfront 이름이 사용자 지정한 이름이 아니라 이 드롭다운 목록에 표시됩니다. 이에 대한 도움이 필요한 경우 Workfront 관리자에게 문의하십시오.</li>
     ...
     <tr>
      ...
      <td><p>사용자가 필드를 사용할 때 선택할 수 있는 개체를 제한하기 위해 개체 유형에 대한 필터를 추가합니다. </p> <p>예를 들어 사용자 이름이 다음 기준을 충족하는 경우에만 선택할 수 있도록 필드를 제한할 수 있습니다.</p> 
       ...
       <p><b>팁:</b> 내부 조회 필드에 필터를 직접 추가하기 전에 필터를 테스트하는 보고서를 만들 수 있습니다. 이렇게 하면 필터가 올바른 개체를 반환하는지 확인하는 데 도움이 됩니다. 그런 다음 보고서에서 텍스트 모드로 전환하고 텍스트 모드 문을 복사하여 내부 조회 필터에 추가할 수 있습니다.</p>
       <p><b>참고</b>:
       <ul> 
        <li>기존 사용자 정의 양식을 편집하는 경우 내부 조회 필드에 필터를 추가해도 사용자가 필드를 사용하여 이미 추가한 오브젝트(필터의 범위 밖)는 제거되지 않습니다.</li> 
        <li>모바일 장치에서는 이 필터를 사용할 수 없습니다. 내부 조회 필드에 필터를 사용하는 경우 필드가 필터의 영향을 받지 않고 사용자의 모바일 장치에 표시됩니다.</li> 
        </ul></p>
      </td>  
     </tr>
     <tr> 
...
1. 위젯에 대해 다음 속성 중 하나를 입력하거나 편집합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 이름</td> 
      <td> <p>(필수) 이 이름은 시스템이 위젯을 식별하는 방법입니다. 위젯을 처음 구성하는 경우 레이블을 입력하면 API 이름 필드가 일치되도록 자동으로 채워집니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p> <p>자세한 내용은 <a href="design-a-form.md#notes-on-field-names-and-labels">필드 이름 및 레이블에 대한 참고</a>을 참조하세요.</p> </td> 
     </tr> 
     <tr> 
...
사용자 정의 양식에 Workfront 기본 필드를 추가할 수 있습니다. 사용자 정의 양식을 오브젝트에 첨부하면 필드가 오브젝트 데이터에서 채워집니다. 예를 들어 사용자 정의 필드가 설명 필드를 참조하고 프로젝트에 첨부된 사용자 정의 양식에 있는 경우 프로젝트 설명을 가져옵니다. (데이터가 없는 경우 필드에 &quot;N/A&quot;가 표시될 수 있습니다.)

+++ 를 확장하면 지원되는 기본 필드 목록이 표시됩니다.

이 목록에는 사용자 정의 양식에 사용할 수 있는 기본 필드와 각 필드를 사용할 수 있는 개체 유형이 표시됩니다.
...
* **전환된 문제 작성자**: 프로젝트
* **통화**: 프로젝트, 템플릿
* **설명**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업, Portfolio, 프로그램, 그룹, 문서, 요금 카드, 작업 역할
* **기간**: 작업, 템플릿 작업
* **기간 유형**: 작업, 템플릿 작업
* **기간 단위**: 작업, 템플릿 작업
* **입력한 사람**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업, 그룹
* **시작 날짜**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업, 그룹
* **환율 날짜**: 프로젝트
* **고정 비용**: 프로젝트, 템플릿
* **고정 수입**: 프로젝트, 템플릿, 청구 기록
* **그룹**: 프로젝트, 템플릿, Portfolio, 프로그램, 회사, 등급 카드
* **문제 유형**: 문제
* **반복**: 작업
* **마지막으로 업데이트한 사람**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업
* **마지막 업데이트 날짜**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업
* **이름**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업, Portfolio, 프로그램, 그룹, 문서, 회사, 요금 카드, 작업 역할
* **소유자**: 프로젝트, 템플릿, Portfolio, 프로그램
* **성과 지표 메서드**: 프로젝트, 템플릿
* **계획된 이익**: 프로젝트, 템플릿
* **계획된 완료 일자**: 프로젝트, 작업, 문제
* **계획된 기간**: 프로젝트, 템플릿
* **계획된 시간**: 프로젝트, 작업, 문제, 템플릿 작업
* **계획된 시작 일자**: 프로젝트, 작업, 문제
* **Portfolio**: 프로젝트, 템플릿, 프로그램
* **우선 순위**: 프로젝트, 작업, 문제, 템플릿, 템플릿 작업
* **프로그램**: 프로젝트, 템플릿
* **예상 완료 일자**: 프로젝트, 작업
* **예상 기간(분)**: 작업
* **예상 시작 일자**: 프로젝트, 작업
...
1. 화면 오른쪽에서 사용자 정의 필드에 대한 옵션을 구성합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 이름</td>
      <td> <p>(필수) 이 이름은 시스템이 필드를 식별하는 방법입니다. 필드를 처음 구성할 때 레이블을 입력하면 API 이름 필드가 자동으로 채워집니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p>
      <p>자세한 내용은 <a href="design-a-form.md#notes-on-field-names-and-labels">필드 이름 및 레이블에 대한 참고</a>을 참조하세요.</p></td> 
     </tr> 
     <tr> 
      <td role="rowheader">지침</td> 
     ...
     <tr>
      ...
      <td><p>참조 필드에 대한 필터를 추가하여 사용자가 필드를 사용할 때 선택할 수 있는 항목 목록을 제한합니다. </p> <p>예를 들어 사용자 이름이 다음 기준을 충족하는 경우에만 선택할 수 있도록 필드를 제한할 수 있습니다.</p> 
       ...
       <p>시스템 필터가 이 필드에 있는 경우 사용자 지정 필터를 추가하지 않으면 기본적으로 이 필터가 적용됩니다. 사용자 지정 필터는 시스템 필터를 재정의합니다.</p>
...
1. 위젯에 대해 다음 속성 중 하나를 입력하거나 편집합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 이름</td> 
      <td> <p>(필수) 이 이름은 시스템이 위젯을 식별하는 방법입니다. 위젯을 처음 구성하는 경우 레이블을 입력하면 API 이름 필드가 일치되도록 자동으로 채워집니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p>
    <p>자세한 내용은 <a href="design-a-form.md#notes-on-field-names-and-labels">필드 이름 및 레이블에 대한 참고</a>을 참조하세요.</p></td> 
     </tr> 
     <tr> 
      <td role="rowheader">URL</td> 
...
1. 화면 오른쪽에서 사용자 정의 필드에 대한 옵션을 구성합니다.
   ...
   <table style="table-layout:auto"> 
    ...
    <tr> 
     ...
     <tr> 
      <td role="rowheader">API 이름</td>
      <td> <p>(필수) 이름은 시스템이 필드를 식별하는 방법입니다. 필드를 처음 구성할 때 레이블을 입력하면 API 이름 필드가 자동으로 채워집니다. 레이블 및 API 이름 필드는 동기화되지 않습니다. 이렇게 하면 시스템에 표시되는 이름을 변경하지 않고도 사용자에게 표시되는 레이블을 변경할 수 있는 옵션이 제공됩니다.</p></td> 
     </tr> 
     <tr> 
      <td role="rowheader">지침</td> 
...
UI 확장 필드 유형을 사용하여 Workfront 사용자 정의 양식에 앱을 포함할 수 있습니다. UI 확장을 만들려면 Adobe Developer Console에서 Adobe App Builder에 액세스할 수 있어야 합니다. 자세한 내용은 문서 [Workfront App Builder을 사용하여 Workfront에 대한 사용자 지정 응용 프로그램 만들기](/help/quicksilver/app-builder/app-builder.md)에서 [Adobe 사용자 지정 양식을 사용하여 앱 포함](/help/quicksilver/app-builder/app-builder.md#embed-an-app-using-a-workfront-custom-form)을 참조하십시오.
