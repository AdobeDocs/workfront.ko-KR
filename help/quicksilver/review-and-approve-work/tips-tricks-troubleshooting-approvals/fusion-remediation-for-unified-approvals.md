...
description: 귀사에서 Adobe 엔터프라이즈 스토리지와 통합 검토 및 승인을 채택함에 따라 레거시 Workfront 증명을 기반으로 구축된 Workfront Fusion 시나리오를 인벤토리, 분류 및 수정합니다.
author: Courtney
...
기존 Workfront 증명을 기반으로 구축된 Workfront Fusion 시나리오는 Adobe 엔터프라이즈 스토리지 프로젝트에 대해 자동으로 작동하지 않습니다. 증명별 모듈, 웹후크 및 API 엔드포인트는 경우에 따라 직접적인 등가물을 가지며 다른 경우에는 상당한 변경을 가집니다. 이 문서는 이러한 시나리오에 의존하는 팀을 Adobe 엔터프라이즈 스토리지 롤아웃으로 가져오기 전에 영향을 받는 시나리오를 인벤터리하고, 분류하고, 수정 경로를 결정하는 데 도움이 됩니다.

기존 Workfront 프로젝트에 범위가 지정된 시나리오는 오늘날과 같이 계속 작동합니다. 이 문서에 설명된 수정 작업은 Adobe 엔터프라이즈 스토리지 프로젝트에 대해 실행하려는 시나리오에 적용됩니다.

통합 검토 및 승인을 위한 기본 지원이 포함된 Fusion 커넥터는 2026년 3분기에 사용할 수 있습니다. 지금 다시 빌드를 계획하지만 빌드하기 전에 새 커넥터를 기다리는 것이 좋습니다. 시나리오는 대체되는 것보다 더 간단하고 안정적입니다.

자세한 자동화 지침은 2026년 3분기 Fusion 커넥터 릴리스와 함께 게시됩니다. 이 문서를 사용하여 시나리오를 인벤터리하고 분류하므로 해당 지침을 사용할 수 있게 되면 작업을 수행할 준비가 된 것입니다.

조직이 Adobe 엔터프라이즈 스토리지에서 Workfront으로 이동할 때 어떤 변화가 있는지에 대한 대략적인 요약을 보려면 [Adobe 엔터프라이즈 스토리지에서 Workfront으로 이동](/help/quicksilver/review-and-approve-work/workfront-storage.md)을 참조하십시오.


## Adobe 엔터프라이즈 스토리지 프로젝트의 Fusion 변경 사항
...
| 시나리오 유형 | 영향 | 앞으로 경로 |
|---|---|---|
| 증명 생성 및 라우팅 | 중단 | 통합 승인 API를 사용하여 다시 빌드 |
| 증명 상태 웹후크 | 중단 | 새 승인 이벤트 트리거로 다시 빌드 |
| 문서 업로드 트리거 | 부분: 다시 테스트 필요 | 마이그레이션 후 감사 및 재테스트 |
...
1. **지금 인벤토리** 활성 Fusion 시나리오의 전체 목록을 가져와서 증명 생성, 증명 상태, 문서 승인 또는 승인 라우팅을 참조하는 모든 시나리오에 태그를 지정합니다. Adobe 엔터프라이즈 스토리지가 활성화될 때까지 기다리지 마십시오.
1. **이전 섹션의 조건을 기준으로 각 시나리오를 편집, 다시 작성 또는 사용 중지로 분류**&#x200B;합니다.
1. **증명 종속 시나리오를 일시 중지**&#x200B;한 다음 여기에 종속된 팀을 Adobe 엔터프라이즈 스토리지 파일럿으로 가져옵니다. 새 모델에 대해 오래된 증명 기반 자동화를 실행하면 자동 오류가 발생하거나 작업이 중복될 수 있습니다.
...
1. **다시 빌드하기 전에 2026년 3분기 Fusion 커넥터 업데이트를 기다립니다.** 업데이트된 커넥터는 통합 검토 및 승인을 위해 특별히 빌드된 모듈을 노출하고 재빌드를 훨씬 더 간단하고 안정적으로 만듭니다. Workfront API 버전 22에 대해서는 이전에 다시 빌드하지 않는 것이 좋습니다. 시간이 중요한 시나리오로 진행할 경우, 새 커넥터가 릴리스되면 해당 작업을 다시 방문하도록 계획하십시오.
