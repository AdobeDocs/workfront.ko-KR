...
1. **필터** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. 연산자를 **같음**(으)로 유지한 다음 **검토 보류 중**&#x200B;을(를) 선택하십시오.

   1. **조건 추가**&#x200B;를 클릭합니다.
   1. 빈 조건 필터를 클릭하고 **필드 선택**&#x200B;을 클릭한 다음 **승인 유형**&#x200B;을 선택합니다.
   1. 연산자를 **같음**(으)로 유지한 다음 **문서 버전**&#x200B;을 선택하십시오.
...
1. **세부 정보** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **이름** 필드에 _기한 초과_&#x200B;을(를) 입력하십시오.
   1. **설명** 필드에 _승인 단계 기한을 과거_&#x200B;로 입력하십시오. 이 설명은 KPI 값 아래에 캡션으로 표시됩니다.
...
1. **필터** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. 빈 조건 필터를 클릭한 다음 **필드 선택**&#x200B;을 클릭합니다.
   1. **상태**&#x200B;를 선택하십시오.
   1. 연산자를 **포함하지 않음**(으)로 변경한 다음 **승인됨**&#x200B;을(를) 선택하십시오.
...
1. **세부 정보** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **이름** 필드에 _완료_&#x200B;을(를) 입력하십시오.
   1. **설명** 필드에 _승인 상태 수_&#x200B;를 입력하십시오. 이 설명은 KPI 값 아래에 캡션으로 표시됩니다.
...
1. **세부 정보** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **이름** 필드에 _취소됨_&#x200B;을(를) 입력하십시오.
   1. **설명** 필드에 _2주 이상 승인 기한_&#x200B;을 입력하십시오. 이 설명은 KPI 값 아래에 캡션으로 표시됩니다.
...
1. **필터** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. 연산자를 **포함하지 않음**(으)로 변경하고 **승인됨**&#x200B;을(를) 선택하십시오.
   ...
   1. **조건 추가**&#x200B;를 클릭합니다.
   1. 빈 조건 필터를 클릭하고 **필드 선택**&#x200B;을 클릭한 다음 **승인 관계**&#x200B;를 선택한 다음 **승인 유형**&#x200B;을 선택합니다.
   1. 연산자를 **같음**(으)로 유지한 다음 **문서 버전**&#x200B;을 선택하십시오.
...
1. **차트 빌드** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **아래쪽(X) 축**&#x200B;의 **필드 업데이트**&#x200B;를 클릭한 다음 **승인** > **상태**&#x200B;를 선택합니다.
...
1. **세부 정보** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **이름** 필드에 _수정 버전_&#x200B;을 입력하십시오.
   1. **설명** 필드에 _이번 달 말 이전에 계획된 불완전한 결정이 있는 문서의 수정 버전 수_&#x200B;를 입력하십시오. 이 텍스트는 차트 이름 옆에 툴팁으로 표시됩니다.
...
1. **차트 빌드** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **차트 종류** 드롭다운 메뉴에서 **막대**&#x200B;를 선택한 상태로 둡니다.
   1. **막대 유형** 드롭다운 메뉴에서 **단순**&#x200B;을(를) 선택한 상태로 둡니다.
   1. **아래쪽(X) 축**&#x200B;에 대한 **필드 업데이트**&#x200B;를 클릭한 다음 **승인** > **문서 버전** > **버전**&#x200B;을 선택합니다.
   1. **집계 유형**&#x200B;을(를) **Count**(으)로 설정합니다.
   1. **필드 업데이트**&#x200B;에서 **왼쪽(Y) 축**&#x200B;을 클릭한 다음 **승인** > **문서 버전** > **문서** > **이름**&#x200B;을 선택합니다.
...
1. 아래 단계에 따라 **테이블 빌드** 섹션을 구성하십시오.
   ...
   <table>
    ...
    <td>승인 &gt; 승인 단계 &gt; 승인 단계 참가자* &gt; 요청자 &gt; 검색 상자에 <em>이름</em>을(를) 입력합니다.</td>
    </tr>
    <tr>
    <td><strong>요청한 날짜</strong></td>
    <td>승인 &gt; 승인 단계 &gt; 승인 단계 참가자* &gt; 생성일</td>
    </tr>
    <tr>
    <td><strong>승인자</strong></td>
    <td>승인 &gt; 승인 단계 &gt; 승인 단계 참가자* &gt; 참가자 사용자 &gt; 검색 상자에 <em>이름</em>을 입력합니다.</td>
    </tr>
    </table>
...
1. **필터** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. 빈 조건 필터를 클릭한 다음 **필드 선택**&#x200B;을 클릭합니다.
   1. **상태**&#x200B;를 선택하십시오.
   1. 연산자를 **같음**(으)로 변경한 다음 **승인 보류 중**&#x200B;을 선택하십시오.
...
1. **필터 편집** > **조건 추가**&#x200B;를 클릭합니다.
   1. 빈 조건 필터를 클릭하고 **필드 선택**&#x200B;을 클릭합니다.
   1. **승인 단계** > **승인 단계 참가자** > **요청자** > 검색 상자에 _이름_&#x200B;을 입력하십시오.
   1. 연산자를 **같음**(으)로 변경한 다음, **나를(로그인한 사용자)**&#x200B;을(를) 선택하여 프로젝트 소유자로 표시된 Workfront의 프로젝트를 표시합니다.
      ![보류 중인 승인 테이블 필터 예](assets/pending-approvals-my-project-filter.png)
1. 화면 오른쪽 상단에서 **저장**&#x200B;을 클릭합니다.
...
1. **세부 정보** 섹션을 구성하려면 아래 단계를 따르십시오.
   ...
   1. **이름** 필드에 _기한 경과 승인_&#x200B;을(를) 입력하십시오.
   1. (선택 사항) **설명** 필드에 설명을 입력합니다. 이 텍스트는 차트 이름 옆에 툴팁으로 표시됩니다.
...
1. **필터 편집** > **조건 추가**&#x200B;를 클릭합니다.
   ...
   1. **조건 추가**&#x200B;를 클릭합니다.
   1. 빈 조건 필터를 클릭하고 **필드 선택**&#x200B;을 클릭한 다음 승인 관계를 선택하고 승인 유형을 선택합니다.
   1. 연산자를 **같음**(으)로 유지한 다음 문서 버전을 선택하십시오.
