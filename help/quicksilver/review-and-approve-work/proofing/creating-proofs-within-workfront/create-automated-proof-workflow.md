...
1. 워크플로우 유형 섹션에서 다음을 선택합니다. **자동화**.
1. (선택 사항) Workfront 관리자가 만들고 사용자와 공유한 자동화된 워크플로 템플릿을 사용하려면 을 클릭합니다. **템플릿 추가**&#x200B;표시되는 상자에서 템플릿을 선택한 다음 을 클릭합니다 **템플릿 추가**.
...
1. 자동화된 워크플로의 첫 번째 단계를 구성합니다.
   ...
   1. (선택 사항) 첫 번째 단계의 이름을 만들려면 **1단계**&#x200B;을 클릭한 다음 이름을 입력합니다.
   1. 다음에서 **수신자** 단계에 대한 섹션에서 단계에 검토자를 추가합니다.
      ...
      >* 사용자를 다른 단계로 직접 드래그하거나 **단계** 다이어그램입니다. 여러 사용자를 선택하려면 Shift+Ctrl(Windows) 또는 Shift+Command(Mac)를 누릅니다.
   ...
   1. 클릭 **스테이지 설정**.
   1. 다음 클릭: **단계 활성화** 옵션을 사용하여 스테이지를 활성화할 방법을 나타냅니다.

      첫 번째 단계에 대해서만 선택할 수 있습니다. **증명 생성 시**, **특정 날짜 및 시간에**, 또는 **수동**.

   1. (조건부) (선택한 경우) **특정 날짜 및 시간에** 이전 단계에서 의 단계를 활성화할 날짜와 시간을 선택합니다. **활성화 날짜** 상자가 나타납니다.
      ...
      <table style="table-layout:auto">
       ...
       <tbody>
        <tr>
         ...
         <td><p>단계의 기한을 설정하려면 <strong>기한 옵션</strong> 드롭다운 목록입니다. 그런 다음 아래에 <strong>기한</strong>를 클릭하고 다음 중 하나를 수행합니다.</p>
          <ul>
           <li>다음을 선택한 경우 <strong>특정 날짜 설정</strong>: 원하는 기한 날짜 및 시간을 선택합니다.</li>
           <li>다음을 선택한 경우 <strong>스테이지 활성화 날짜부터 계산</strong>: 단계 활성화 날짜에 추가할 영업일 수를 선택하여 기한을 결정합니다.</li>
          </ul></td>
        </tr>
        <tr>
        ...
        <tr>
         ...
         <td><p>단계의 기본 의사 결정자를 선택합니다(승인자 이상의 증명 역할을 가진 사용자를 단계에 하나 이상 추가한 후에만 사용 가능). 기본 의사 결정자를 선택하는 경우 <strong>필요한 결정은 단 하나입니다.</strong> 옵션이 이 스테이지에서 비활성화됩니다.</p></td>
        </tr>
        <tr>
         <td role="rowheader">이 단계에 대해 하나의 결정만 필요</td>
         <td>의사 결정자 중 한 명이 결정을 내리면 전체 검토 프로세스를 종료합니다.<p>에서 사용자를 지정한 경우에는 이 옵션을 사용할 수 없습니다. <strong>주요 의사 결정자</strong>드롭다운 메뉴.</p></td>
        </tr>
        <tr>
...
1. 다른 단계를 추가하고 구성하려면:
   ...
   1. 클릭 **새 단계**.
   1. (선택 사항) 첫 번째 단계의 이름을 만들려면 **2단계** (또는 **3단계**, **4단계**&#x200B;등)을 누른 다음 이름을 입력합니다.

   1. 다음을 클릭합니다. **단계 활성화**&#x200B;을 선택한 다음 옵션을 선택하여 스테이지가 자동으로 활성화되는지 또는 수동으로 활성화되는지 여부를 지정합니다.

      옵션 외에 **증명 생성 시**, **특정 날짜 및 시간에**, 또는 **수동**, 이전 단계에서 발생한 사항에 따라 달라지는 옵션을 선택할 수 있습니다.
   ...
   1. 이전 단계에서 발생한 사항에 따라 달라지는 단계 활성화 옵션을 선택한 경우 표시되는 옵션을 사용하여 활성화 설정을 구성합니다.
      ...
      예를 들어 을 선택한 경우 **이전 단계 상태가 변경되는 경우**&#x200B;를 선택하고 **이전 단계**&#x200B;에서 상태를 선택한 다음 **상태가 다음으로 변경됨** 상자.
...
1. 다음에서 **이메일 알림** 섹션에서 선택한 사용자에게 이메일 알림 및 사용자 정의 메시지를 전송할지 여부를 선택합니다 [자동화된 워크플로를 사용하여 고급 증명 만들기](#workflow) 이 문서의 앞부분에서:
...
1. 증명이 생성된 후 다음을 클릭: **증명 열기** 증명 뷰어를 시작합니다.
   ...
   계정에 증명이 활성화되지 않은 사용자는 여전히 문서를 보고 증명에 대한 댓글을 달 수 있습니다 [.](../../../timesheets/config-timesheet-prefs/config-time-logged-hrs-days.md)
