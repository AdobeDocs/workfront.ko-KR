...
* 이전 증명 버전에 대해 검토자에게 이메일로 알리지 않은 경우(예: 표준 또는 사용자 정의 이메일 없음) 새 버전 페이지에는 기본적으로 메시지가 포함되지 않습니다. 검토자에게 새 버전을 알리려면 메시지 보내기 링크를 클릭합니다. 그런 다음 사용자 정의 메시지 추가 링크를 클릭하여 사용자 정의 제목과 메시지를 추가할 수 있습니다.
