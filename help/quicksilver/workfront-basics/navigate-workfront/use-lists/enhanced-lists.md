...
title: 향상된 목록 사용
description: 고급 목록은 목록 항목을 표시하기 위해 표 형식을 사용하며 표준 목록과는 다른 모양과 느낌을 가집니다.
author: Lisa
...
보고 있는 향상된 목록에 따라 다음 중 하나를 수행하십시오.

1. 목록의 오른쪽 상단에 있는 파란색 단추를 클릭합니다. 이 옵션을 선택하면 정보를 입력할 수 있는 대화 상자가 열립니다. 데이터는 표에 새 행으로 저장됩니다.

   또는

1. 목록 맨 아래에 있는 **새 행**&#x200B;을 클릭합니다. 이 옵션은 테이블에 새 행을 추가합니다. 셀을 두 번 클릭하여 정보를 입력합니다. 각 셀은 목록 항목에 대한 필드를 나타냅니다. 필드가 있어야 목록에 표시됩니다.

   고급 목록은 다음 필드 유형을 지원합니다.

   * 한 줄 텍스트: 제한된 영숫자 정보를 입력할 수 있습니다.
   * 숫자: 숫자 형식으로 정보를 입력해야 합니다.
...
1. 열 순서를 바꾸려면 **끌기** 아이콘 ![끌기 아이콘](assets/drag-icon.png)을 클릭하고 열을 원하는 위치로 이동합니다. 열을 이동하면 목록이 자동으로 변경됩니다.
   ...
   일부 목록을 사용하면 열 자체를 기본 설정 순서로 끌어서 놓을 수도 있습니다.
...
1. <span class="preview">(선택 사항) 열을 다시 정렬하려면 **선택됨** 섹션의 열 이름을 마우스로 가리킨 다음 **드래그** 아이콘 ![드래그 아이콘](assets/drag-icon.png)을 클릭합니다. 열을 세로로 이동하여 순서대로 배치하십시오.</span>
1. **저장**&#x200B;을 클릭합니다.
...
행 순서를 변경하려면 다음을 수행합니다.

1. 행을 원하는 순서로 끌어다 놓습니다.

   >[!NOTE]
   >
   >정렬을 적용한 경우에는 행 순서를 수동으로 변경할 수 없습니다.
...
1. **저장**&#x200B;을 클릭합니다.
   ...
   사용자와 보기를 공유한 다음 해당 액세스 권한을 제거하면 사용자의 **공유 보기**&#x200B;에서 보기가 제거됩니다. 액세스 권한이 제거될 때 목록에 공유 보기가 적용된 경우 시스템 기본 보기가 적용됩니다.
...
편집 권한이 없는 보기가 공유되면 보기를 복사하고 새 이름으로 저장할 수 있습니다. 보기를 복사하려면 먼저 보기를 변경해야 합니다.

1. 보기 드롭다운에서 설정을 수정하고 복사하려는 **공유 보기**&#x200B;의 보기 위로 마우스를 가져간 후 **자세히** 메뉴 ![추가 메뉴](assets/more-icon.png)를 클릭하고 **기본 설정으로 복사**&#x200B;를 클릭합니다.

   새 보기가 자동으로 만들어집니다. 복사된 보기의 이름이 `Original view name (copy)` 패턴을 따르며 보기의 **내 보기** 섹션에 표시됩니다.

   이 보기의 소유자이며 이름을 바꾸고, 편집하고, 공유하거나 삭제할 수 있습니다. 원본 뷰의 소유자가 해당 뷰에 대한 공유 액세스를 제거하면 공유 원본을 복사하여 만든 뷰에 대한 액세스 권한이 계속 유지됩니다.

   >[!NOTE]
   >
   >**기본 설정으로 복사** 옵션은 사용자와 공유된 보기를 변경한 경우에만 사용할 수 있습니다.
...
편집 권한이 없는 보기를 공유하고 해당 보기를 업데이트하면 원래 보기로 다시 설정할 수 있습니다.

1. **보기** 드롭다운에서 재설정할 **공유 보기**&#x200B;의 보기 위로 마우스를 가져간 후 **자세히** 메뉴 ![추가 메뉴](assets/more-icon.png)를 클릭하고 **기본값으로 재설정**&#x200B;을 클릭합니다.

   보기 요소(열, 필터 및 그룹화)는 사용자와 공유된 원래 설정으로 재설정됩니다.

   >[!NOTE]
   >
   >**기본값으로 재설정** 옵션은 나와 공유된 보기를 변경한 경우에만 사용할 수 있습니다.

   ![보기 옵션 복사 및 재설정](assets/glist-copy-view-shared-with-you.png)
...
조건부 서식은 일반적인 기준에 따라 보기에서 중요한 정보를 강조 표시하는 데 도움이 됩니다.

1. **셀 서식 지정** 아이콘 ![셀 서식 지정 아이콘](assets/format-cells-icon.png)을 클릭합니다. **서식** 상자가 열립니다.
...
1. **If** 줄에서 필드를 선택하고 필드 값을 선택한 다음 수정자를 추가합니다. 수정자는 선택한 필드 유형에 따라 변경됩니다.
   ...
   >[!TIP]
   >
   >고급 목록에 표시되는 필드만 조건부 서식에 사용할 수 있습니다.

1. (선택 사항) 필드 값을 추가하는 대신 **다른 필드와 비교** 아이콘 ![다른 필드와 비교](assets/compare-to-another-field-icon.png)를 클릭하고 선택한 필드의 값과 비교할 필드를 선택합니다. 예를 들어 요청 항목에 대한 제목 및 설명 필드를 비교할 수 있습니다.
...
1. **서식** 줄에서 서식을 지정할 열을 나타내는 필드를 선택합니다.
1. (선택 사항) 선택한 필드 옆에 있는 **색상 원** 아이콘 ![색상 형식 아이콘](assets/color-format-icon.png)을 클릭하여 확장하고 **셀 채우기** 영역에서 다른 색상을 선택하여 셀의 배경색을 변경하거나 **텍스트 색상** 영역에서 색상을 선택하여 셀의 텍스트 색상을 변경합니다.
...
1. 그룹화가 적용되면 [그룹] 옵션을 다시 열어 모든 그룹화를 한 번에 축소 또는 확장하거나, 다른 필드로 그룹화를 변경하거나, 모든 그룹화를 지울 수 있습니다.
   ...
   목록에 그룹화를 적용하면 **그룹화** 단추에 표시기가 나타납니다.
...
1. 열 위로 마우스를 가져간 다음 아래쪽 화살표를 클릭하고 **정렬**&#x200B;을 선택합니다.
   ...
   >[!NOTE]
   >
   >일부 열은 목록에 따라 정렬 불가능할 수 있습니다.

   ![열을 기준으로 정렬](assets/glist-sort-by-column.png)

1. (선택 사항) 그룹화 내에서 작업을 정렬하려면 **그룹화**&#x200B;을 클릭하고 적용된 그룹화의 줄로 이동한 후 정렬 드롭다운을 클릭하고 오름차순 또는 내림차순을 선택합니다.

   ![그룹화에서 정렬](assets/sort-in-groups.png)

   >[!TIP]
   >
   >정렬 순서는 정렬하는 필드 유형에 따라 다릅니다.
...
1. 목록의 오른쪽 위 모서리에 있는 검색 상자에 검색할 키워드를 입력합니다. 입력할 때 목록에 결과가 강조 표시됩니다.

   ![강조 표시된 검색어](assets/glist-search-highlighted.png)

   >[!NOTE]
   >
   >모든 목록 항목의 모든 열을 검색합니다. 목록이 길면 검색에는 스크롤하여 확인해야 하는 항목이 포함됩니다. 목록이 필터링되면 검색은 현재 표시된 것만 봅니다.
