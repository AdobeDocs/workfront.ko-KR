...
>[!IMPORTANT]
>
>현재 Workfront MCP 서버는 AWS을 사용하는 고객만 사용할 수 있습니다. GCP 또는 Azure을 사용하는 고객은 가까운 시일 내에 이 기능을 사용할 수 있습니다.
...
다른 MCP 호환 AI 에이전트 플랫폼(예: [!DNL Gemini] 또는 [!DNL Microsoft Copilot])을 사용하는 경우 해당 플랫폼의 설명서에 나와 있는 단계에 따라 사용자 지정 MCP 서버를 추가합니다. MCP 서버 URL을 입력하라는 메시지가 표시되면 해당 지역의 URL을 입력합니다.

| 지역 | URL |
| --- | --- |
| 미국 | `https://mcp.workfront.adobe.com/mcp/v1/workfront` |
...
1. **앱 만들기** 단추를 선택합니다.
1. 커넥터에 원하는 이름(예: &quot;Workfront&quot;)을 지정하고 원하는 MCP 서버 URL을 입력합니다.
...
1. 커넥터를 만든 후 로그인 창이 나타납니다. Adobe ID 자격 증명을 사용하여 인증합니다. 둘 이상에 속해 있는 경우 원하는 Workfront 인스턴스를 선택해야 합니다.
   ...
   >참고:
   >
   >Workfront 인스턴스는 해당 인스턴스의 영역에 대한 MCP 서버와 연결해야 합니다. 예를 들어 EU 인스턴스는 EU MCP 서버에 연결해야 합니다.
   >
   >인스턴스를 선택하면 MCP 서버 지역과 호환되지 않는 인스턴스가 회색으로 표시되며 해당 인스턴스에 연결할 수 없습니다.
   >
   >MCP 서버 영역과 호환되지 않는 인스턴스에 연결하려면 해당 영역에 대한 올바른 URL을 사용하여 새 MCP 연결을 설정하십시오.
...
1. **앱 만들기** 단추를 선택합니다.
1. 앱에 원하는 이름(예: &quot;Workfront&quot;)을 지정하고 원하는 MCP 서버 URL을 입력합니다.
...
| 연결하려는 Workfront 인스턴스가 회색으로 표시되고 지역에서 연결할 수 없다는 메시지가 표시됩니다 | MCP 서버는 인스턴스가 아닌 다른 지역(EU 또는 US)에 대해 구성됩니다. | Workfront 인스턴스가 할당된 지역의 URL을 사용하여 MCP 서버를 설정합니다. |
