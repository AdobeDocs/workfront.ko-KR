...
* [커넥터 디렉토리에서 클라우드 데스크탑에 연결](#connect-to-claude-desktop-from-the-connectors-directory)
* [URL을 사용하여 클라우드에 연결](#connect-to-claude-with-a-url)
* [스킬을 사용하여 클라우드 동작 사용자 지정](#customize-claude-behavior-with-skills)
...
1. [!DNL Claude] 열기
1. **사용자 지정**&#x200B;을 클릭합니다.
1. 왼쪽 탐색에서 **커넥터**&#x200B;를 클릭합니다.
1. 커넥터 목록에서 **[!DNL Adobe Workfront]**&#x200B;을(를) 찾습니다.

   표시되지 않으면 이 문서에서 [관리자 필수 구성 요소](#admin-prerequisites)를 참조하십시오.
1. **연결**&#x200B;을 클릭합니다.
1. 메시지가 표시되면 Workfront 인스턴스에 로그인합니다. 프로필 및 Workfront 인스턴스를 선택해야 할 수 있습니다. 선택하는 프로필에 따라 연결되는 작업 영역이 결정됩니다.

   인증이 완료되면 Workfront MCP 서버가 연결됩니다.
...
1. 왼쪽 탐색에서 **커넥터**&#x200B;를 선택합니다.
1. **+** 아이콘을 선택한 다음 **사용자 지정 커넥터 추가**&#x200B;를 선택합니다.
...
Workfront MCP 서버를 ChatGPT와 연결하는 절차는 ChatGPT Desktop을 사용하는지 Codex를 사용하는지, 웹에서 ChatGPT를 사용하는지에 따라 다릅니다.

* [ChatGPT 데스크탑 또는 ChatGPT 코드에 연결](#connect-to-chatgpt-desktop-or-chatgpt-codex)
* [웹에서 ChatGPT에 연결](#connect-to-chatgpt-on-the-web)
* [사용자 지정 GPT로 ChatGPT 동작 사용자 지정](#customize-chatgpt-behavior-with-custom-gpts)

### ChatGPT 데스크탑 또는 ChatGPT 코드에 연결

+++ 를 확장하여 Workfront을 ChatGPT Desktop 또는 ChatGPT Codex에 연결하기 위한 단계별 지침을 봅니다.

1. ChatGPT에서 **설정**&#x200B;을 엽니다.
1. 왼쪽 탐색에서 **플러그인**&#x200B;을 클릭합니다.
1. 창의 오른쪽 상단에 있는 **서버 추가**&#x200B;를 클릭합니다.
1. 서버 이름을 입력합니다.
1. 형식에는 **스트리밍 가능한 HTTP**&#x200B;를 선택하십시오.
1. MCP 서버 URL 설정:
...
1. **저장**&#x200B;을 클릭합니다.
1. 표시되는 목록에서 추가할 MCP 서버에 대해 **인증**&#x200B;을 클릭합니다.
1. Workfront에 로그인.
프로필 및 Workfront 인스턴스를 선택해야 할 수 있습니다. 선택하는 프로필에 따라 연결되는 작업 영역이 결정됩니다.
1. ChatGPT의 MCP 서버 목록에서 새 MCP 서버의 오른쪽에 있는 토글이 계속 켜졌는지 확인합니다.

+++

### 웹에서 ChatGPT에 연결

+++ 를 확장하여 Workfront을 웹에서 ChatGPT에 연결하기 위한 단계별 지침을 봅니다.

1. 자격 증명을 사용하여 [ChatGPT](https://chatgpt.com)에 로그인합니다.
1. 왼쪽 아래에서 이름을 선택한 다음 **설정**&#x200B;을 선택합니다.
1. 왼쪽 탐색에서 **보안 및 로그인**&#x200B;을 선택합니다.
1. https://chatgpt.com/plugins에서 ChatGPT 플러그인 페이지로 이동합니다.
1. Plugins 페이지의 오른쪽 상단에 있는 Plus 아이콘을 클릭합니다.
1. **이름** 필드에 MCP 서버의 이름을 입력하십시오.
1. **연결** 필드에서 **서버 URL**&#x200B;을(를) 선택하고 MCP 서버 URL을 입력합니다.

   ```
   https://mcp.workfront.adobe.com/mcp/v1/workfront
   ```

1. 인증이 **OAuth**(기본적으로 설정됨)로 설정되어 있는지 확인하십시오.
1. 위험 메시지를 읽고 읽은 상태임을 보여 주는 상자를 선택합니다.
1. Click **Create**.
1. 앱이 생성되면 Workfront 로그인 창이 나타납니다. Adobe ID 자격 증명을 사용하여 인증합니다. 프로필 및 Workfront 인스턴스를 선택해야 할 수 있습니다. 선택하는 프로필에 따라 연결되는 작업 영역이 결정됩니다.
...
+++ 를 확장하여 Workfront을 Copilot에 연결하기 위한 단계별 지침을 봅니다.
...
1. 인증을 위해 **OAuth 2.0**&#x200B;을 선택한 다음 **동적 검색**&#x200B;을 선택하십시오.
1. 패널 하단의 **만들기**&#x200B;를 클릭합니다.

   앱이 등록되었습니다.

1. 앱이 등록되면 나타나는 패널에서 **연결되지 않음**&#x200B;을 클릭한 다음 **새 연결 만들기**&#x200B;를 클릭하고 **만들기**&#x200B;를 클릭합니다.
1. **연결되지 않음**, **새 연결 만들기**, **만들기**&#x200B;를 차례로 클릭하여 연결을 설정합니다.
1. 표시되는 로그인 패널에서 Workfront에 로그인하고 사용할 인스턴스를 선택한 다음( 둘 이상의 인스턴스에 대한 액세스 권한이 있는 경우) **계속**&#x200B;을 클릭합니다.
...
## Workfront을 사용자 지정 MCP 솔루션에 연결

사용자 정의 애플리케이션 또는 에이전트를 구축하는 경우 Workfront MCP 서버에 직접 연결할 수 있습니다.

다음과 같은 두 가지 방법으로 연결할 수 있습니다.

* [서비스 간 토큰으로 연결](#connect-with-a-service-to-service-token)
* [OAuth와 연결](#connect-with-oauth)

### 서비스 간 토큰으로 연결

1. Adobe Developer Console을 사용하여 서비스 자격 증명을 만듭니다. 자세한 내용은 [서버 간 인증](https://developer.adobe.com/developer-console/docs/guides/authentication/ServerToServerAuthentication/)을 참조하십시오.
1. 다음 정보를 사용하여 Workfront MCP 서버에 연결합니다.

   * **URL**: `https://mcp.workfront.adobe.com/mcp/v1/workfront`
   * **머리글**:

     * `Authorization: Bearer <access_token>`
     * `wf-url: <your_subdomain>.my.workfront.com`(자격 증명에 둘 이상의 Workfront 인스턴스(예: 미리 보기 및 프로덕션)에 액세스할 수 있는 경우 필요합니다.)
...
* [여러 Workfront 인스턴스에 동시에 연결할 수 있습니까?](#can-i-connect-to-multiple-workfront-instances-at-the-same-time)
* [어느 관리자가 이 기능을 활성화합니까?](#which-administrator-enables-this)
* [Workfront 인스턴스가 Adobe IMS(Identity Management System)에서 활성화되지 않은 경우 Workfront MCP 서버를 사용할 수 있습니까?](#can-i-use-the-workfront-mcp-server-if-my-workfront-instance-isnt-enabled-on-adobe-identity-management-system-ims)
