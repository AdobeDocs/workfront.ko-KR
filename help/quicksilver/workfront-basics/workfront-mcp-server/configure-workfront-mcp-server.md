...
다른 MCP 호환 AI 에이전트 플랫폼(예: [!DNL Gemini] 또는 [!DNL Microsoft Copilot])을 사용하는 경우 해당 플랫폼의 설명서에 나와 있는 단계에 따라 사용자 지정 MCP 서버를 추가합니다. MCP 서버 URL을 입력하라는 메시지가 표시되면 다음을 입력하십시오. `https://mcp.workfront.adobe.com/mcp/v1/workfront`
