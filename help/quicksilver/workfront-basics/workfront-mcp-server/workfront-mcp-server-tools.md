...
| 제목 | 도구 이름 | 기능 | 액션 |
| --- | --- | --- | --- |
| 오브젝트 검색 | `workflow_search_any_object` | 사용하지 않음. 대신 `insights_find_id_by_name` 또는 `insights_find_workfront_data` 사용 | 읽기 |
...
| 필드 이름 확인 | `workflow_resolve_field_names_any_object` | 사용하지 않음. 대신 `insights_search_fields`을(를) 사용합니다. | 읽기 |
