...
| 이해 당사자에게 일괄 승인 알림 | `approvals_send_approval_reminder` | 전체 프로젝트, 폴더, 캠페인 또는 기한 창에서 보류 중인 모든 승인자에게 승인 미리 알림 이메일을 보냅니다. | 쓰기 |
| 승인 템플릿 일괄 업데이트 | `approvals_update_template` | 에셋에 템플릿 적용, 처음부터 또는 기존 승인 플로우에서 새 템플릿 작성, 템플릿 편집, 템플릿 및 에셋 간 대량 작업 수행과 같은 여러 템플릿에 대한 템플릿 업데이트를 수행합니다. | 쓰기 |
| 승인 참가자를 일괄적으로 추가하거나 제거합니다. | `approvals_update_approval_participants` 및 `approvals__submit_update_approval_participants` | 한 번의 작업으로 전체 포트폴리오, 프로그램 또는 프로젝트 범위에서 참가자를 추가, 제거 또는 교체합니다. | 쓰기 |
...
| 작업 영역에서 글로벌 레코드 유형 제거 | `planning_remove_global_record_type_from_ws` | 작업 영역에서 글로벌 레코드 유형의 연결을 해제합니다. 해당 작업 영역에서 모든 레코드를 삭제합니다. | 쓰기 |
...
| 제목 | 도구 이름 | 기능 | 액션 |
| --- | --- | --- | --- |
| 댓글 쿼리 | `comment-stream_query_comments` | 페이지 매김이 있는 개체 ID로 주석을 쿼리합니다. | 읽기 |
| 댓글 가져오기 | `comment-stream_get_comment` | ID로 단일 주석을 가져옵니다. | 읽기 |
| 댓글 수 가져오기 | `comment-stream_get_comments_count` | 개체에 대한 총 최상위 댓글 수를 가져옵니다. | 읽기 |
| 댓글 만들기 | `comment-stream_create_comment` | 개체에 새 주석을 만듭니다. | 쓰기 |
| 답변 만들기 | `comment-stream_create_reply` | 기존 댓글에 대한 회신을 만듭니다. | 쓰기 |
| 댓글 업데이트 | `comment-stream_update_comment` | 기존 댓글 또는 답변을 업데이트합니다. | 쓰기 |
| 댓글 삭제 | `comment-stream_delete_comment` | ID로 댓글을 삭제합니다. | 쓰기 |
| 반응 추가 | `comment-stream_add_reaction` | 댓글에 반응(예: )을 추가합니다. | 쓰기 |
| 반응 제거 | `comment-stream_remove_reaction` | 댓글에서 반응(예: )을 제거합니다. | 쓰기 |
...
| 제목 | 도구 이름 | 기능 | 액션 |
| --- | --- | --- | --- |
| 문서 읽기 | `insights_read_docs` | 조건, 상태, 날짜 또는 필드 경로와 같은 Workfront 플레이북 또는 도메인 설명서를 로드합니다. 데이터를 쿼리하기 전에 필요한 첫 번째 단계입니다. | 읽기 |
| 현재 사용자 가져오기 | `insights_get_current_user` | 이름, ID 및 URL을 포함한 고유한 Workfront ID를 검색합니다. | 읽기 |
| 필드 검색 | `insights_search_fields` | 프로젝트, 작업, 문제, 사용자, 포트폴리오, 팀 등에 대한 사용 가능한 필드(표준 및 사용자 지정)를 검색합니다. | 읽기 |
| 필드 경로 가져오기 | `insights_get_field_paths` | 데이터 쿼리 도구에 필요한 엔티티의 점 표기법 필드 경로를 확인합니다. | 읽기 |
| 이름별 ID 찾기 | `insights_find_id_by_name` | 프로젝트, 작업, 사용자, 포트폴리오 등과 같은 이름별로 Workfront 개체의 ID를 조회합니다. | 읽기 |
