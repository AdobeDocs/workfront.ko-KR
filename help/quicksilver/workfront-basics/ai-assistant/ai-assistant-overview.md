...
* AI 길잡이는 사용자가 연 페이지에 대한 문맥에 따라 달라집니다. 예를 들어 프로젝트 페이지에서 &#39;이 프로젝트 요약&#39;을 AI Assistant에 입력하면 해당 특정 프로젝트에 대한 요약이 반환됩니다.
...
* Workfront Planning AI Assistant에는 Workfront AI Assistant와는 다른 기능이 있습니다.

  Workfront Planning의 AI 도우미에 대한 자세한 내용은 [Adobe Workfront Planning AI 도우미 개요](/help/quicksilver/planning/general/planning-ai-assistant-overview.md)를 참조하십시오.

* AI 어시스턴트는 현재 영어로만 제공됩니다.
...
사용자에게 Workfront에서 유효한 권한이 있는 경우 AI 어시스턴트는 다음 개체 유형과 연결된 데이터를 쿼리할 수 있습니다.
...
Adobe 세대 AI 계약에 서명하지 않은 상태에서 사용자가 AI 길잡이를 사용하려고 하면 다음 메시지가 표시됩니다.

* 사용자: 조직에 대해 AI 어시스턴트가 활성화되지 않았으며 Workfront 관리자에게 연락하여 조직에 대해 AI 어시스턴트를 요청할 수 있다는 메시지가 사용자에게 표시됩니다.
* 관리자: 관리자는 서명된 Adobe 세대 AI 계약이 없다는 안내를 받고 서명을 위해 계약 사본을 보내도록 요청할 수 있습니다.
...
1. Adobe AI 계약 메시지가 나타나면 **계약 검토**&#x200B;를 클릭합니다.
1. Adobe Gen AI 계약에 서명할 조직 내 개인의 이름과 이메일 주소를 입력합니다.

   서명을 위해 계약이 이 개인에게 전송됩니다. 서명되고 반환된 후, Adobe이 계약을 검토하는 경우 조직에 대해 AI 어시스턴트가 활성화됩니다.

   >[!NOTE]
   >
   >* Adobe이 AI 비서를 검토하고 활성화하도록 계약서에 서명하고 반환한 후 영업일 기준으로 1~3일 정도 걸립니다.
   >* 활성화한 후 조직의 인스턴스에서 사용할 수 있는 AI Assistant 옵션이 표시되지 않으면 Workfront 고객 지원에 문의하십시오.
...
| `summarize` | 파일을 업로드하거나 프로젝트를 요약할 때 등의 항목을 요약합니다. |

>[!NOTE]
>
> 모든 영역에서 모든 키워드를 사용할 수 있는 것은 아닙니다.
>
