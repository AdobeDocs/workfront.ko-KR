...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>사용자 정의 양식을 관리하는 오브젝트에 권한 이상 부여</p> <p>추가 액세스 요청에 대한 자세한 내용은 <a href="../../workfront-basics/grant-and-request-access-to-objects/request-access.md" class="MCXref xref">오브젝트에 대한 액세스 요청 </a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
1. 추가된 사용자 정의 양식의 순서를 변경할 객체로 이동한 다음 객체 편집을 시작합니다.
   ...
   **예:** 예를 들어 프로젝트의 사용자 정의 양식을 관리하려면 프로젝트로 이동하여 **자세히** 메뉴 ![](assets/more-icon.png)을 클릭한 다음 을 클릭합니다 **편집** .

1. 다음에서 **사용자 지정 Forms** 프로젝트, 작업 및 문제에 대한 섹션에서 ![](assets/move-icon---dots.png) 아이콘: 사용자 정의 양식 이름 옆에 있습니다. 다른 모든 객체의 경우 **Forms 관리**. 이 옵션은 객체에 하나 이상의 사용자 정의 양식이 첨부된 경우에만 표시됩니다.
1. 양식 드래그 ![](assets/move-icon---dots.png) 를 클릭하여 목록의 새 위치로 이동합니다.
1. 프로젝트, 작업 및 문제 사용자 정의 양식의 경우 **저장**.

   다른 모든 객체의 경우 **관리를 완료했습니다.** > **변경 내용 저장**.
...
1. 사용자 정의 양식을 제거할 객체로 이동한 다음 객체 편집을 시작합니다.
   ...
   예를 들어 프로젝트로 이동하여 **자세히** 메뉴 ![](assets/more-icon.png)을 클릭한 다음 을 클릭합니다 **편집** .

1. 클릭 **사용자 지정 Forms**.
1. 프로젝트, 작업 및 문제 사용자 정의 양식의 경우 **X** 아이콘은 양식 오른쪽에 있으며 객체에서 제거됩니다.

   다른 모든 객체의 경우 **Forms 관리**&#x200B;을(를) 클릭하고 **X** 아이콘은 양식 오른쪽에 있으며 객체에서 제거됩니다.
...
* 서로 다른 두 객체에 동일한 계산된 필드가 있는 경우, 오류를 방지하기 위해 계산이 동일해야 합니다. 여러 양식을 포함하여 사용자 정의 양식에 계산된 필드를 추가하는 방법에 대한 자세한 내용은 [사용자 정의 양식에 계산된 데이터 추가](../../administration-and-setup/customize-workfront/create-manage-custom-forms/add-calculated-data-to-custom-form.md) .
...
1. 목록 개체에서 사용자 정의 양식이 첨부된 개체를 선택한 다음 **편집** 아이콘 ![](assets/edit-icon.png).
1. 클릭 **사용자 지정 Forms**.
...
1. 다음을 클릭합니다. **선택** 드롭다운 메뉴를 클릭하고 선택한 모든 오브젝트에 추가할 추가 양식을 선택합니다.
   ...
   * 추가 양식을 적용하면 양식에 다른 양식과 공통되는 모든 필드가 **공통 필드** 섹션을 편집하면 됩니다.

1. (선택 사항) 모든 오브젝트에 사용자 정의 양식을 추가했지만 아직 오브젝트를 저장하지 않은 경우 사용자 정의 양식이 오브젝트에 표시되는 순서를 변경할 수 있습니다.

   양식 순서 변경에 대한 자세한 내용은 [오브젝트에 첨부된 여러 사용자 정의 양식 순서 바꾸기](#reorder-multiple-custom-forms-attached-to-an-object) 이 문서에서.

1. 클릭 **양식 제거** 개체에서 사용자 정의 양식을 제거합니다.

   개체에서 사용자 정의 양식을 제거하는 방법에 대한 자세한 내용은 [오브젝트에서 사용자 정의 양식 제거](#remove-a-custom-form-from-an-object).

   여러 개체에서 양식을 대량으로 제거할 때에는 다음 사항을 고려하십시오.

   * 양식을 변경한 경우 이를 제거하면 변경 사항이 손실되어 복구할 수 없습니다.
   * 양식을 제거한 후 해당 양식의 모든 필드 **공통 필드** 섹션은 이 섹션에서 제거되며 더 이상 여기에서 편집할 수 없습니다.

1. 클릭 **양식 복원** 양식을 개체를 편집하기 전의 상태로 복원합니다.
1. (선택 사항) 한 번에 하나의 양식을 축소하려면 양식 이름 옆에 있는 축소 화살표를 클릭합니다.

   또는

   클릭 **Forms 접기** 모든 양식을 동시에 축소할 수 있습니다.

1. (선택 사항) 양식 이름 옆에 있는 확장 화살표를 클릭하여 한 번에 하나의 양식을 확장합니다.

   또는

   클릭 **Forms 확장** 을 눌러 모든 양식을 동시에 확장합니다. 
