...
1. (선택 사항) **사람 또는 팀 태그 지정** 영역에서 이 댓글에 포함할 사용자 또는 팀의 이름 또는 전자 메일을 입력하거나 목록에 표시될 때 선택합니다.
...
1. (선택 사항) 이미지를 업데이트에 첨부합니다.
   ...
   * **이미지 추가** 아이콘 ![더하기 기호 아이콘으로 이미지 산 추가](assets/add-image-mountain-with-plus-icon.png)를 클릭하고 컴퓨터에서 이미지를 찾습니다.
   ...
   >* 새 주석을 마우스 오른쪽 단추로 클릭한 다음 **붙여넣기**&#x200B;를 클릭하거나 키보드에서 Ctrl+V(Windows의 경우 ⌘+V)를 눌러 이미지를 붙여넣을 수 있습니다.
