...
title: 작업 로드 밸런서 탐색
description: 작업 로드 밸런서를 사용하여 리소스의 가용성을 이해하고 사용자에게 작업을 할당할 수 있습니다. 이 문서에서는 작업 로드 밸런서에 대한 보기를 업데이트하고 탐색하는 데 사용할 수 있는 아이콘과 설정을 사용하는 방법을 안내합니다.
author: Alina
...
Adobe Workfront의 작업 로드 밸런서를 사용하여 가용성에 따라 사용자에게 작업을 할당합니다. 이 문서에서는 설정 및 옵션을 사용하여 작업 로드 밸런서를 탐색하고 사용자와 관련된 정보를 표시하는 방법에 대해 설명합니다. 여기에 나열된 추가 문서에서는 작업 로드 밸런서를 사용하여 리소스를 관리하고 작업을 수행하는 방법에 대해 설명합니다.

작업 로드 밸런서는 Adobe Workfront의 여러 영역에서 사용할 수 있습니다. 탐색하는 것은 모든 영역에서 유사합니다.

작업 로드 밸런서가 있는 위치에 대한 자세한 내용은 [작업 로드 밸런서를 찾습니다.](https://experienceleague.adobe.com/docs/workfront/using/manage-resources/the-workload-balancer/locate-workload-balancer.html?lang=en).
...
## 작업 로드 밸런서에서 항목을 보기 위한 고려 사항

작업 로드 밸런서를 볼 때 다음 사항을 고려하십시오.

* 작업 로드 밸런서는 지정에 따라 두 개의 분리된 영역에 작업 항목을 표시합니다. 작업 항목 및 사용자는 다음 영역에 표시됩니다.

   * **할당되지 않은 작업**: 할당이 없거나 Job 역할이나 팀에만 할당된 항목
   * **지정된 작업 시간**: 하나 이상의 사용자에게 할당된 항목입니다. 지정된 항목이 지정된 사용자 이름 아래에 표시됩니다.

   >[!NOTE]
   >
   >* 주 할당자인 사용자에게도 할당된 작업 역할 또는 팀에 지정된 항목은 지정되지 않은 작업 영역에 표시되지 않습니다.
   >
   >* 사용자에게 지정된 항목과 작업 역할이 선택된 작업 역할이 미지정 작업 영역에 표시됩니다.
   >* 두 명 이상의 사용자에게 할당된 항목은 지정된 작업 영역의 모든 사용자 이름 아래에 표시됩니다.


* 일정 기간 동안 프로젝트에 작업이 없으면 프로젝트 수준의 막대가 흐리게 표시됩니다.

   ![](assets/wb-break-in-project-timeline-with-no-tasks-highlight-350x80.png)

* 특정 항목을 볼 수 있는 권한이 없으면 이 항목은 로 표시됩니다 **액세스할 수 없는 작업 항목** 또는 **액세스할 수 없는 프로젝트**.

   ![](assets/balancer-inaccessible-items-and-projects-highlighted-350x108.png)

* 작업 항목의 이름이 왼쪽에 표시되고 타임라인이 오른쪽에 표시됩니다.
* 각 작업 항목에 대한 총 계획 시간 수는 작업 항목 이름의 오른쪽 및 작업 항목의 타임라인을 나타내는 막대의 왼쪽에 표시됩니다.
* 각 프로젝트에 대한 총 계획 시간 수가 프로젝트 이름의 오른쪽 및 프로젝트의 타임라인을 나타내는 막대의 왼쪽에 표시됩니다.

   프로젝트에 대한 계획 시간 정보는 작업 로드 밸런서에 나열된 모든 항목에서 계획된 시간의 총 시간이며 프로젝트에서 계획된 시간의 총 시간이 아닙니다.

작업 로드 밸런서에서 정보를 보는 방법에 대한 자세한 내용은 다음 문서를 참조하십시오.

* [작업 로드 밸런서를 찾습니다.](../workload-balancer/locate-workload-balancer.md)
* [작업 로드 밸런서에서 정보 필터링](../workload-balancer/filter-information-workload-balancer.md)
* [링크와 작업 로드 밸런서 공유](../workload-balancer/share-link-for-workload-balancer.md)
* [요약을 사용하여 작업 로드 밸런서에서 작업 항목 업데이트](../workload-balancer/update-items-in-summary-panel-in-workload-balancer.md)

작업 로드 밸런서를 사용하여 리소스를 관리하는 방법에 대한 자세한 내용은 다음 문서를 참조하십시오.

* [작업 로드 밸런서에서 작업 할당 개요](https://experienceleague.adobe.com/docs/workfront/using/manage-resources/the-workload-balancer/assign-work-in-workload-balancer.html?lang=en)
* [작업 로드 밸런서에서 사용자 할당 관리](https://experienceleague.adobe.com/docs/workfront/using/manage-resources/the-workload-balancer/manage-user-allocations-workload-balancer.html?lang=en)


## 리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서를 탐색합니다

작업 로드 밸런서를 탐색하는 것은 사용자가 액세스하는 모든 영역에서 유사합니다.

다음 하위 섹션에서는 여러 프로젝트에 대한 작업 로드 밸런서에서 정보를 보는 방법에 대해 설명합니다.

작업 로드 밸런서에서 많은 설정 및 옵션을 조정하여 사용자에게 가장 적합한 시간대에서 초점을 맞추는 데 필요한 정보를 표시할 수 있습니다.

보기에 적용할 설정을 선택한 후 작업 로드 밸런서는 사용자가 브라우저 또는 장치에서 액세스할 때마다 이러한 설정을 기억합니다.

### 리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서에 액세스

여러 프로젝트에 대한 작업 로드 밸런서를 탐색하려면

1. 을(를) 클릭합니다. **기본 메뉴** 아이콘 ![](assets/main-menu-icon.png) Workfront의 오른쪽 위 모서리에서 을(를) 클릭하고 **리소스**.
1. 클릭 **작업 로드 밸런서** 왼쪽 패널에 표시됩니다.

   ![](assets/nwe-balancer-global.png)

   작업 로드 밸런서는 다음 두 영역에서 현재 주로부터 시작하는 작업 지정 정보를 표시합니다.

   * 다음 **할당되지 않은 작업** 영역에는 다음 작업 항목이 표시됩니다.

      * 필터를 적용하면 역할, 팀 또는 할당되지 않은 작업 항목(작업 및 문제)이 표시됩니다.
할당되지 않은 작업 영역은 기본적으로 작업 항목을 표시하지 않습니다. 필터를 사용하여 이 영역에 적절한 정보를 표시하는 것이 좋습니다.

         필터 사용에 대한 자세한 내용은 [작업 로드 밸런서에서 정보 필터링](../workload-balancer/filter-information-workload-balancer.md).

      * 프로젝트별 그룹 설정을 활성화한 경우에만 표시됩니다. 자세한 내용은 섹션을 참조하십시오 [보기 사용자 지정](#customize-the-view) 참조하십시오.
   * 다음 **지정된 작업 시간** 영역에는 다음 작업 항목이 표시됩니다.

      * 시스템의 모든 활성 사용자가 기본적으로 이 영역에 표시됩니다. 필터를 사용하여 이 영역의 정보 양을 제한하는 것이 좋습니다. 사용자가 항목에 할당되면 작업 항목도 해당 이름 아래에 표시됩니다.

      * 하나 이상의 사용자에게 할당된 작업 및 문제가 사용자의 이름 아래에 표시됩니다.

         지정된 작업 영역의 사용자 이름 아래 작업 항목은 다음 기준으로 이 순서로 정렬됩니다.

         1. 계획 시작 일자(가장 오래된 최초)
         1. 계획 완료 일자(가장 오래된 최초)
         1. 프로젝트별 알파벳(처음 두 기준이 여러 작업 항목에 대해 동일한 경우에만)

            >[!TIP]
            >
            >* &quot;프로젝트 정렬 기준&quot; 설정에서 옵션을 선택하여 프로젝트 정렬을 사용자 지정할 수 있습니다.
            >
            >* 프로젝트는 &quot;프로젝트별 그룹&quot; 설정을 활성화할 때만 표시됩니다.

            > 
            >설정 사용자 지정에 대한 자세한 내용은 섹션을 참조하십시오 [보기 사용자 지정](#customize-the-view) 참조하십시오.


1. (선택 사항) **필터** 아이콘 ![](assets/filter-icon.png) 에서 **지정된 작업 시간** 영역을 선택한 다음 **기본 필터** 에서 **추천** 필터 상자의 영역.

   기본 필터를 적용하면 팀 및 해당 작업 항목에 속하는 사용자가 표시됩니다. 이 필터의 복사본을 편집할 수 있습니다.

   >[!TIP]
   >
   >기본 필터는 리소스 영역에서 작업 로드 밸런서에서만 사용할 수 있습니다.

1. 다음 단계를 계속 수행하여 작업 로드 밸런서를 탐색합니다.

   * [작업 로드 밸런서에서 시간대 선택](#select-a-time-frame-in-the-workload-balancer)
...
1. 에서 작업 로드 밸런서에 액세스 **리소스** 영역에 설명된 대로 [리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서에 액세스](#access-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 참조하십시오.

   작업 로드 밸런서는 현재 주부터 시작되는 작업 지정 정보를 표시합니다.

1. 가로 스크롤을 사용하여 화면의 제한을 초과하는 작업 항목의 타임라인을 볼 수 있습니다.
1. 을(를) 클릭합니다. **뒤로 또는 앞으로** 아이콘 ![](assets/back-and-forward-icons.png) 왼쪽 위 모서리에서 타임라인을 탐색한 다음 을 클릭합니다 **오늘** 를 클릭하여 현재 주로 돌아갑니다.
1. 을(를) 클릭합니다. **시간대 드롭다운 메뉴** 도구 모음에서 표시할 기간의 시작 날짜를 클릭합니다. 기본적으로 달력에서 선택한 첫 번째 주는 탐색한 주입니다.

   ![](assets/calendar-date-picker-wb.png)

1. 다음 옵션에서 작업 로드 밸런서에서 한 번에 표시할 주 수를 선택합니다.
   * 1주
   * 2주
   * 4주. 기본 설정입니다.
   * 6주
...
1. 도구 모음에서 다음 옵션 중 하나를 클릭하여 다른 시간대별로 정보를 표시합니다.
   * **일**: 기본적으로 오늘 날짜부터 시작하여 4주에 대한 일별 정보를 표시합니다.
   * **주**: 4주 동안 주별 정보를 표시합니다.
   * **월**: 3개월 동안 월별로 정보를 표시합니다.

1. 다음 섹션에 설명된 대로 작업 로드 밸런서를 계속 탐색합니다.

### 보기 사용자 지정

1. 에서 작업 로드 밸런서에 액세스 **리소스** 영역에 설명된 대로 [리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서에 액세스](#access-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 참조하십시오.

   작업 항목 이름은 왼쪽에 나열되며 작업 로드 밸런서의 오른쪽에 있는 막대로 표시됩니다. 막대 길이는 작업 항목의 타임라인을 나타냅니다.

1. (선택 사항 및 권장) [지정되지 않음] 및 [지정된 작업 영역]에서 필터를 사용하여 사용자와 관련된 작업 항목이나 사용자만 표시합니다.

   자세한 내용은 [작업 로드 밸런서에서 정보 필터링](../workload-balancer/filter-information-workload-balancer.md).

   기본적으로 파란색 막대는 프로젝트 및 작업의 타임라인을 나타내고 마룬 막대는 문제를 나타냅니다.

   프로젝트와 일치하도록 색상 테마를 선택하면 프로젝트 및 작업의 막대 색상을 변경할 수 있습니다. 자세한 내용은 이 절차를 계속 읽어보십시오.

   지정된 작업 영역의 작업 항목은 다음 기준에 따라 프로젝트별로 이 순서로 정렬됩니다.
   1. 계획 시작 일자(가장 오래된 최초)
   1. 계획 완료 일자(가장 오래된 최초)
   1. 프로젝트별 알파벳(처음 두 기준이 여러 작업 항목에 대해 동일한 경우에만)

1. 을(를) 클릭합니다. **오른쪽 화살표** [지정되지 않음] 또는 [지정된 영역]의 왼쪽에 있는 [지정되지 않음] 영역과 사용자 이름 아래(지정된 영역)의 모든 항목을 확장합니다.

   >[!TIP]
   >
   >작업 항목은 &quot;프로젝트별 그룹&quot; 설정을 활성화할 경우에만 지정되지 않은 영역의 프로젝트 이름 아래에 나열됩니다.


1. 을(를) 클릭합니다. **아래쪽 화살표** [지정되지 않음] 또는 [지정된 영역]의 왼쪽에 있는 [지정되지 않음] 영역과 사용자 이름 아래(지정된 영역)의 모든 항목을 축소합니다.

1. 을(를) 끌어다 놓습니다 **분리선** 왼쪽 패널과 타임라인 영역 사이에 왼쪽 패널의 크기를 조정합니다.

   ![](assets/separation-line-between-left-panel-and-timeline-highlighted-nwe-350x174.png)

1. 을(를) 클릭합니다. **설정** 아이콘 ![](assets/settings-gear-icon.png).

   설정 패널이 오른쪽에 표시됩니다.

   ![](assets/settings-box-options-global-with-color-theme-and-percentage-wb-nwe.png)

   아래 나열된 옵션에서 선택하여 작업 로드 밸런서에서 보는 정보를 업데이트한 다음 **X 아이콘** 설정 상자의 오른쪽 상단에서 클릭하여 닫습니다.

   * **프로젝트별 그룹**: 이 옵션을 선택하면 [지정되지 않음] 및 [지정된 작업 영역]의 항목이 프로젝트별로 그룹화됩니다. 기본적으로 선택됩니다.

   * **문제 발생 시간 포함**: 이 옵션을 선택하면 사용자에게 할당된 문제가 지정된 작업 영역에 사용자 이름 아래에 표시되고 사용자에게 할당되지 않은 문제가 지정되지 않은 작업 영역에 표시됩니다. 문제의 계획 시간은 지정된 작업 영역의 프로젝트 및 사용자에 대한 계획 시간 계산에 포함됩니다.
   * **예상 날짜 표시**: 이 옵션을 선택하면 계획된 타임라인과 함께 작업 항목의 예상 타임라인이 표시됩니다. 다음 사항에 주의하십시오.
      * 프로젝트, 작업 및 문제에 대한 예상 타임라인은 작업, 문제 및 프로젝트 막대 위에 어두운 파란색 선으로 표시됩니다.
      * 아래 설명된 대로 색상 테마를 업데이트하더라도 계획된 타임라인의 외부에 있는 투영된 타임라인은 연한 파란색으로 표시됩니다.
      * 보기에 액세스할 수 없는 항목의 투영된 타임라인은 아래에 선이 있는 밝은 회색으로 표시됩니다.
      * 기한 계획 완료 일자 이전에 태스크 또는 문제가 완료되면 남은 일수의 할당 번호가 처리되어 사용자의 할당에 계산되지 않습니다. 예상 날짜 표시 설정과 할당 표시 아이콘이 모두 사용으로 설정된 경우에만 표시됩니다.

      >[!TIP]
      >
      >작업 항목은 선택한 기간 동안 작업 로드 밸런서에 계획되거나 예상 시간 표시 시간(동시에 둘 다 필요한 것은 아님)이 발생할 때 표시됩니다.

   * **완료된 작업 표시**: 이 기능이 활성화되면 완료된 작업 및 문제가 지정된 작업 영역에 표시됩니다. 기본적으로 활성화되어 있습니다.

      작업이 완료되면 작업 또는 문제 표시줄의 오른쪽 위 모서리에 녹색 확인 표시 아이콘이 표시됩니다. 선택한 프로젝트 기간 동안의 작업 또는 문제가 완료되면 프로젝트에 대해 동일한 아이콘이 표시됩니다.
   * **남은 시간 표시**: 이 기능이 활성화되면 Workfront은 사용자가 일정에 따라 작업할 수 있는 일별 시간과 사용자의 지정된 작업 영역에서 할당된 시간 간의 차이를 표시합니다. 기본적으로 비활성화되며 할당된 시간이 기본적으로 표시됩니다.
   * 에서 **색상 테마 선택** 섹션에서 프로젝트 및 작업 표시줄에 사용할 색상을 선택합니다.

      >[!TIP]
      >
      >색상 테마를 선택하는 설정은 문제 표시줄의 색상에 영향을 주지 않습니다. 문제는 항상 색상 막대에 표시됩니다.

      다음 중에서 선택합니다.
      * **기본값**: 모든 프로젝트 및 해당 작업 항목에 대한 막대가 파란색으로 표시됩니다.
      * **프로젝트**: 각 프로젝트와 연결된 막대 및 해당 작업은 프로젝트 이름에 따라 변경됩니다. 프로젝트에 속하는 모든 작업은 프로젝트 색상과 일치하는 막대에 표시됩니다. 프로젝트 막대는 작업과 구분하기 위해 밝은 음영으로 표시됩니다. 할당을 표시하지 않도록 선택할 때 프로젝트 막대에 프로젝트 아이콘도 포함됩니다.
      * **프로젝트 상태**: 각 프로젝트와 연결된 막대 및 해당 작업 항목은 프로젝트 상태의 색상으로 변경됩니다.

         프로젝트 상태는 프로젝트 그룹과 연결됩니다. 그룹에 그룹별 상태가 없는 경우 작업 항목 막대의 색상은 시스템 수준 프로젝트 상태의 색상입니다. 사용자 정의 상태는 물론 시스템 모두 표시됩니다. 그룹 상태에 대한 자세한 내용은 [그룹 상태 만들기 또는 편집](../../administration-and-setup/manage-groups/manage-group-statuses/create-or-edit-a-group-status.md).
   * 에서 **에 사용자 할당 표시** 섹션에서 다음 중에서 선택합니다.
      * **시간**: 할당된 시간을 시간으로 표시합니다. 기본값입니다.
      * **백분율**: 총 가용 시간의 백분율로 할당된 시간을 표시합니다.
   * 에서 **기본 설정 정렬** 섹션에서 작업 로드 밸런서에서 항목을 정렬할 방법을 선택합니다. 다음 옵션 중에서 선택합니다.
      * **기본 역할별로 사용자 정렬**: 사용자는 지정된 작업 영역에서 기본 역할의 알파벳 순서로 표시됩니다.
      * **사용자를 알파벳순으로 정렬**: 사용자는 지정된 작업 영역에서 이름을 알파벳 순서로 표시합니다.
      * **프로젝트 정렬 기준**: 드롭다운 메뉴에서 프로젝트 필드를 선택하여 지정되지 않음 또는 지정된 작업 영역의 해당 필드에 따라 프로젝트를 사전순으로 정렬합니다.
...
1. 섹션에 설명된 대로 리소스 영역에서 작업 로드 밸런서에 액세스합니다 [리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서에 액세스](#access-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 참조하십시오.
1. 을(를) 클릭합니다. **할당 표시 아이콘** ![](assets/show-allocations-icon-small.png) 작업 품목에 대한 일별 또는 주별 계획 시간을 조회하려면

   작업 항목의 막대 이름을 미지정 및 지정된 작업 영역의 일별 또는 주별 계획 시간 단위로 바꿉니다. 이 설정은 기본적으로 비활성화됩니다.

   초과 할당을 표시하는 일은 빨간색으로 표시됩니다.

   >[!TIP]
   >
   >* 할당 표시 옵션은 프로젝트, 작업, 문제 및 액세스할 수 없는 항목에 대해 표시되는 내용에만 영향을 줍니다. 사용자에 대한 일별 계획 시간은 기본적으로 표시되며 숨길 수 없습니다.
   >* 프로젝트에 대한 일별 계획 시간을 표시하려면 프로젝트별 그룹 설정을 활성화해야 합니다.
   >* 주 단위로 작업 로드 밸런서를 볼 때 표시되는 시간은 주간 계획 시간입니다.



1. (선택 사항) 사용자 라인에서 할당된 시간을 마우스로 가리키면 사용자의 용량 및 할당을 이해할 수 있습니다. 용량은 사용자의 일정에 따라 사용 가능한 것입니다.

   ![](assets/overallocation-vs-capacity-tooltip-wb-nwe.png)

1. (선택 사항) **할당 아이콘 숨기기** ![](assets/show-allocations-icon-small.png) 작업 항목의 막대에 작업 및 문제 이름을 표시합니다.
1. 을(를) 클릭합니다. **추가 메뉴** 아이콘 ![](assets/more-icon.png) 작업 또는 문제 이름의 오른쪽에 있는 아래 옵션 중 하나를 클릭합니다.

   ![](assets/more-menu-right-of-task-350x104.png)

   * **여기에 지정**&#x200B;을 입력한 다음 작업 항목을 지정할 사용자, 역할 또는 팀의 이름을 **사람, 역할 또는 팀 검색** 필드.
      ...
      * Mac에서: CMD를 클릭한 채 작업 또는 문제 표시줄을 클릭합니다.
      ...
      작업 로드 밸런서에서 사용자에게 작업 항목을 지정하는 방법에 대한 자세한 내용은 [작업 로드 밸런서에서 작업 할당 개요](../workload-balancer/assign-work-in-workload-balancer.md).

      >[!TIP]
      >
      >Workfront 또는 그룹 관리자가 사용자 환경에서 위임을 활성화한 경우 지정 탭을 사용하여 사용자를 작업이나 문제에 지정합니다. 작업 위임에 대한 자세한 내용은 [작업 및 문제 위임 관리](../../manage-work/delegate-work/how-to-delegate-work.md).

   * **할당 편집**&#x200B;그런 다음 사용자의 일별 또는 주별 할당을 편집합니다. 사용자 할당 관리에 대한 자세한 내용은 [작업 로드 밸런서에서 사용자 할당 관리](../workload-balancer/manage-user-allocations-workload-balancer.md).

   * **요약 열기**. 요약 패널이 오른쪽에 열리면 지정 필드를 클릭하고 사용자, 역할 또는 팀의 이름을 **사람, 역할 또는 팀 검색** 필드를 사용하여 항목을 할당합니다. 자세한 내용은 섹션을 참조하십시오 [작업 및 문제에 대한 자세한 정보 표시](#display-more-information-about-tasks-and-issues) 참조하십시오.


1. (선택 사항) 작업 항목의 막대 내에 있는 사용자에 대한 일별 또는 주별 할당을 두 번 클릭하여 할당된 시간 수를 편집한 다음 **저장** 아이콘 ![](assets/save-allocations-wb.png) 할당을 저장하려면 또는 **취소** 아이콘  ![](assets/cancel-allocations-wb.png) 조정한 할당을 제거하려면

   >[!TIP]
   >
   >저장 및 취소 아이콘은 작업 끝이나 문제 타임라인 표시줄을 향해 표시됩니다.
   >
   >![](assets/cancel-and-save-icon-on-adjust-allocation-bar-wb-highlighted.png)

   사용자 할당 관리에 대한 자세한 내용은 [작업 로드 밸런서에서 사용자 할당 관리](../workload-balancer/manage-user-allocations-workload-balancer.md).

1. 클릭 **일괄 지정** 작업 항목을 일괄 할당하려면 다음을 수행합니다.

   자세한 내용은 [작업 로드 밸런서를 사용하여 일괄 작업 할당](../workload-balancer/assign-work-in-workload-balancer-in-bulk.md).
1. 항목을 **할당되지 않은 작업** 영역 또는 사용자로부터 해당 영역을 삭제하고 다른 사용자에게 놓아 할당합니다.

   자세한 내용은 [드래그 앤 드롭으로 작업 로드 밸런서에서 작업 할당](../workload-balancer/assign-work-in-workload-balancer-by-drag-and-drop.md).


### 차트에서 할당 보기

일별 또는 주별 숫자로 할당을 보는 대신 차트에서 볼 수 있습니다.

1. 섹션에 설명된 대로 리소스 영역에서 작업 로드 밸런서에 액세스합니다 [리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서에 액세스](#access-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 참조하십시오.
1. 을(를) 클릭합니다. **차트 아이콘** ![](assets/user-allocation-chart-icon.png) 사용자 할당을 차트 형식으로 표시합니다.

   사용자가 너무 많이 할당된 일 수는 빨간색 블록으로 표시되고, 사용자가 제대로 할당되지 않거나 용량이 초과된 일 수는 파란색 블록으로 표시됩니다.

   블록 크기는 할당의 양을 나타냅니다. 상자가 클수록 사용자가 해당 일 또는 주의 작업 항목에 더 많이 할당됩니다.

   ![](assets/user-allocation-chart-350x237.png)



### 작업 및 문제에 대한 자세한 정보 표시

작업 로드 밸런서에서 작업 및 문제에 대한 자세한 정보를 볼 수 있습니다.


1. 섹션에 설명된 대로 리소스 영역에서 작업 로드 밸런서에 액세스합니다 [리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서에 액세스](#access-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 참조하십시오.
1. [요약] 패널에서 자세한 내용을 보려면 다음 중 하나를 수행합니다.

   * 작업 또는 문제 표시줄을 클릭하여 오른쪽의 요약 패널을 엽니다.
   * 을(를) 클릭합니다. **요약 열기** 아이콘 ![](assets/summary-panel-icon.png)을 클릭한 다음 작업 또는 문제 표시줄을 클릭하여 요약 패널을 엽니다.
   * 을(를) 클릭합니다. **자세히** 작업이나 문제 오른쪽에 있는 메뉴를 클릭한 다음 **요약 열기**.

   작업 로드 밸런서의 요약에서 작업 정보 업데이트에 대한 자세한 내용은 다음을 참조하십시오 [요약을 사용하여 작업 로드 밸런서에서 작업 항목 업데이트](../workload-balancer/update-items-in-summary-panel-in-workload-balancer.md).

1. 작업 또는 문제 이름을 마우스로 가리키면 작업에 대한 자세한 정보가 표시됩니다. 작업 또는 문제 위에 다음 정보가 있는 상자가 표시됩니다.
   ...
   * 계획된 시간 수입니다.
   * 작업의 경우 이전 번호
   * 작업의 경우, 작업을 수행할 준비가 되었는지 여부를 나타내는 상자 상단 모서리에 있는 표시기입니다.
...
1. 을(를) 클릭합니다. **전체 화면** 아이콘 ![](assets/full-screen.png) 작업 로드 밸런서를 전체 화면에 표시합니다.

   작업 로드 밸런서는 전체 화면을 차지합니다. 브라우저 창과 탭은 보기에서 제외됩니다.

1. 을(를) 클릭합니다. **전체 화면 종료** 아이콘 ![](assets/exit-full-screen.png) 기본 화면으로 돌아가서 브라우저 탭 내의 작업 로드 밸런서를 확인합니다.


## 팀의 작업 로드 밸런서 탐색

팀의 작업 로드 밸런서를 탐색하는 방법은 여러 프로젝트에 대한 작업 로드 밸런서를 탐색하는 방법과 유사합니다. 자세한 내용은 섹션을 참조하십시오 [여러 프로젝트에 대한 작업 로드 밸런서 탐색](#navigate-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 참조하십시오.

1. 을(를) 클릭합니다. **기본 메뉴** 아이콘 ![](assets/main-menu-icon.png)를 클릭한 다음 **팀**.
기본적으로 홈 팀의 페이지가 표시됩니다.
1. 클릭 **작업 로드 밸런서** 왼쪽 패널에 표시됩니다. 기본적으로 팀의 작업 로드 밸런서가 표시됩니다.

   ![](assets/nwe-balancer-team-350x172.png)

   팀의 작업 로드 밸런서에는 기본적으로 다음 정보가 표시됩니다.

   * 에서 **할당되지 않은 작업** 영역: 팀이나 팀 및 작업 역할에 할당되며 사용자에게 할당되지 않은 작업 항목입니다.
   * 에서 **지정된 작업 시간** 영역: 사용자에게 할당된 작업 항목은 사용자의 이름 아래에 표시됩니다.

1. 에 설명된 대로 팀 작업 로드 밸런서 탐색을 계속합니다. [리소스 영역에서 여러 프로젝트에 대한 작업 로드 밸런서를 탐색합니다](#navigate-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 섹션 을 참조하십시오.

## 단일 프로젝트의 작업 로드 밸런서 탐색
...
1. 클릭 **작업 로드 밸런서** 왼쪽 패널에 표시됩니다. 을(를) 클릭하여 **자세히 표시**, 그런 다음 **작업 로드 밸런서**.
   ...
   프로젝트에 대한 작업 로드 밸런서에는 기본적으로 다음 정보가 표시됩니다.

   * 에서 **할당되지 않은 작업** 영역: 역할 또는 팀에 할당되고 사용자에게 할당되지 않은 프로젝트의 작업 항목입니다.
   * 에서 **지정된 작업 시간** 영역: 하나 이상의 사용자에게 할당된 프로젝트의 작업 항목입니다.

   필터를 사용하여 본인에게 중요한 사용자만 표시하는 것이 좋습니다.

   예를 들어 팀이나 그룹에 속하는 사용자만 표시하는 것이 좋습니다. 자세한 내용은 [작업 로드 밸런서에서 정보 필터링](../workload-balancer/filter-information-workload-balancer.md).

1. (선택 사항) **필터** 아이콘 ![](assets/filter-icon.png) 지정된 작업 영역에서 **이 프로젝트의 작업 항목** 옵션 **추천** 필터 패널의 영역. 이 필터는 기본적으로 선택되어 있지 않습니다.

   이 옵션을 선택하면 선택한 프로젝트의 사용자에게 할당된 항목만 표시됩니다.

   옵션을 선택하지 않으면 항목이 속한 프로젝트에 관계없이 프로젝트 사용자에게 할당된 모든 항목이 표시됩니다.

1. (선택 사항 및 권장) 할당된 작업 영역에 필터를 적용하여 본인에게 중요하지만 프로젝트의 항목에 할당되지 않을 수 있는 사용자를 표시한 다음 **모든 사용자 표시** 아이콘 ![](assets/show-all-users-icon-project-workload-balancer.png).

   모든 사용자를 표시하면 아직 작업이나 프로젝트의 다른 역할에 할당되지 않은 Workfront의 모든 사용자를 표시할 수 있습니다.

   필터를 먼저 적용하여 표시되는 사용자 수를 줄일 수 있습니다.

   예를 들어 팀이나 그룹에 속하는 사용자를 위해 먼저 필터링한 다음 해당 사용자를 모두 표시할 수 있습니다.

   필터를 만드는 방법에 대한 자세한 내용은 [작업 로드 밸런서에서 정보 필터링](../workload-balancer/filter-information-workload-balancer.md).

   >[!NOTE]
   >
   > 모든 사용자 표시 옵션은 프로젝트의 작업 로드 밸런서에 대해서만 사용할 수 있습니다.


1. (선택 사항) **역할 할당 표시** 아이콘 ![](assets/show-role-allocation-icon.png).

   역할 할당 패널이 표시됩니다.

   프로젝트의 작업 역할과 연관된 계획 시간 및 시나리오 계획자의 프로젝트에 연결된 이니셔티브와 연관된 작업 역할에 대한 정보를 볼 수 있습니다.
...
1. 에 설명된 대로 프로젝트의 작업 로드 밸런서를 계속 탐색합니다. [여러 프로젝트에 대한 작업 로드 밸런서 탐색](#navigate-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 섹션 을 참조하십시오.
