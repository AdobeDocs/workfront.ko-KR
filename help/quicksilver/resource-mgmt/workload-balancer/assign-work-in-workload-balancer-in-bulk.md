...
업무 균형자를 사용하여 사용자 및 작업 에이전트에 작업을 할당하는 방법에 대한 일반 정보는 [업무 균형자에서 작업 할당 개요](../../resource-mgmt/workload-balancer/assign-work-in-workload-balancer.md)를 참조하십시오.
...
* 작업 역할 및 사용자를 일괄 할당할 때 다음 작업을 수행할 수 있습니다.
  ...
  * 모든 유효한 조합에서 사용자와 역할 사이에 할당하십시오.
...
* 3개의 다른 프로젝트에서 45개의 작업이 Jackie Simm에게 할당됩니다. Jackie는 조직을 떠나며 이제 다른 사용자에게 Jackie의 작업을 다시 할당해야 합니다. 할당 기능을 사용하여 이러한 작업을 신규 사용자에게 할당할 수 있습니다.
...
선택한 프로젝트의 리소스를 할당하거나 바꿀 수 있습니다.

자원 지정은 다음과 같을 수 있습니다.
...
업무 균형자에서 일괄 할당을 사용하여 리소스를 할당하거나 바꿀 때 다음과 같은 문제가 발생합니다.

* 새 리소스는 선택한 프로젝트 내의 원래 리소스에 현재 지정된 모든 작업 항목에 할당됩니다.
* 새 리소스는 이미 완료로 표시된 작업 항목에 할당되지 않았습니다.
* 사용자 대 사용자 교체의 경우, 첫 번째 사용자와 연관된 역할이 두 번째 사용자의 역할과 일치하지 않으면 두 번째 사용자가 기본 역할에 할당됩니다.

자원을 일괄 지정하려면

1. 위에서 설명한 대로 업무 균형자 일괄 할당 영역에서 작업 항목을 선택하고 **리소스 할당**&#x200B;을 선택합니다.
...
1. **할당**&#x200B;을 클릭합니다.
   ...
   확인란에는 선택한 두 번째 리소스로 최초 할당이 대체된 작업 항목 수가 표시됩니다.
...
1. **할당 해제**&#x200B;를 클릭합니다.\
   확인 메시지가 지정된 사용자가 제거된 작업 항목 수를 표시합니다.
