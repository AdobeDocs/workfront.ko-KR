 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>다음에 대한 액세스 권한 이상:</p> 
    ...
    </ul> <p data-mc-conditions="QuicksilverOrClassic.Quicksilver"><span>필터를 작성하거나 편집할 때 필터, 보기 및 그룹화에 대한 액세스 편집</span> </p> <p><b>메모</b>
...
여전히 액세스 권한이 없는 경우 Workfront 관리자에게 액세스 수준에 추가 제한을 설정하는지 문의하십시오. Workfront 관리자가 액세스 수준을 변경하는 방법에 대한 자세한 내용은 <a href="../../administration-and-setup/add-users/configure-and-grant-access/create-modify-access-levels.md" class="MCXref xref">사용자 정의 액세스 수준 만들기 또는 수정</a>.</p> </td>
  ...
  <tr> 
   ...
   <p>편집하거나 삭제할 필터에 대한 권한 관리</p>
     </p> <p>추가 액세스 요청에 대한 자세한 내용은 <a href="../../workfront-basics/grant-and-request-access-to-objects/request-access.md" class="MCXref xref">오브젝트에 대한 액세스 요청 </a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
* 에서 업무 균형자에 액세스하는 위치에 따라 Workfront에서 이미 정보를 필터링하고 있을 수 있습니다. 사전 적용된 필터에 대한 자세한 내용은 섹션 을 참조하십시오 [업무 균형자에서 사전 적용된 필터](#pre-applied-filters-in-the-workload-balancer) 이 문서에서.
...
* 한 영역의 업무 균형자에서 필터를 만들면 다른 영역에서는 사용할 수 없습니다.
  ...
  업무 균형자 위치에 대한 자세한 내용은 [업무 균형자 찾기](../../resource-mgmt/workload-balancer/locate-workload-balancer.md).
...
* **미할당 작업 영역**: 사용자에게 아직 할당되지 않은 작업 항목입니다.
* **할당된 작업 영역**: 사용자에게 할당된 작업 항목입니다.

  각 영역에 표시되는 항목에 대한 자세한 내용은 [업무 균형자 탐색](../../resource-mgmt/workload-balancer/navigate-the-workload-balancer.md).
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td><b>기본적으로 미할당 작업 영역에 표시되는 항목</b> </td> 
  ...
  <tr> 
   ...
   <td> <p>시스템 기본 필터 시 선택한 프로젝트에서 하나 이상의 작업 항목과 해당 프로젝트의 작업 항목에 할당된 사용자 <b>이 프로젝트의 작업 항목</b> 이(가) 선택되어 있습니다. </p>

<p>시스템 기본 필터일 때 <b>이 프로젝트의 작업 항목</b> 이(가) 선택 해제되면 프로젝트의 할당된 작업 영역에 선택한 프로젝트에서 하나 이상의 항목에 할당된 사용자의 모든 작업 항목이 표시됩니다.  </p> 이 필터는 기본적으로 선택 해제되어 있습니다.

<b>참고</b>
<p>프로젝트의 업무 균형자에서 모든 사용자 표시 옵션을 활성화하면 시스템의 모든 사용자가 표시됩니다. 자세한 내용은 <a href="../workload-balancer/navigate-the-workload-balancer.md" class="MCXref xref">업무 균형자 탐색</a></p>
...
업무 균형자에서 미할당 작업 및 할당된 작업 영역에 대한 필터를 만드는 프로세스는 업무 균형자에 액세스하는 위치에 관계없이 동일합니다. 업무 균형자 찾기에 대한 자세한 내용은 [업무 균형자 찾기](../../resource-mgmt/workload-balancer/locate-workload-balancer.md).

필터를 처음부터 만들거나 미리 정의된 필터 중 하나를 편집할 수 있습니다. 편집할 수 있는 기존 필터에 대한 자세한 내용은 [업무 균형자에서 기존 필터 편집](#edit-an-existing-filter-in-the-workload-balancer) 이 문서의 섹션.

1. 업무 균형자로 이동합니다.

   업무 균형자 액세스에 대한 자세한 내용은 [업무 균형자 탐색](../../resource-mgmt/workload-balancer/navigate-the-workload-balancer.md).

1. 다음을 클릭합니다. **필터** 아이콘 ![](assets/filter-icon.png) 다음 중 하나의 오른쪽 위 모서리 **할당 해제된 작업** 또는 **할당된 작업** 영역.
...
1. (선택 사항 및 조건부) 리소스 영역의 업무 균형자에 액세스하는 경우 미리 정의된 기본 필터가 할당된 작업 영역에 이미 적용되었을 수 있습니다. 기본 필터의 복사본을 편집하고 저장할 수 있습니다.
   ...
   에 액세스하는 경우 [!UICONTROL 업무 균형자] 프로젝트에서 &quot;[!UICONTROL 이 프로젝트의 작업 항목]&quot;필터가 이미 적용되었을 수 있습니다. 이 프로젝트의 사용자에게 할당된 작업 항목만 표시됩니다. 이 필터의 복사본을 복제하고 저장할 수 있습니다.

   기본적으로 [!UICONTROL 업무 균형자] 프로젝트의 에는 프로젝트의 모든 사용자에게 할당된 모든 작업 항목이 표시됩니다.
...
1. 필터를 만들려면 다음을 수행하십시오.
   ...
   1. 첫 번째 드롭다운 메뉴에서 필드 이름을 선택하거나 **필드 찾아보기** 을 눌러 기본적으로 표시되지 않는 필드의 이름을 입력합니다.
   ...
   1. (조건부) 을 클릭한 경우 **필드 찾아보기**&#x200B;에 필드 이름을 입력합니다. **검색** 필드를 선택하고 목록에 표시될 때 선택합니다.
...
1. (선택 사항) **필터 추가** 다른 필터 기준을 추가하려면 4단계의 작업을 반복합니다.

   <!--(NOTE: ensure this stays correct)-->

1. 클릭 **적용** 필터 결과를 저장하지 않고 선택한 업무 균형자 영역에 적용합니다.
...
1. **저장**&#x200B;을 클릭합니다.
   ...
   저장된 필터 적용에 대한 자세한 내용은 섹션을 참조하십시오 [업무 균형자에서 저장된 필터 삭제](#delete-a-saved-filter-in-the-workload-balancer) 이 문서에서.

1. (조건부) 마우스를 위에 놓기 **필터 아이콘** ![](assets/filter-icon.png) 의 오른쪽 위 모서리 **할당 해제된 작업** 또는 **할당된 작업** 이름 또는 현재 적용된 필터 수로 도구 설명을 표시할 영역입니다.
...
1. 기존 필터 위에 마우스를 놓고 **자세히** 메뉴 ![](assets/more-menu.png)을 클릭한 다음 을 클릭합니다 **복제**.

   ![](assets/filter-more-menu-options-wb.png)

   >[!TIP]
   >
   > 필터를 편집하는 동안 **자세히** 필터 편집 상자의 왼쪽 아래 모서리에 있는 메뉴를 클릭한 다음 을 클릭합니다 **복제**.
...
1. (선택 사항) **필터 추가** 중복 필터에 문을 더 추가합니다.
1. 클릭 **저장** 에 중복 필터를 저장하려면 **내 필터** 영역입니다.
...
1. 다음을 클릭합니다. **필터 아이콘** ![](assets/filter-icon.png) 의 오른쪽 위 모서리 **할당 해제됨** 또는 **할당된 작업** 영역.\
   필터 빌더가 오른쪽에 표시됩니다.

1. 편집할 필터를 마우스로 가리킨 다음 **편집** ![](assets/wb-edit-filter-icon.png).
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 다음을 클릭합니다. **삭제** 아이콘 ![](assets/delete.png) 기존 필터 문을 제거합니다.
...
1. 업무 균형자로 이동
1. 다음을 클릭합니다. **필터 아이콘** ![](assets/filter-icon.png) 의 오른쪽 위 모서리 **할당 해제된 작업** 또는 **할당된 작업** 영역.\
   필터 빌더 상자가 오른쪽에 표시됩니다.

1. 필터 위로 마우스를 가져간 다음 **자세히** 메뉴 ![](assets/more-menu.png)을 클릭한 다음 을 클릭합니다 **삭제**.
   ![](assets/filter-more-menu-options-wb.png)

   >[!TIP]
   >
   >필터를 편집하는 동안 **자세히** 필터 편집 상자의 왼쪽 아래 모서리에 있는 메뉴를 클릭한 다음 을 클릭합니다 **삭제**.

1. (선택 사항) **취소** 삭제를 방지하고 필터 목록으로 돌아갑니다.
1. 클릭 **삭제** 삭제 확인.
...
1. 필터 위로 마우스를 가져간 다음 **자세히** 메뉴 ![](assets/more-menu.png)을 클릭한 다음 을 클릭합니다 **공유.**

   ![](assets/filter-more-menu-options-wb.png)

   >[!TIP]
   >
   > 필터를 편집하는 동안 **자세히** 필터 편집 상자의 왼쪽 아래 모서리에 있는 메뉴를 클릭한 다음 을 클릭합니다 **공유**.
...
1. 활성화 **시스템 전체 보기** 설정. 이렇게 하면 Workfront의 모든 사용자에게 필터를 볼 수 있는 권한이 부여됩니다.
   ...
   에서 필터를 공유할 사용자, 팀, 역할, 그룹 또는 회사의 이름을 입력하십시오. **액세스 권한 부여 대상** 필드.

   ![](assets/new-filters-sharing-ui-wb-350x422.png)

1. (선택 사항) 엔티티 이름 옆의 오른쪽 화살표를 클릭하여 필터에 대한 권한을 편집한 다음, **보기** 또는 **관리** 옵션을 선택합니다.
...
1. **공유**&#x200B;를 클릭합니다. 필터는 지정한 엔티티와 공유됩니다.
   ...
   공유한 필터는에 표시됩니다. **나와 공유됨** 필터 상자의 영역입니다.
