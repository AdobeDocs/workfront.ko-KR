...
>[!NOTE]
>
>완료 단추는 Workfront의 모든 영역에서 완료 상태로 표시로 표시됩니다.

## 개요

특정 권한이 있는 사용자는 [!UICONTROL 완료] 단추를 구성하여 시스템의 특정 상태와 연결할 수 있습니다. [!UICONTROL 완료] 단추가 [!UICONTROL Workfront]의 작업에 대해 작동하는 방법에는 두 가지가 있습니다.

* 사용자에게 지정된 홈 팀이 있는 경우 [!DNL Workfront] 관리자 또는 [!UICONTROL 계획] 라이선스가 있는 사용자가 팀 구성원의 특정 상태를 반영하도록 [!UICONTROL 완료] 단추를 구성할 수 있습니다. 이 문서에서 [팀에 대한 [!UICONTROL 완료] 단추 구성](#configure-the-uicontrol-done-button-for-a-team)을 참조하십시오.
* 사용자에게 [!UICONTROL 홈 팀]이 없지만 프로필에 [!UICONTROL 다른 팀]이 있는 경우, Workfront은 사용자와 연결된 모든 팀에서 [!UICONTROL 완료] 단추 설정을 검색합니다. 이 선택은 무작위이며 모든 팀과 연관된 상태가 작업에 사용됩니다.
* 사용자에게 홈 팀이 할당되지 않은 경우 작업에 대한 [!UICONTROL 완료] 단추가 전체 상태로 연결되어 있습니다. There are no configuration options available in this scenario. The [!UICONTROL Done] button automatically defaults to this status.
...
1. 각 작업 항목 유형에 대해 하나 이상의 상태를 선택합니다.
   ...
   >   
   >   
   >  예를 들어 [!UICONTROL 완료] 단추를 [!UICONTROL 진행 중]과(와) 연결하면 상태를 [!UICONTROL 새로 만들기]에서 [!UICONTROL 진행 중]&#x200B;(으)로 변경하는 사용자의 작업 항목이 [!UICONTROL 완료]&#x200B;(으)로 표시됩니다.
   >   
...
1. **[!UICONTROL 조직]** 섹션에서 **[!UICONTROL 홈 팀]** 필드를 선택합니다. 설정을 사용자와 연결할 팀의 이름을 입력하십시오. 목록에서 팀 이름을 클릭합니다.
