...
이 문서에서는 Workfront Planning에서 레코드를 연결할 때 고려해야 하는 고려 사항에 대해 설명합니다.

레코드를 서로 연결하거나 다른 개체에 연결하는 방법에 대한 자세한 내용은 [레코드 연결](/help/quicksilver/planning/records/connect-records.md)을 참조하십시오.

## 전제 조건

Workfront Planning에서 레코드를 연결하려면 먼저 다음을 연결해야 합니다.

* 두 가지 레코드 유형
* 다른 응용 프로그램의 개체가 있는 레코드 종류
...
* 레코드 유형을 연결하면 연결된 레코드 유형이 연결 필드로 표시됩니다. 레코드 유형 테이블 및 해당 레코드 페이지에서 연결된 레코드 유형이 표시됩니다.
...
Workfront에서 레코드를 수동으로 또는 자동으로 연결할 수 있습니다.

### 수동으로 레코드 연결

다음 영역에서 다른 레코드 또는 다른 응용 프로그램의 개체에 레코드를 수동으로 연결할 수 있습니다.
...
레코드 유형을 서로 연결하거나 레코드 유형을 다른 응용 프로그램의 개체 유형에 연결한 후 다음과 같은 방법으로 레코드와 개체를 자동으로 연결할 수 있습니다.

* 자동화 사용

  자동화를 구성하는 Planning 레코드에서 레코드나 Workfront 객체를 생성할 수 있습니다.

  정의한 조건이 충족되면 레코드 또는 개체가 생성되고 자동화를 트리거하는 레코드에 자동으로 연결됩니다.
...
* 요청 양식을 사용하여 레코드 만들기
  ...
  Planning 요청을 실행할 때 레코드를 생성할 수 있습니다. 요청과 레코드는 자동으로 연결됩니다.

  >[!NOTE]
  >
  >원래 요청에서 레코드 연결을 끊을 수 없습니다.
