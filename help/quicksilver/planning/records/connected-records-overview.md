...
Adobe Systems Workfront Planning 레코드를 서로 연결하거나 다른 응용 프로그램의 개체에 연결할 수 있습니다.
...
* 연결 중인 레코드의 조회 필드 값은 수동으로 업데이트할 수 없습니다.
  ...
  조회 필드의 값은 원래 레코드 또는 개체에서 업데이트된 후 자동으로 연결하는 Workfront Planning 레코드를 채웁니다.

* Workfront Planning 및 보기 또는 작업 영역 및 레코드 유형에 대한 상위 권한에 액세스할 수 있는 모든 사용자는 레코드 간 또는 레코드와 다른 애플리케이션의 개체 간의 연결을 볼 수 있습니다. 그들은 연결하려는 응용 프로그램에서의 사용 권한에 관계없이 연결된 레코드와 개체를 볼 수 있습니다.
...
* Workfront Planning의 레코드를 Planning 레코드의 다음 영역에서 Workfront 객체 또는 Experience Manager Assets 객체에 연결할 수 있습니다.
   ...
   * 연결된 레코드의 연결된 레코드 페이지 탭에 있는 레코드의 페이지입니다.
