...
먼저 두 레코드 형식을 서로 연결하거나 다른 응용 프로그램의 개체 형식에 레코드 형식을 연결해야 합니다. 이렇게 하면 연결된 레코드 필드가 만들어집니다. 그런 다음 연결된 레코드 필드에서 레코드를 서로 연결하거나 다른 응용 프로그램의 레코드를 다른 개체에 연결할 수 있습니다.
...
* Adobe Workfront Planning은 다른 응용 프로그램의 객체를 사용하여 레코드를 기록합니다.
   ...
   * Adobe Experience Manager
      ...
      * 자산
        ...
        Workfront Planning에서 다음 유형의 자산을 연결할 수 있습니다.
...
<td> 
   <p> 추가 제품</p> </td> 
   <td> 
   <p> Adobe Workfront 외에도 다음 응용 프로그램의 오브젝트와 레코드를 연결하려면 다음이 있어야 합니다.</p>
...
<tr> 
  <tr> 
   ...
   <p>Workfront 및 AEM Assets에서 연결하려는 오브젝트의 오브젝트 유형에 대한 액세스 권한 이상을 볼 수 있습니다. </p>  
</td> 
  </tr>
  <tr> 
   <td role="rowheader"><p>개체 권한</p></td> 
   <td>   <p>작업 영역 및 레코드 유형에 대한 또는 그 이상의 권한을 기여하여 레코드를 연결합니다. </p>  
...
1. 검색 상자에 레코드 이름을 입력한 다음 목록에 표시될 때 선택합니다
   ...
   * 레코드 유형을 연결할 때 연결된 조회 필드를 추가한 경우 연결된 필드는 연결된 레코드의 정보로 채워집니다.
   ...
   >* Workfront 자동 완성 필드를 조회 필드로 추가할 수 없습니다. 프로덕션 환경에는 프로젝트 소유자 또는 프로젝트 스폰서와 같은 사람 유형 필드가 포함됩니다. <span class="preview">미리 보기 환경에서 사람 유형 필드를 조회 필드로 추가할 수 있습니다.</span>
   >
...
### 테이블 보기 또는 레코드의 세부 정보 영역에서 Workfront Planning 레코드를 Workfront 객체에 연결
...
1. (선택 사항) 적어도 볼 수 있는 권한이 있는 모든 개체를 표시하려면 **모두 보기**&#x200B;를 클릭합니다.
   ...
   ![프로젝트를 선택할 개체 상자 연결](assets/connect-objects-box-to-select-projects.png)
...
### 테이블 보기 또는 레코드의 세부 정보 영역에서 Workfront Planning 레코드를 Adobe Experience Manager 객체에 연결

<!--when we will have more applications to link to from Planning, change the title to something like: Connect Workfront Planning records to objects from other applications-->

>[!IMPORTANT]
>
>Adobe Experience Manager 라이선스가 있어야 하며, Workfront Planning 레코드를 Adobe Experience Manager에 연결하려면 조직의 Workfront 인스턴스가 Adobe 비즈니스 플랫폼 또는 Adobe Admin Console에 온보딩되어야 합니다.
>
...
레코드 종류와 Adobe Experience Manager 개체 간에 연결을 만들면 개별 레코드를 Experience Manager 개체에 연결할 수 있습니다. 연결을 만들 때 Experience Manager에서 연결한 에셋 필드는 연결한 레코드 종류에 자동으로 채워집니다.
...
Experience Manager 개체와 레코드를 연결하려면 다음을 수행하십시오.
...
1. 레코드 유형의 카드를 클릭하여 레코드 유형 페이지를 엽니다.
1. 레코드 종류 페이지의 왼쪽 위 모서리에 있는 **보기** 드롭다운 메뉴에서 **테이블** 보기를 선택하십시오.
...
1. (조건부) 선택한 레코드 종류를 Experience Manager 개체와 연결한 후 레코드의 연결된 필드로 이동하여 필드를 클릭하거나 **연결**&#x200B;을 클릭하여 Experience Manager 개체를 레코드에 추가한 다음 **+** 아이콘을 클릭합니다.

   >[!TIP]
   >
   >  레코드의 세부 정보 페이지에 있는 연결된 개체 필드에서 **+** 아이콘을 클릭하여 자산을 레코드에 연결할 수 있습니다.

   [컨텐츠 권고자] 상자가 표시됩니다.

   ![AEM 레코드 연결용 콘텐츠 관리자 상자](assets/content-advisor-assets-nothing-selected.png)
...
1. **Assets** 탭에서 다음 자산 유형 중 일부를 클릭하여 선택합니다.
...
1. 에셋 또는 콘텐츠 조각 선택을 마치면 **선택**&#x200B;을 클릭합니다. <!-- we might change this to Connect-->
   ...
   * 선택한 Experience Manager 에셋 또는 콘텐츠 조각이 연결된 레코드 필드에 추가됩니다.
   * 연결된 필드(또는 조회 필드)는 Experience Manager 연결 개체의 정보로 채워집니다.

     Experience Manager 에셋 또는 콘텐츠 조각 필드의 기존 정보는 연결된 필드 또는 조회 필드에 자동으로 표시됩니다. <!--verifying of fragments also share lookup fields - not sure from the UI available-->
...
1. **선택하지 않은 필드** 영역 <!--not sure if this is also possible for fragments??-->에서 Experience Manager Assets 개체 필드 추가
   ...
   **선택한 필드** 영역에서 Workfront 개체 필드를 제거합니다.
   ...
   이렇게 하면 레코드에서 연결된 필드가 추가되거나 제거됩니다. 제거된 필드와 관련된 정보는 Adobe Experience Assets에 남아 있습니다.
...
1. 레코드 이름을 클릭합니다.
   ...
   미리보기 페이지가 열립니다.
1. (선택 사항) **새 탭에서 열기** 아이콘 ![새 탭에서 세부 정보 열기](assets/open-details-in-a-new-tab-icon.png)을 클릭하여 새 브라우저 탭에서 레코드 페이지를 엽니다.
1. (선택 사항 및 조건부) 레코드 페이지 헤더의 이동 경로에서 레코드 유형의 이름을 클릭하여 동일한 계층의 다른 레코드 유형에 액세스합니다. 이동 경로에서 계층을 보려면 연결하는 레코드의 레코드 유형에 대한 계층이 있어야 합니다. 자세한 내용은 [작업 영역 계층 만들기](/help/quicksilver/planning/architecture/create-workspace-hierarchies.md)를 참조하십시오.
...
레코드와 개체가 즉시 추가됩니다.
1. (선택 사항) 레코드 또는 개체의 연결된 카드 위에 마우스를 놓은 다음 **레코드 연결 끊기** 아이콘 ![레코드 연결 끊기](assets/disconnect-icon-with-tooltip.png)를 클릭하여 선택한 레코드에서 연결을 끊습니다.
...
1. 레코드 페이지에서 기존 **연결된 레코드 페이지** 탭을 클릭합니다. 먼저 **연결된 레코드 페이지**&#x200B;를 만들어야 합니다.
...
* Workfront 또는 그룹 관리자는 Workfront 개체 유형에 다음 중 하나를 추가해야 합니다.

   * 레이아웃 템플릿의 Workfront 프로젝트, 포트폴리오 및 프로그램에 대한 계획 섹션입니다.

   * Planning은 다음 객체 중 하나에 대한 사용자 정의 양식에 사용자 정의 필드를 연결합니다.
