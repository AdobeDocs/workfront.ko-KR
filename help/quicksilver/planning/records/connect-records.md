...
<tr> 
  ...
  <tr> 
   ...
   <p><span class="preview">연결 중인 레코드에 대한 권한 관리</span></p>
   <p><span class="preview">연결 중인 레코드에 대한 이상의 권한 보기</span></p>
...
Adobe Experience Manager 개체와 레코드를 연결하려면 다음을 수행하십시오.
...
1. (조건부) 선택한 레코드 종류를 AEM 개체와 연결한 후 레코드의 연결된 필드로 이동하여 필드를 클릭하거나 **연결**&#x200B;을 클릭하여 Experience Manager 개체를 레코드에 추가한 다음 **+** 아이콘을 클릭합니다.
...
1. 에셋 또는 콘텐츠 조각 선택을 마치면 **선택**&#x200B;을 클릭합니다. <!-- we might change this to Connect-->
   ...
   * 연결된 필드(또는 조회 필드)는 Experience Manager 연결 개체의 정보로 채워집니다.
     ...
     Experience Manager 에셋 또는 콘텐츠 조각 필드의 기존 정보는 Planning에 자동으로 추가된 연결 또는 조회 필드에 표시됩니다.

     Planning의 GenStudio 작업 영역에서 GenStudio for Performance Marketing 레코드 유형을 AEM 개체와 연결하는 경우 AEM의 캠페인, 지역, 가상 사용자, 제품 및 채널에 대한 GenStudio 정보를 볼 수 있습니다.

     Workfront Planning에서 Adobe GenStudio for Performance Marketing 작업 영역을 보려면 AEM 및 Workfront Planning 라이선스 외에 GenStudio 라이선스가 있어야 합니다.
