...
1. (선택 사항 및 조건부) <span class="preview">레코드 페이지 헤더의 이동 경로에서 레코드 종류의 이름을 클릭하여 동일한 계층의 다른 레코드 종류에 액세스합니다. 이동 경로에서 계층을 보려면 연결하는 레코드의 레코드 유형에 대한 계층이 있어야 합니다. 자세한 내용은 [작업 영역 계층 만들기](/help/quicksilver/planning/architecture/create-workspace-hierarchies.md)를 참조하십시오. </span>
