...
<td> 
   ...
   <ul><li><p>AEM 에셋을 Planning 유형과 연결하기 위한 AEM Assets 및 Workfront 간의 통합 및 Adobe Experience Manager Assets 라이선스.</p>
...
레코드 종류와 Adobe Experience Manager Assets 간에 연결을 만들면 개별 레코드를 Experience Manager 에셋에 연결할 수 있습니다. 연결을 만들 때 Experience Manager에서 연결한 에셋 필드는 연결한 레코드 종류에 자동으로 채워집니다.
...
1. (조건부) 선택한 레코드 종류를 Experience Manager에 연결한 후 레코드의 연결된 필드로 이동하여 필드를 클릭하거나 **연결**&#x200B;을 클릭하여 Experience Manager 자산을 레코드에 추가한 다음 **+** 아이콘을 클릭합니다.
...
1. **Assets** 영역에서 다음 자산 유형 중 일부를 선택하려면 클릭하세요.
...
1. **콘텐츠 조각**&#x200B;을 클릭하여 연결된 레코드 필드에 추가할 콘텐츠 조각을 선택합니다.
...
1. 에셋 또는 콘텐츠 조각 선택을 마치면 **선택**&#x200B;을 클릭합니다. <!-- we might change this to Connect-->
   ...
   * 연결된 필드(또는 조회 필드)는 Experience Manager 연결된 에셋의 정보로 채워집니다.
...
1. (선택 사항) Experience Manager에 연결한 레코드 유형으로 이동하고 연결된 레코드 필드에서 에셋의 이름을 클릭합니다. 에셋의 Experience Manager 세부 정보가 팝업 창에 표시됩니다.
