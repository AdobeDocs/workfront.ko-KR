...
1. (조건부) 관리자가 사용자 정의 분기를 활성화한 경우 Workfront에서 사용자 정의 분기가 구성되는 방식과 관련된 문제를 감지하면 타임라인 보기를 열 때 경고를 받을 수 있습니다.
   ...
   * 분기 날짜 사이에 간격 또는 겹침이 탐지된 경우 이제 사용자 정의 분기를 설정할 수 있으며 편집이 필요할 수 있다는 알림을 받을 수 있습니다.
...
1. 타임라인 보기에서 레코드를 만들거나 해당 날짜를 편집하려면 다음 중 하나를 수행하십시오.
   ...
   * 타임라인에서 위치 및 날짜를 업데이트하려면 레코드를 드래그 앤 드롭하십시오. 레코드 표시줄을 이동하면 시작 및 종료 날짜가 즉시 업데이트됩니다.
   ...
   >[!TIP]
   >
   >분류에 표시된 Workfront 및 Adobe Experience Manager 개체 유형에 대한 타임라인에서는 레코드 크기를 조정하고 드래그 앤 드롭할 수 없습니다.
...
1. (선택 사항) 그룹화할 필드를 최대 3개까지 추가하려면 **조건 추가**&#x200B;를 클릭합니다.
...
1. (선택 사항) 타임라인 보기에서 항목을 드래그 앤 드롭하여 날짜를 조정합니다.
...
1. (조건부 및 선택 사항) Workfront 관리자인 경우 **사용자 정의 영역 사용** 상자에서 **설정으로 이동**&#x200B;을 클릭하여 설정 영역으로 이동하여 사용자 정의 영역을 설정합니다. 사용자 정의 분기를 설정한 후 기본 분기 대신 타임라인 보기에 표시할 수 있습니다. Workfront 관리자가 아닌 경우 관리자에게 조직에 대해 사용자 정의 분기를 활성화하도록 요청할 수 있습니다.
...
1. (조건부 및 선택 사항) 타임라인 보기에 그룹화를 추가한 경우 **그룹화 색상 설정** 섹션에서 그룹화에 대한 색상을 설정하려면 다음 옵션 중 하나를 선택하십시오.
   ...
   * **필드 값**: 그룹화의 색상은 그룹화하는 필드의 색과 일치합니다.
   ...
   >    
   >* 프로덕션 환경에서는 연결된 레코드 또는 해당 조회 필드에 색상을 일치시킬 수 없습니다. <span class="preview">미리 보기 환경에서는 연결된 레코드에 색상을 일치시킬 수 있지만 조회 필드에는 색상을 일치시킬 수 없습니다. </span>
...
1. (조건부) 압축 모드에서 타임라인을 보고 있으면 **보기 전환**&#x200B;을 클릭합니다. **압축** 모드에서 분류가 표시되지 않습니다.
   ...
   >[!TIP]
   >
   >**표준 보기로 전환**&#x200B;에서 **다시 표시 안 함**&#x200B;을 선택합니다. 상자, 보기를 전환하기 전에
   >
