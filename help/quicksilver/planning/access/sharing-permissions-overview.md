...
* 작업 공간 및 레코드 유형을 다른 사용자와 공유할 때 레코드 유형의 권한 수준은 기본적으로 작업 공간과 레코드 유형과 연관된 레코드 및 필드에 자동으로 상속됩니다.
...
작업 공간 및 레코드 유형에 기본적으로 권한을 부여하면 레코드 유형에서 레코드 권한이 상속됩니다.

프로덕션 환경에서 레코드에 대한 권한 수준은 다음과 같습니다.
...
미리보기 환경에서 레코드에 대한 권한 수준은 다음과 같습니다.
...
작업 영역 및 레코드 유형에 대한 권한을 부여할 때 레코드 권한은 항상 상속됩니다.

레코드 유형에서 받은 레코드의 상속된 권한을 제거할 수 있습니다. Workspace 관리자 및 레코드 작성자는 레코드에 대한 관리 권한을 유지합니다.

사용자에게 레코드 종류보다 레코드에 대한 더 낮거나 더 높은 권한을 부여할 수 없습니다.

다음과 같은 시나리오가 있습니다.

| Workspace 및 레코드 유형 권한 | 레코드에 대한 자동 상속된 권한 | 상속된 권한이 꺼져 있을 때(수동으로 부여됨) 가능한 레코드 권한 |
|--------|--------|-------------|
| 관리 | 관리 | 관리, 권한 제거* |
| 참여 | 관리 | 관리, 권한 제거* |
| 보기 | 보기 | 보기, 권한 제거* |
...
>[!NOTE]
>
>*레코드에서 권한을 제거할 때 작업 영역에서 해당 권한을 제거하지 않으면 사용자는 작업 영역과 레코드 유형에 대한 보기 권한을 계속 보유합니다.
