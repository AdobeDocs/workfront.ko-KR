...
각 작업에 대한 자세한 내용과 예를 보려면 [Workfront Planning API 개발자 설명서](https://developer.adobe.com/wf-planning/).
...
기본적으로 Planning API 요청은 목록의 시작 부분부터 500개의 결과를 반환합니다. 결과 수에 대한 기본 제한을 재정의하려면 다음을 사용할 수 있습니다. `limit` 매개 변수를 요청에 추가하고 다른 숫자로 설정하면 최대 2000개의 결과가 나옵니다.

를 추가하여 대용량 데이터 세트에 대해 페이지 번호를 매긴 응답을 사용하는 것이 좋습니다. `offset` 매개 변수를 요청에 추가합니다. 페이지 매김된 응답을 사용하면 반환되어야 하는 첫 번째 결과의 위치를 지정할 수 있습니다.
...
정렬에 대한 자세한 내용은 [API에서 쿼리 결과 정렬](#sorting-query-results-in-the-api) 이 문서에서.
