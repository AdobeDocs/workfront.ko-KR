...
1. <span class="preview">**기본 선택 항목** 필드에서 기본적으로 이 필드에 표시할 선택 항목을 선택합니다. 두 개 이상의 선택을 할 수 있습니다. 사용자는 이 필드를 업데이트할 때 기본 선택 항목을 제거할 수 있습니다.</span>

1. Click **Create**.

   새 다중 선택 필드가 레코드 유형에 열로 추가되고 해당 값을 레코드와 연결할 수 있습니다.

1. (선택 사항) 테이블 보기의 필드 이름 위에 마우스를 놓고 필드 이름 오른쪽에 있는 드롭다운 메뉴를 클릭한 다음 **필드 편집**&#x200B;을 클릭합니다.
1. **값 표시** 설정을 켜서 각 선택 항목에 대한 값을 표시합니다. 값은 Workfront 데이터베이스에 나타나는 각 선택 항목의 이름입니다.

   >[!NOTE]
   >
   >* Workfront은 각 선택 항목에 고유한 값을 할당합니다.
   >
   >* 값은 소문자 형식의 선택 이름과 일치합니다. 여러 단어는 밑줄로 구분됩니다.
   >
   >* 값은 여러 필드 간에 반복될 수 있지만 한 필드에 대해 고유해야 합니다.
   >
   > ![값 표시 전환](assets/show-values-toggle-and-choices-with-values.png)

1. (선택 사항) API 호출 및 기타 통합에서 선택 값을 사용합니다.
...
1. <span class="preview">**기본 선택** 필드에서 기본적으로 이 필드에 표시할 선택을 선택합니다. 한 가지 선택만 할 수 있습니다. 사용자는 이 필드를 업데이트할 때 기본 선택을 제거할 수 있습니다. </span>
...
1. **새 필드** 탭에 다음 정보를 추가하십시오.
   ...
   * **날짜 형식**: 이 필드에 표시할 날짜 형식의 형식입니다. <!--update this casing - submitted bug for it-->
      ...
      * **시간 포함**: 타임스탬프를 포함하려면 이 옵션을 선택하십시오. 이 옵션은 기본적으로 선택되지 않습니다. 필드를 저장한 후에는 시간을 포함할 수 없습니다.
         ...
         * **24시간**: 예: 18:00
         * **12시간**: 예: 오후 6:00
      * **모든 공동 작업자에게 동일한 시간대를 사용**&#x200B;한 다음 **시간대** <!--(************** logged bug to change this to "Show the same time zone to all users" ****************)-->을(를) 선택하십시오. 사용자의 시간대에 관계없이 필드를 보는 모든 사람에게 동일한 타임스탬프를 표시하려면 선택하십시오. 선택한 시간대가 이 필드의 열 헤더에 추가됩니다.
...
사람 필드 유형을 사용하여 레코드에 사용자를 추가할 수 있습니다. 이는 자동 완성 필드이며 Workfront 인스턴스에 이미 존재하는 사용자 <!--, roles, or teams-->만 추가할 수 있습니다.

>[!TIP]
>
>* 사람 필드에 사용자 이름을 추가하면 해당 사용자의 기본 작업 역할과 이메일도 필드에 표시됩니다. 사용자 이메일을 보려면 액세스 수준의 사용자 개체에 대해 연락처 정보 보기 설정을 활성화해야 합니다.
>
>* Adobe Admin Console에 추가된 사용자만 사람 필드에 표시됩니다. Workfront 사용자는 사람 필드에 추가하려고 할 때 표시되지 않습니다.
>
...
1. <span class="preview">**기본 사용자** 또는 **기본 사용자** 필드에서 기본적으로 이 필드에 표시할 사용자를 선택합니다. </span>

   <span class="preview">**여러 값 허용** 설정을 선택한 경우 두 명 이상의 사용자를 선택할 수 있습니다. </span>

   <span class="preview">사용자는 이 필드를 업데이트할 때 기본 사용자를 제거할 수 있습니다.</span>

   >[!TIP]
   >
   ><span class="preview">이 설정에서 기본 사용자로 선택된 사용자를 비활성화하면 사용자의 이름이 기본값으로 유지됩니다. 필드 값에서 비활성화된 사용자를 제거하지 않으며 해당 사용자가 비활성화되었음을 다른 사용자에게 알리지 않습니다.</span>
...
레코드 ID 필드 유형을 사용하여 각 레코드에 대한 시스템에서 생성한 읽기 전용 식별자를 표시할 수 있습니다.

레코드 ID 필드가 만들어지면 영숫자 값으로 자동 채워집니다. 공식 필드 및 API 통합에서 사용하여 각 레코드를 고유하게 식별할 수 있습니다.

레코드 ID 필드의 값은 수동으로 수정할 수 없습니다.

1. 이 문서의 [처음부터 필드 만들기](#create-fields-from-scratch) 섹션에 설명된 대로 필드 만들기를 시작한 다음 **레코드 ID** 필드 형식을 선택합니다.

   ![레코드 ID 필드 형식](assets/record-id-field-type.png)
...
1. Click **Create**.
   ...
   새 레코드 ID 유형 필드가 레코드 유형에 열로 추가되고 해당 값은 영숫자 값으로 미리 채워집니다.
