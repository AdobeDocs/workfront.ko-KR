...
1. 필드를 클릭한 다음 양식의 오른쪽 패널에 있는 컨트롤을 사용하여 해당 크기나 다음 정보 중 하나를 정의합니다.
   ...
   * **논리 추가**: 필드가 표시되거나 숨겨지기 위해 충족되어야 하는 조건을 정의합니다.
   ...
   각 필드의 필드 유형은 양식에서 필드를 선택한 후 오른쪽 패널 상단에 표시됩니다. 요청 양식에 있는 필드 유형의 이름이 테이블 보기의 이름과 다릅니다. 다음 표에서는 테이블 보기의 필드 유형 이름과 레코드 유형의 요청 양식에 있는 동일한 필드 이름의 차이점에 대해 설명합니다.

   | Workfront Planning 필드 유형 | 요청 양식 필드 유형 |
   |-------------------------------|-------------------------|
   | 한 줄 텍스트 | 한 줄 텍스트 |
   | 단락 | 서식 포함 텍스트 |
   | 통화, 숫자, 백분율 | 한 줄 텍스트 |
   ...
   >[!NOTE]
   >
   >   필드의 형식은 그대로 유지됩니다. 예를 들어 통화 및 백분율이 한 줄 텍스트 필드로 표시되더라도 필드의 정보는 통화 및 백분율 값으로 표시됩니다.
