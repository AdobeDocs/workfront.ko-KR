...
## 요청 양식에 승인 추가에 대한 고려 사항

* 요청 양식에 한 명 또는 여러 명의 승인자를 추가할 수 있습니다. 사용자만 승인자로 추가할 수 있습니다.
...
* 요청 양식에 여러 승인자를 추가하는 경우 Workfront Planning에서 레코드가 생성되기 전에 모든 승인자가 요청을 수락해야 합니다.
...
1. 시작 만들기에 설명된 [대로 레코드 종류에 대한 요청 양식을 만들고 Adobe Systems Workfront Planning](/help/quicksilver/planning/requests/create-request-form.md)에서 요청 양식을 관리합니다.
1. 구성을&#x200B;**클릭합니다**.

   **구성** 영역이 표시됩니다.

   ![구성 탭](assets/configuration-tab.png)
1. **승인자** 필드에서 드롭다운 아이콘을 클릭하고 목록에서 하나 이상의 이름을 선택합니다

   또는

   시작 승인자의 이름을 입력한 후 목록에 표시되면 선택합니다.
   ...
   >
   >
   >* 요청 양식에 한 명 또는 여러 명의 승인자를 추가할 수 있습니다.
   >
   >* 두 명 이상의 승인자를 추가하는 경우 Workfront Planning에서 레코드를 생성하기 전에 모든 승인자가 요청 권한을 승인해야 합니다.
   >
   >* 한 명 이상의 승인자가 요청을 거부하면 요청이 거부되고 레코드가 생성되지 않습니다. 요청은 Workfront의 요청 영역에 있는 제출됨 섹션의 계획 탭에 남아 있습니다.
   >
   >* 모든 승인자는 요청 또는 거부 전에 결정을 내려야 합니다.


1. (선택 사항) 이전에 요청 양식을 공유한 적이 없는 경우 Publish **클릭합니다**.
