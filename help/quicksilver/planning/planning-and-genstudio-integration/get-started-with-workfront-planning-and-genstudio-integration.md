...
* GenStudio for Performance Marketing에서 캠페인, 제품, 가상 사용자, 지역 및 활성화를 수정하고 Workfront Planning에서 동일한 정보를 실시간으로 업데이트합니다.
...
* 캠페인, 제품, 가상 사용자, 지역 및 채널에 대한 GenStudio 정보를 Planning의 GenStudio 작업 영역에서 이러한 레코드 유형에 연결할 때 AEM Assets 및 컨텐츠 조각과 동기화합니다.
...
<td> 
   ...
   <td> 
   <p>다음은 조직에 Workfront Planning을 사용하는 Workfront 인스턴스가 두 개 이상 있는 경우에 대한 시나리오입니다.</p>
   <ul><li>Adobe GenStudio for Performance Marketing을 구매할 때 회사에 Workfront 인스턴스가 여러 개 있는 경우 모든 Workfront 인스턴스에서 GenStudio 작업 영역을 볼 수 있습니다.</li>
   <li>원래 인스턴스가 Workfront에 이미 통합된 후 회사에서 Adobe GenStudio for Performance Marketing 인스턴스를 더 추가하는 경우 GenStudio 작업 영역은 원래 Workfront 인스턴스에서만 볼 수 있습니다. Workfront의 추가 인스턴스를 Adobe GenStudio에 연결하는 방법에 대한 자세한 내용은 계정 담당자에게 문의하십시오. </li></ul>    
</td> 
  </tr>
   </tbody> 
</table>
...
Planning에서 GenStudio 작업 영역의 레코드 유형에 대한 Planning 필드를 생성할 수도 있습니다.
...
### GenStudio 작업 영역 레코드와 AEM 개체 간의 메타데이터 동기화

AEM 개체 유형(에셋 및 콘텐츠 조각)이 GenStudio 작업 공간에서 GenStudio 레코드 유형에 연결되면 GenStudio 레코드와 AEM 개체 간에 다음 정보가 동기화됩니다.

* AEM 에셋 및 콘텐츠 조각 정보는 레코드 유형이 연결될 때 추가된 조회 필드에 GenStudio 작업 공간에 표시됩니다.
* GenStudio 레코드 정보는 AEM의 Campaign 탭에 있는 AEM 에셋 또는 메타데이터 탭에 있는 콘텐츠 조각에 표시됩니다.
