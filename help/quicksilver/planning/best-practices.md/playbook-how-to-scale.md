---
title: '첫 번째 우승을 지속 가능한 모멘텀으로 전환: 관리형 스케일링을 위한 플레이북'
description: 첫 번째 성공적인 구현 후 Adobe Workfront Planning을 롤아웃하는 방법을 알아봅니다.
...
그러나 이것은 채택의 가장 취약한 단계이기도 합니다. 엄격한 거버넌스를 향해 너무 빨리 움직이면 팀이 스프레드시트로 돌아갈 위험이 있습니다. 너무 느슨하게 움직이면 해결하려고 했던 무질서한 상황을 재현할 수 있습니다.

이 플레이북에서는 먼저 가시성을 우선시하고 지역 혁신을 가능하게 하며 기업 표준이 조기에 부과되지 않고 실제 운영 성공에서 나올 수 있도록 하는 조정 엔진으로서 Workfront Planning을 사용하는 방법에 대한 균형 잡힌 접근 방식에 대해 설명합니다.

다음은 경험에서 얻은 학습과 무엇을 시청해야 하는지에 대한 몇 가지 팁입니다.

## &#x200B;1. 성공 트랩

때로는 Workfront Planning 채택의 가장 위험한 단계가 첫 번째 성공적인 사용 사례 또는 개념 증명 직후 인 것처럼 느껴질 수 있습니다. 열의는 높지만, 기술적 부채와 행정적 무질서에 대한 두려움은 두 가지 똑같이 피해를 주는 반응으로 이어질 수 있다:

* **거버넌스 초과**: 새 팀이 스프레드시트로 되돌아갈 수 있도록 시스템을 단단히 잠그십시오.

* **제로 거버넌스**: 모든 팀이 고유한 필드 및 레코드 형식을 만들 수 있도록 하여 레거시 환경에서 발견되는 분산된 메타데이터를 다시 만듭니다.

## &#x200B;2. 핵심 철학: 조정 엔진으로서의 Workfront 계획

팀이 다른 것이 되지 않도록 하는 대신, 조정할 수 있도록 이러한 차이점이 표시되는 위치로 Workfront Planning을 지정합니다.

이러한 노력을 지원하려면 다음 사항을 고려하십시오.

* **채택 속도 관리**: 기존 환경을 최적화하기 전에 새 도구로 확장하는 것은 신중해야 합니다. 먼저 단순화를 선택하는 것은 매우 효율적인 토대를 제공하는 반면, 이를 통해 조정할 준비가 된 팀의 가치 실현 시간을 지연시킬 수 있습니다. 이러한 변화를 주도하는 가장 효과적인 방법은 가시성이 1단계임을 인식하는 것입니다. 프레젠테이션 및 스프레드시트의 무질서한 확산에서 벗어나 전사적으로 준비된 공유 툴을 향한 추진력이 장기적인 목표를 궁극적으로 여는 것입니다.

  먼저 정리해야 하는 권한 대신, 진행 중인 유지 관리에 더 적은 양의 리소스를 할당하고 긴급한 비즈니스 요구 사항을 해결하는 데 더 많은 리소스를 할당하는 것이 좋습니다.

  예를 들어, 분류법을 &quot;정리하는&quot; 데에만 1년을 보내는 것은 증분 가치를 거의 산출하지 않습니다. 그러나 팀 간의 가시성을 제공하면 이해 당사자가 필요로 하는 전환 가치를 제공하는 동시에 시간이 지남에 따라 환경을 관리하는 데 필요한 통합 데이터 구조를 제공할 수 있습니다.

  통합 엔터프라이즈 달력 또는 통합 Go-To 시장 로드맵을 통해 팀 간의 가시성을 확보할 수 있습니다.

* **실제 상황을 파악**: 독립 팀은 자체 프로세스를 자연스럽게 개발하며, 이 프로세스는 종종 사일로된 스프레드시트에 숨겨져 있습니다. 이러한 프로세스를 공유 환경에서 볼 수 있도록 함으로써 이러한 프로세스를 최종적으로 해결하고 개선할 수 있는 한 곳에 배치할 수 있습니다.

  >[!IMPORTANT]
  >
  >Workfront Planning으로 전환해도 문제가 발생하지 않습니다. 이미 존재하는 계획을 조명합니다.


* **진행 상태를 확인합니다**: 팀이 자신의 필드를 요청할 때 &quot;무분별하게&quot; 진행되지 않습니다. 입양의 표시라고 생각하십시오. 즉, 팀은 Workfront Planning을 작업 공간으로 봅니다.

* **부채를 관리하십시오. 숨기지 마십시오**: 나중에 발산되는 분류법을 정리하는 데 필요한 노력에 대해 염려하는 것은 당연합니다. 그러나 엄격한 표준을 너무 일찍 강제하는 대신 종종 팀은 프로세스와 부채가 숨겨져 있는 스프레드시트로 되돌아갈 수 있습니다. 팀이 현재 분류로 Workfront Planning을 시작할 수 있도록 허용하면 해당 부채를 가시적이고 통제되는 환경으로 이동하는 것입니다. 따라서 최종 조정은 하나의 압도적인 마이그레이션 프로젝트가 아닌 반복적인 작업이 됩니다.

## &#x200B;3. 가이드 자율 거버넌스 모델

도로 및 로컬 놀이터(또는 관리되는 기본값 및 템플릿)의 레인을 정의하는 동시에 팀이 그 안에서 고유한 경로를 유연하게 선택할 수 있도록 합니다.

가이드 자율 거버넌스 모델의 다음 구성 요소를 고려하십시오.

* [글로벌 레인](#the-global-lanes)
* [로컬 플레이그라운드](#the-local-playgrounds-or-the-spokes)

자세한 내용은 다음을 참조하십시오.

* [전략적인 홈 구축: 30일 런치패드](/help/quicksilver/planning/best-practices.md/30-day-launchpad.md)
* [성공 설계: 캠페인 계층 구조 모델링](/help/quicksilver/planning/best-practices.md/model-campaign-hierarchy.md)
...
* **규칙**: 팀은 여기서 혁신할 수 있습니다. 지역 분야를 3개 팀 이상이 채택하면 글로벌 레인으로 승격할 수 있다.

## &#x200B;4. 거버넌스의 역설을 고려하십시오. 팀을 먼저 고려하고 표준을 따릅니다.

Workfront Planning을 확장하는 데 있어 일반적인 문제는 엔터프라이즈 거버넌스 또는 팀의 운영 정렬과 같이 가장 먼저 고려할 사항을 결정하는 것입니다.

이를 수행하는 가장 효과적인 방법은 엔터프라이즈 가치가 양방향 거리에 구축되어 있음을 인식하는 것입니다.

* **팀에는 관련성이 필요합니다**: 엔터프라이즈는 해당 팀이 적극적으로 실행할 때만 가치를 실현합니다. 따라서 거버넌스는 알려진 운영 요구 사항을 지원하는 구조를 제공하여 팀을 지원해야 합니다.

* **기업에 가시성이 필요합니다**: 데이터를 집계할 수 있을 만큼 깨끗할 때만 리더가 정보에 입각한 결정을 내릴 수 있습니다. 따라서 팀은 포트폴리오 가시성에 필요한 최소한의 실행 가능한 메타데이터를 제공하여 기업을 지원해야 합니다.

관리형 확장의 목표는 가시성을 제공할 수 있을 만큼 표준화하여 이 두 요구의 교차점을 찾는 것이지만 팀 실행을 지연시킬 정도로 크지는 않습니다.

거버넌스 모델을 구축하는 다음 단계를 고려하십시오.

* [우선 순위 조정: 데이터와 시각화 비교](#align-priorities-data-vs-visualization)
* [서비스 주도 관찰 단계](#the-service-led-observation-phase)
* [소급 조화](#retroactive-harmonization)

### 우선 순위 조정: 데이터와 시각화 비교

크기를 조정할 때 값의 정의가 성향 간에 서로 다르다는 것을 인지합니다.

* **관리자 또는 제품 소유자**&#x200B;가 통합 분류 및 분류를 평가합니다. 이 솔루션의 목표는 장기적인 확장성을 지원하는 깔끔한 데이터 아키텍처입니다.

* **관련자 또는 리더**&#x200B;는 글로벌 달력 또는 포트폴리오 타임라인에서 캡처할 수 있는 시각화 및 insight을 평가합니다. 그들의 목표는 데이터를 실행 가능하게 만드는 번개 순간을 식별하는 것입니다.

>[!NOTE]
>
>전략은 관리자가 요구하는 데이터 표준에 대한 팀의 준수에 대한 인센티브로 이해관계자 시각화의 필요성을 이용하는 것이다. 리더쉽이 요구하는 달력을 전달하여 통합 분류법을 얻을 수 있습니다.
...
관리자는 다음을 수행하여 이 교환을 용이하게 할 수 있습니다.

* **표준화보다 작업 우선 순위 지정**: 팀들이 전역 정의가 없어 중단되는 것보다 분산된 작업 공간에서 적극적으로 계획을 수립하도록 하는 것이 좋습니다. 이 활동은 건강한 실제 표준을 구축하는 데 필요한 원시 데이터입니다.

* **가시성 최소값 확인**: 리더쉽과 협력하여 엔터프라이즈 보고에 대해 정리해야 하는 3~5개 필드(예: `Strategic Alignment`, `Start Date`, `Budget`)를 확인합니다. 가시성 최소 필드에만 집행 에너지를 집중하십시오.

### 서비스로서의 거버넌스

이미 알려진 요구 사항이 팀 간에 발생하면 기업은 이러한 패턴을 글로벌 서비스로 통합할 수 있습니다. 이는 서비스로서의 거버넌스입니다.

서비스로 거버넌스를 달성하려면 다음 작업을 수행하십시오.

* **성공적인 패턴을 관찰하십시오**: 팀에서 이미 구축하여 채택한 성공적인 분류법을 확인하십시오.

* **공동 핸드셰이크에 도달**: 팀 챔피언을 모아 로컬 성공을 공유 엔터프라이즈 표준으로 구체화합니다.

* **서비스로 배포**: 제한이 아닌 작업을 수행하는 사람들의 보고 및 팀 간 정렬을 간소화하는 방법으로 새 전역 레인을 롤아웃합니다.

>[!IMPORTANT]
>
>거버넌스는 운영상의 성공에 대한 대응이 되어야 하며, 이를 위한 전제 조건이 되어서는 안 된다는 점을 명심하십시오.

## &#x200B;5. 필드 관리를 위한 크기 조절 역학

### 패턴 기반 필드 성장 모델

이러한 철학을 적용하려면 데이터 구조에 대한 사려깊은 접근이 필요하다. 무질서한 거버넌스를 방지하려면 모든 개별 요청에 대해 글로벌 필드를 구축해야 한다는 충동에 저항하십시오.

대신 다음 필드 성숙도 경로를 사용하여 실제 사용 시 엔터프라이즈 표준을 안내하도록 하십시오.

* **수준 1: 로컬 실험**: 팀 A가 작업 영역에 사용자 지정 필드를 만듭니다.
...
필드가 더 이상 관련이 없어 사용 중단해야 하는 시점에 도달합니다.

Workfront Planning에는 현재 필드에 대한 기본 아카이브 기능이 없으므로 로컬 필드를 중단하려면 UI를 어수선하지 않고 내역 데이터를 보존하기 위해 의도적인 소프트 중단 프로세스가 필요합니다.

필드를 중단하려면:

1. 데이터 마이그레이션을 수행합니다. 테이블 보기(또는 Fusion)를 사용하여 로컬 그림자 필드의 값을 새 전역 레인 필드로 대량 복사합니다. 이 이동 중에 데이터의 유효성을 검사하고 정리해야 합니다.

2. 사용 중지를 위한 필드 이름 바꾸기: 로컬 필드의 이름을 `[DEPRECATED]` 또는 `z_` 같은 접두사로 바꿉니다(예: `z_Language (Old)`). 이는 필드를 필드 선택기의 맨 아래로 밀고 사용자에게 더 이상 신뢰할 수 있는 소스가 되지 않아야 한다는 신호를 보냅니다.

3. 모든 레코드 양식에서 더 이상 사용되지 않는 필드를 제거합니다. 가장 중요한 단계입니다. 이렇게 하면 필요한 경우 기존 테이블 보기 또는 보고서에 이전 데이터가 계속 표시되도록 하면서 새 데이터를 입력할 수 없습니다.

4. 마이그레이션 도중 데이터가 누락되지 않도록 30~60일 동안 더 이상 사용되지 않는 필드(접두사가 추가되고 양식에서 제거됨)를 유지하여 종료 기간을 시작합니다. 이 기간이 지난 후 데이터가 전역 레인에 완전히 조정된 경우 작업 영역에서 로컬 필드를 삭제할 수 있습니다.

## &#x200B;6. Workfront 표류를 방지하십시오.

Planning이 복잡해지지 않도록 하려면 다음을 수행합니다.

* **추상화 계층 이해**: Planning의 모든 필드는 전략적 질문에 답해야 합니다. 필드가 전술적 추적에만 사용되는 경우(예: 필드가 &quot;이 증명이 승인되었습니까?&quot;와 같은 질문에 답하는 경우) Workfront에 보관하십시오.

* **먼저 통합을 달성하십시오**: 팀이 새 메타데이터 필드를 원하는 경우 먼저 전역 분류법을 확인하도록 초대하십시오. 이를 위해서는 팀장에게 글로벌 분류 작업 영역에 대한 읽기 전용 액세스 권한을 부여해야 합니다(섹션 7 참조). 전술적 요구 사항을 기존 전략 필드에 매핑하면 불필요한 중복을 방지하고 보고 무결성을 유지할 수 있습니다.

## &#x200B;7. 읽기 전용 액세스 가시성 모델

팀이 전역 분류 작업 영역에 읽기 전용 액세스를 허용하여 자체 워크플로우에 적용할 수 있는 개념을 확인함으로써 사일로 작업의 노이즈 없이 사일로 느낌을 해결할 수 있습니다.

다음 사항을 고려하십시오.

* **문제**: 팀(스포크)은 자신의 레코드만 볼 수 있으므로 격리된 느낌이 듭니다.

* **수정 사항**: 공유 레코드 형식에 대해 기본으로 지정된 작업 영역에 대해 팀에 읽기 전용 액세스 권한을 부여합니다.

  자세한 내용은 [작업 영역 공유](/help/quicksilver/planning/access/share-workspaces.md)를 참조하십시오.

* **결과**: 팀은 영감을 얻고 맞춤화하기 위한 광범위한 엔터프라이즈 컨텍스트를 볼 수 있지만 로컬 작업 영역은 깔끔하고 특정 작업에 집중되어 있습니다.

## &#x200B;8. 워크샵을 통한 성장 관리

Workfront Planning을 확장하는 것은 기술적인 문제만큼이나 문화적인 과제입니다. 타깃팅된 워크샵을 사용하여 거버넌스 격차를 해소하십시오.

다음은 제공할 수 있는 워크샵에 대한 아이디어입니다.

* [&quot;Necessary dies&quot; 검색 워크숍](#the-necessary-mess-discovery-workshop)
* [&quot;전략적 가시성&quot; 정렬 세션](#the-strategic-visibility-alignment-session)
* [&quot;Spoke-to-global&quot; 쇼케이스](#the-spoke-to-global-showcase)
* [&quot;지속적인 지원&quot; 운영 시간](#the-ongoing-support-office-hours)

### &quot;Necessary dies&quot; 검색 워크숍

* **대상**: 지역 마케팅 리드 및 운영 챔피언.

* **목표**: 현재 격리된 실제 또는 실제 조각화된 운영 데이터를 문서화합니다.
...
* **대상**: 높은 수준의 마케팅 관련자(예: 리더십 역할을 가진 사람)입니다.

* **목표**: 단순화부터 첫 번째 불안을 개선합니다.

* **메시지**: &quot;시작하려면 완벽한 분류가 필요하지 않습니다. 완벽한 분류법을 작성하는 환경으로 Workfront Planning을 사용하고 있습니다.&quot;

* **결과**: Workfront이 현재 상태에 있는 동안 조정 엔진으로 Workfront Planning을 진행할 수 있도록 승인합니다.

### &quot;Spoke-to-global&quot; 쇼케이스

* **대상**: Workfront Planning을 탐색하는 새 팀.

* **목표**: 고립된 느낌을 줄이십시오.

* **메시지**: &quot;팀 A의 로컬 작업이 지정된 기본 Workspace을 자동으로 피드하는 방법을 확인하십시오. 또한 작업에 대해 동일한 가시성을 가질 수 있습니다.&quot;

* **결과**: 로컬 독립성을 잃지 않고 연결할 수 있는 이점을 확인할 수 있는 새 부서의 옵트인

### &quot;지속적인 지원&quot; 운영 시간

* **대상**: 모든 Workfront Planning 사용자(현재 및 향후)입니다.
...
## &#x200B;9. 규모에 대한 스태핑: 역할 및 책임

관리되는 크기 조절 모델이 성공하려면 도구 구성 그 이상이 필요합니다. 이를 위해서는 글로벌 및 Spoke 팀 전체에 걸쳐 명확한 역할 분배가 필요합니다.

아래 하위 섹션에서 크기 조정 모델을 관리하는 데 필요한 주요 플레이어에 대한 아이디어를 찾을 수 있습니다.

### 엔터프라이즈 설계자(Central Center of Excellence 또는 Marketing Operations)

* **포커스**: 엔터프라이즈 무결성, 시스템 성능 및 통합 데이터 분류법.
...
* **포커스**: 전략적 정렬, OKR 가시성 및 포트폴리오 시각화(예: 전역 일정 조정)
...
성공적인 구현의 모든 학습은 추가 구현에 사용할 수 있는 체크리스트를 생성해야 합니다.

다음은 체크리스트에 포함되어야 하는 항목의 예입니다.

* [ ] **챔피언 식별**: 이 새 팀의 프로세스 소유자 또는 챔피언은 누구입니까?

* [ ] **로컬 델타 정의**: 이 팀에는 현재 Global 표준에서 제공하지 않는 2~3개의 필드가 필요합니까?

* [ ] **전역 레인에 매핑**: 미래 팀의 요구 사항의 80%를 충족할 수 있는 기존 전역 필드는 무엇입니까?
