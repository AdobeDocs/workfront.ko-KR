...
레코드 유형이 작업 영역 내에서 연결되면 이러한 연결을 구성하는 계층 구조를 만들 수 있습니다.

계층은 레코드 및 객체 유형을 상위-하위 관계로 구성하며 각각 최대 4개의 객체 유형을 포함할 수 있습니다. 하나의 작업 영역에서 최대 5개의 계층을 생성할 수 있습니다.
...
1. **개체 추가**&#x200B;를 클릭하고 드롭다운 메뉴에서 개체 유형을 선택합니다. 이 유형이 계층의 첫 번째 객체 유형이 됩니다. <!--logged bug to correct to "Add object type"-->
   ...
   첫 번째 객체 유형은 Planning 레코드 유형만 될 수 있습니다.
...
1. (조건부) 연결 필드가 여러 개일 경우 목록에서 하나를 선택합니다.
...
1. (조건부) 연결된 필드가 없으면 **연결 만들기**&#x200B;를 클릭하고 새 연결을 추가한 다음 **저장**&#x200B;을 클릭합니다.
...
1. (조건부) 연결된 필드를 만들 때 **연결된 레코드 형식에 해당 필드 만들기**&#x200B;를 선택하지 않은 경우 오류가 발생하여 먼저 <!--check back on these steps; this is supposed to be seamless, but now you have to abandon creating a hierarchy to do this-->을(를) 수행해야 합니다.

   1. **새 계층 구조** 상자에서 **취소**&#x200B;를 클릭합니다.
   1. 작업 영역 이름의 왼쪽에 있는 뒤로 화살표를 클릭한 다음, 상위로 선택할 레코드 종류의 카드를 클릭합니다.
   1. 위 단계에서 선택한 레코드 종류의 테이블 보기를 연 다음 하위 항목으로 사용할 개체 형식이 있는 연결 필드로 이동하여 열 머리글 위로 마우스를 가져간 다음 **편집** 필드를 클릭합니다.
   1. **연결된 레코드 종류의 해당 필드 만들기** 설정을 켜고 **저장**&#x200B;을 클릭하세요.
   1. 작업 영역의 **설정** 영역으로 돌아가서 **새 계층**&#x200B;을 다시 클릭한 다음 단계에 따라 계층을 만듭니다.
...
1. (선택 사항) 계층에서 마지막 레코드 유형의 이름을 클릭한 다음 해당 레코드 유형의 보기에서 레코드의 이름을 클릭합니다. 레코드의 세부 정보 페이지가 열립니다. 레코드 페이지 상단의 레코드 이동 경로에서 만든 계층 구조를 찾습니다.
