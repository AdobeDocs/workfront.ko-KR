...
1. 예를 들어 다음 정보를 추가합니다.
   ...
   * **여러 레코드 허용**: 이 옵션을 선택한 상태로 두면 연결된 레코드 유형 필드(제품 정보)가 원본 레코드(캠페인)에 표시될 때 사용자가 여러 레코드를 선택할 수 있습니다. 이 경우 사용자는 여러 제품을 선택하여 하나의 캠페인에 연결할 수 있습니다.
...
1. (선택 사항) **레코드 모양** 섹션에서 **제목** 전환을 선택 해제하여 연결된 필드에 레코드의 썸네일 이미지만 표시합니다. 활성화되면 레코드에 썸네일과 제목이 모두 표시됩니다. 토글 기능은 기본적으로 활성화되어 있습니다.
...
1. 예를 들어 다음 정보를 추가합니다.
   ...
   * 
      * **여러 레코드 허용**: 연결된 프로젝트 형식 필드(프로젝트 정보)가 원래 레코드(캠페인)에 표시되는 경우 이 옵션을 선택한 상태로 두면 사용자가 여러 프로젝트를 선택할 수 있습니다.
