...
* 계층에 다음 객체 유형을 추가할 수 없습니다.
   * 연결 가능한 레코드 유형 또는 글로벌 레코드 유형으로 설정된 경우에도 다른 작업 영역의 레코드 유형입니다.
   * 기타 모든 Workfront 개체.
   * AEM Assets
* 계층은 Planning 레코드 유형과 Workfront 객체 유형을 동시에 포함할 수 있습니다.
...
* 연결된 필드에 대해 **연결된 레코드 형식에 해당 필드 만들기** 설정을 켜야 합니다.

  연결된 레코드 유형에 해당 필드를 만들지 않는 연결이 있는 레코드 유형도 계층의 일부가 될 수 있지만 계층 설정 중에 새 연결을 만들 때는 항상 연결된 레코드 유형에 해당 필드를 만들어야 합니다.
* 다음은 계층 설정에 대한 규칙입니다.
...
예를 들어 계층을 만들고 전술, 프로그램, 프로젝트 순으로 캠페인을 연결하는 경우 계층에 연결된 유형의 레코드로 이동하면 레코드가 배치된 위치를 계층 구조에서 볼 수 있습니다.
