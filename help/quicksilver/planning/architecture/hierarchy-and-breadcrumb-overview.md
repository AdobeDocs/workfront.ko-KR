...
* 작업 영역 계층 구조에서 다음 객체 유형만 연결할 수 있습니다.
   ...
   * Workfront 프로젝트. Workfront 프로젝트는 다른 레코드 유형의 상위 항목으로 추가할 수 없습니다. 항상 계층의 마지막 하위 항목입니다.
* 계층에 다음 객체 유형을 추가할 수 없습니다.
   * 연결 가능한 레코드 유형 또는 글로벌 레코드 유형으로 설정된 경우에도 다른 작업 영역의 레코드 유형입니다. 계층을 작성 중인 작업 영역에 글로벌 레코드 유형이 추가된 경우에만 계층에 글로벌 레코드 유형을 추가할 수 있습니다.
   * 기타 모든 Workfront 개체.
   * AEM Assets
...
* 계층에 포함할 레코드 및 개체에 대해 연결된 필드에 대해 **연결된 레코드 형식에 해당 필드 만들기** 설정이 켜져 있어야 합니다.
* 다음은 계층 설정에 대한 규칙입니다.
