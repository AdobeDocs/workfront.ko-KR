...
title: 제출된 요청 보기
description: 귀하 또는 다른 사람이 제출한 요청 또는 제출하지 않고 초안으로 저장된 요청을 볼 수 있는 Adobe Workfront 영역에 대해 알아보십시오.
author: Alina
feature: Requests
topic: Collaboration
role: User
...
귀하 또는 다른 사용자가 제출한 요청이나 귀하가 시작했지만 제출하지 않은 요청을 볼 수 있습니다. 완료되지 않은 요청은 초안으로 저장됩니다.
...
* 레거시 환경을 사용할 때 Workfront 요청
* 새로운 경험을 사용할 때 Workfront 및 Planning 요청이 표시됩니다.

  >[!NOTE]
  >
  >* 자신의 초안 요청만 볼 수 있습니다.
  >* 새 요청 환경에서 제출된 요청 및 초안이 동일한 목록에 있습니다.
  >* 레거시 환경에서 생성된 초안은 새 요청 환경에 표시되지 않습니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>모든 Adobe 워크플로 또는 Adobe 워크플로 패키지</p> 
...
Adobe Workfront Planning 요청으로 생성된 객체에 대한 링크를 포함하는 요청 영역에서 제출된 요청을 볼 수 있습니다.

>[!NOTE]
>
>생성된 객체에 대한 링크는 요청 자체가 레코드를 생성한 경우 Planning 요청에 대해서만 새 요청 환경에서 사용할 수 있습니다.
>
>Workfront 요청이 프로젝트 또는 다른 오브젝트로 전환되는 경우 전환된 해당 오브젝트에 대한 링크를 새 요청 경험의 요청 목록에서 사용할 수 없습니다.
...
1. (선택 사항) **필터**&#x200B;를 클릭하고 표시할 요청에 대한 조건을 추가하기 시작합니다.
   ...
   필터에 대한 자세한 내용은 [요청 영역에서 보기 만들기 및 관리](/help/quicksilver/manage-work/requests/create-requests/create-views-for-requests-list.md)를 참조하십시오.

1. (선택 사항) 보기를 업데이트하거나 다른 사용자와 공유하려면 **보기** 드롭다운 메뉴를 클릭합니다.

   자세한 내용은 [요청 영역에서 보기 만들기 또는 편집](/help/quicksilver/manage-work/requests/create-requests/create-views-for-requests-list.md)을 참조하세요.
1. (선택 사항) **상태** 열을 확인하여 요청 상태를 확인합니다. 새 요청 경험에서 사용할 수 있는 상태는 다음과 같습니다.

   * **초안**: 이 요청은 아직 제출되지 않았습니다.
   * **검토 대기 중**: (Planning만 해당) 이 요청에는 승인자가 있으며, 승인자 중 요청을 연 사람이 없습니다.
   * **검토 중**: (Planning만 해당) 이 요청에는 승인자가 있으며 한 명 이상의 승인자가 요청을 열었지만 결정되지 않았습니다.
   * **거부됨**: (Planning에만 해당) 이 요청에는 승인자가 있으며 거부되었습니다. 이 요청은 레코드를 만들지 않습니다.
   * **진행 중**:
...
1. 제출된 모든 요청을 보려면 왼쪽 패널에서 **제출됨**&#x200B;을 클릭하십시오.

   최대 2,000개의 요청을 볼 수 있으며 여러 페이지에 표시될 수 있습니다.

   >[!TIP]
   >
   >제출된 요청 목록의 열은 사용자 정의할 수 없습니다.

   ![전송된 요청 새 목록](assets/nwe-submitted-requests-new-list-350x57.png)
...
1. 기본적으로 다음 열이 표시됩니다.
   ...
   작업이나 프로젝트로 변환할 때 문제가 유지되지 않으면 문제의 이름이 흐리게 표시되고 더 이상 클릭할 수 없습니다. 문제 변환에 대한 자세한 내용은 <a href="../../../manage-work/issues/convert-issues/convert-issues.md" class="MCXref xref">Adobe Workfront의 문제 변환 개요</a>를 참조하십시오. </p> </td>
   </tr> 
      <tr> 
         <td role="rowheader">전환 대상:</td> 
         <td> <p>요청이 변환된 작업 또는 프로젝트일 수 있는 해결 개체의 이름입니다. </p> <p>작업 또는 프로젝트의 이름을 클릭하여 엽니다. </p> <p>요청이 변환되지 않은 경우 이 필드는 비어 있습니다. </p> </td> 
      </tr> 
      <tr> 
         <td role="rowheader">경로</td> 
         <td>요청이 원래 제출된 요청 대기열, 항목 그룹 및 대기열 항목의 이름입니다. </td> 
      </tr> 
      <tr> 
         <td role="rowheader">상태</td> 
         <td>요청 또는 해결 개체(작업 또는 프로젝트)의 현재 상태입니다.</td> 
      </tr> 
      <tr> 
         <td role="rowheader">입력 일자</td> 
         <td>요청이 제출된 날짜 또는 요청이 변환되었을 때 삭제된 경우 해결 개체가 만들어진 날짜입니다. </td> 
      </tr> 
      <tr> 
