...
>[!NOTE]
>
>시간을 분 단위로 저장할 때 증가분이 반올림되는 방식으로 인해 기존 실제 시간 필드에 부정확한 시간이 표시될 수 있으므로 가능할 때마다 실제 시간 필드를 사용하는 것이 좋습니다. 또한 보고서의 차트에 이전 실제 시간이 올바르게 표시되지 않습니다.
> 
>기존 실제 시간을 사용하는 모든 사용자 정의 공식이 실제 시간으로 마이그레이션되었습니다. 기존 실제 시간은 더 이상 계산 및 공식에 사용할 수 없습니다.
...
실제 시간과 레거시 실제 시간의 차이에 대한 자세한 내용은 이 문서의 [실제 시간과 레거시 실제 시간](#actual-hours-vs-legacy-actual-hours) 섹션을 참조하십시오.

>[!NOTE]
>
>모든 보고서에서 실제 시간 필드를 사용하는 것이 좋습니다. 보고서의 차트에 기존 실제 근로시간이 올바르게 표시되지 않습니다.
> 
>필드를 바꿀 때 기존 실제 시간은 값을 분 단위로 저장하는 반면 실제 시간은 소수점 이하 자릿수를 사용하여 값을 시간 단위로 저장합니다.
...
API 호출의 이러한 필드에 액세스할 때 변환 시간을 분 단위로 고려해야 합니다.<!-- or in calculated custom fields or columns.-->

프로젝트, 작업 또는 문제에 대해 기록된 실제 시간이 현재 Workfront 데이터베이스에 분으로 저장되어 있으며 해당 valuefield는 `actualWorkRequired`입니다.

2025년 10월 이후 API 버전 21의 실제 시간은 데이터베이스의 다음 필드 및 단위에 저장됩니다.
...
>[!NOTE]
>
>기존 실제 시간을 사용하는 모든 사용자 정의 공식이 실제 시간으로 마이그레이션되었습니다. 계산 및 수식에서 이전 실제 시간 또는 `actualWorkRequired`을(를) 더 이상 사용할 수 없습니다.
