...
이 시나리오에서는 원래 문제가 작업, 프로젝트 또는 문제의 해결 가능한 개체가 됩니다.
...
* Workfront 또는 그룹 관리자가 원래 문제 유지 및 해결 방법을 작업에 연결하고, 원래 문제 유지 및 해결 방법을 선택한 프로젝트에 연결하고, 전환 중에 이러한 설정을 변경할 수 있도록 허용 을 선택하지 않은 경우 문제를 작업 또는 프로젝트로 전환할 때 이러한 설정을 변경할 수 없습니다.\
  ![프로젝트 환경 설정 문제 영역](assets/qs-setup-project-preferences-issues-area-some-boxes-unselected-350x217.png)

* Workfront 또는 그룹 관리자가 원래 문제 유지 및 해결 방법을 작업에 연결하고, 원래 문제 유지 및 해결 방법을 프로젝트에 연결하고 선택 또는 선택 취소한 경우 및 전환 중에 이러한 설정을 변경할 수 있도록 허용 을 선택한 경우 문제를 작업 또는 프로젝트로 전환할 때 이러한 설정을 변경할 수 있습니다.\
...
해결 중 오브젝트의 상태가 변경될 때 해결 가능한 문제 상태를 자동으로 업데이트 옵션을 선택했는지 여부에 관계없이, 해결 중 오브젝트(프로젝트 또는 작업)에 대한 기본 상태가 변경될 때마다 해결 가능한 오브젝트(문제)의 상태가 그에 따라 변경됩니다. 이러한 변경을 트리거하기 위해 기본 상태만 이미 매핑되어 있습니다.

문제가 작업의 해결 객체로 설정된 경우 다음과 같은 기본 작업 상태가 기본 문제 상태에서 다음과 같은 변경 사항을 트리거합니다.
...
문제가 프로젝트의 해결 가능한 객체로 설정된 경우 다음의 기본 프로젝트 상태는 기본 문제 상태에서 다음의 변경 사항을 트리거합니다. 일부 프로젝트 상태는 문제 상태의 변경을 트리거하지 않습니다. 문제가 프로젝트가 다음 상태 중 하나로 전환되기 전의 상태로 유지됩니다.
...
>[!NOTE]
>
>문제 상태가 [닫힘]이 되면(작업 또는 프로젝트 닫힘 결과) 작업 또는 프로젝트를 닫은 후 상태가 변경되는 항목에 관계없이 문제가 [닫힘] 상태로 유지됩니다.

#### 해결 중 오브젝트의 사용자 지정 상태를 해결 중 오브젝트의 사용자 지정 상태와 동기화 {#synchronize-the-custom-status-of-the-resolving-object-with-the-custom-status-of-the-resolvable-object}

작업 또는 프로젝트의 상태를 사용자 정의 상태로 변경하면 다음 2가지 조건이 충족되는 경우에만 문제 상태가 사용자 정의 문제 상태로 변경됩니다.

* 해결 중 오브젝트 변경 사항 상태 옵션을 선택하면 해결 가능한 문제 자동 업데이트 상태가 표시됩니다. 자세한 내용은 [해결 가능한 개체를 처리하도록 Adobe Workfront 설정](#set-up-adobe-workfront-to-handle-resolvable-objects)을 참조하십시오.
...
* 프로젝트의 타임라인이 다시 계산되는 경우.

문제가 다른 문제로 해결되면 문제 중 하나가 업데이트될 때 완료율이 업데이트됩니다.

## 작업 또는 프로젝트에서 해결 가능한 개체 찾기

>[!NOTE]
>
>이 섹션의 단계를 수행하려면 적어도 작업 및 프로젝트에 대한 보기 액세스 권한과 보려는 해결 가능한 객체가 포함된 작업 또는 프로젝트에 대한 보기 권한이 있어야 합니다.

해결 중 오브젝트의 위치는 작업 및 프로젝트에 대해 동일합니다.

1. 문제를 전환하여 만든 프로젝트 또는 작업으로 이동합니다.
1. 페이지 왼쪽에서 **작업 세부 정보** 또는 **프로젝트 세부 정보** 탭을 선택합니다.
1. **개요** 섹션 아래쪽에서 작업 또는 프로젝트의 해결 가능한 개체가 표시되는 **이 해결 중** 필드를 찾습니다.

   ![필드 확인](assets/this-resolves-field.png)
...
![개체를 확인하는 중](assets/resolving-icon.png)
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td><strong>해결 가능한 항목이 있습니다</strong>: 프로젝트 또는 작업에 해결 가능한 문제가 있으면 True 값을 표시하고 그렇지 않으면 False 값을 표시합니다.</td> 
   <td>✓</td> 
   <td>✓</td> 
   <td>✓</td> 
  </tr> 
  <tr> 
   <td><strong>원래 문제 이름, 원래 문제 입력 날짜, 작성자 이름</strong>: 텍스트 모드 사용자 지정 보기에서 원래 문제의 이름, 입력 날짜 및 문제를 만든 사용자의 이름을 표시합니다.<br>자세한 내용은 <a href="../../../reports-and-dashboards/reports/custom-view-filter-grouping-samples/view-display-original-issue-info-task-project-list.md" class="MCXref xref">보기: 작업 또는 프로젝트 목록에 원본 문제 정보 표시</a>를 참조하십시오.<br></td> 
   <td> </td> 
   <td> ✓</td> 
   <td> ✓</td> 
  </tr> 
  <tr> 
   <td> <p><strong>해결 가능한 개체:</strong> 프로젝트, 작업 보고서 또는 목록에 대한 텍스트 모드 사용자 지정 보기에 모든 해결 가능한 개체 목록을 표시합니다.</p> <p>자세한 내용은 <a href="../../../reports-and-dashboards/reports/custom-view-filter-grouping-samples/view-resolvable-objects-task-project-report.md" class="MCXref xref">보기: 작업 또는 프로젝트 보고서의 해결 가능한 개체</a>를 참조하십시오.</p> </td> 
   <td> </td> 
   <td>✓</td> 
   <td> ✓</td> 
  </tr> 
