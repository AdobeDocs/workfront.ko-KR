...
1. 문제가 포함된 프로젝트, 작업 또는 반복을 연 다음 왼쪽 패널에서 **문제**&#x200B;를 클릭합니다.
   ...
   >[!NOTE]
   >
   >문제 섹션 링크 옆에 있는 숫자는 섹션에 나열된 미해결(해결되지 않은) 문제의 수를 나타냅니다.


1. 열거나 닫은 문제를 표시하려면 **필터** 드롭다운 메뉴에서 아래 나열된 필터를 클릭하십시오.

   >[!TIP]
   >
   >필터 목록은 시스템 또는 그룹 관리자가 표시하도록 선택한 항목에 따라 다릅니다.
   >
   >문제 목록에 필터를 적용하지 않으면 상태에 관계없이 모든 문제가 표시됩니다.
