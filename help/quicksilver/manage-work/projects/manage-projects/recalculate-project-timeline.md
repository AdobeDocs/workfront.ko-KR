...
description: 타임라인을 다시 계산하면 프로젝트 관련 여러 요소가 프로젝트 타임라인에 미치는 영향을 확인할 수 있습니다. 프로젝트의 타임라인은 계획 및 예상 날짜를 나타냅니다.
author: Alina
...
타임라인을 다시 계산하면 프로젝트 관련 여러 요소가 프로젝트 타임라인에 미치는 영향을 확인할 수 있습니다. 프로젝트의 타임라인은 계획 및 예상 날짜를 나타냅니다.

일정, 인력 휴가 및 프로젝트 범위를 벗어나는 기타 항목을 변경하는 것은 프로젝트 타임라인에 즉시 영향을 주지 않습니다. 프로젝트 타임라인은 타임라인이 다시 계산될 때 영향을 받습니다. 다시 계산될 때까지 외부 영향을 프로젝트에 적용하지 않습니다.

이 문서에서는 타임라인 재계산이 수행되는 방법을 설명합니다.
...
-->
프로젝트 작업에 포함된 사용자에 대한 특별한 액세스 없이 자동 타임라인 재계산이 수행됩니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>프로젝트에 대한 액세스 편집</p> <p>시스템 관리자가 시스템의 모든 프로젝트에 대한 타임라인을 재계산할 수 있습니다</p> <p><b>메모</b>
...
기본적으로 프로젝트 타임라인은 프로젝트 범위가 변경되거나 매일 밤 자동으로 다시 계산됩니다. Workfront 관리자는 설치 프로그램의 프로젝트 환경 설정 영역에서 타임라인 설정을 관리하여 매일 밤 또는 모든 범위 변경 시 타임라인을 자동으로 계산할지 여부를 결정합니다. 자세한 내용은 [프로젝트에 대한 타임라인 재계산 구성](../../../administration-and-setup/set-up-workfront/configure-system-defaults/configure-timeline-recalculations-projects.md).

>[!NOTE]
>
>프로젝트의 타임라인이 15년을 초과하는 경우 해당 프로젝트에 대해 자동 다시 계산이 비활성화됩니다. 15년 이상 프로젝트의 업데이트 유형 수동만 선택할 수 있습니다. 프로젝트의 날짜를 15년 미만으로 변경하는 경우 자동으로 계산되기 전에 타임라인을 수동으로 다시 계산해야 합니다.

* [프로젝트 타임라인의 자동 재계산](#automatic-recalculation-of-project-timelines)
* [프로젝트 타임라인의 자동 재계산을 트리거하는 작업](#actions-that-trigger-an-automatic-recalculation-of-project-timelines)

### 프로젝트 타임라인의 자동 재계산 {#automatic-recalculation-of-project-timelines}

Adobe Workfront은 다음 조건이 모두 충족되는 프로젝트에서만 매일 타임라인을 재계산합니다.

* 현재 상태 있음
* 프로젝트의 업데이트 유형이 자동 또는 자동 및 변경 시점으로 설정되어 있습니다

   프로젝트 업데이트 유형에 대한 자세한 내용은 [프로젝트 업데이트 유형 개요](../../../manage-work/projects/planning-a-project/project-update-type-overview.md).

* 지난 3개월 내에 마지막 업데이트 날짜 있음\
   Workfront 관리자는 [프로젝트에 대한 타임라인 재계산 구성](../../../administration-and-setup/set-up-workfront/configure-system-defaults/configure-timeline-recalculations-projects.md).

* 프로젝트 타임라인의 마지막 계산 날짜가 현재 달력 일 내에 없습니다. 즉, 프로젝트 타임라인의 마지막 계산 날짜가 현재 날짜의 00:00 이전입니다.

프로젝트에 대한 타임라인이 업데이트되는 빈도를 구성할 수 있습니다. 프로젝트 타임라인이 업데이트되면 프로젝트에 대한 변경 사항을 기반으로 다시 계산됩니다.
...
자세한 내용은 [프로젝트 업데이트 유형을 선택합니다](../../../manage-work/projects/manage-projects/select-project-update-type.md).
...
프로젝트 수명 내 다양한 범위가 변경되면 다음 작업을 포함하여 프로젝트 타임라인이 자동으로 재계산됩니다.

* 작업 상태를 업데이트하는 중입니다.
* 작업을 다른 프로젝트로 이동
* 작업의 계획 일자 또는 계획 완료 일자 갱신.
* 기간 유형, 작업 제약 조건 또는 작업에 대한 담당자 수를 업데이트하는 중입니다.
* 선행 작업 관계를 업데이트하는 중입니다.
* 작업의 계획 완료 날짜에도 시간을 추가하는 작업에 승인 추가\
...
## 수동 다시 계산 {#manual-recalculation}

프로젝트 소유자는 개별 프로젝트의 타임라인을 수동으로 다시 계산할 수 있습니다. Workfront 관리자는 Workfront의 모든 타임라인을 수동으로 다시 계산할 수 있습니다.

* [개별 프로젝트 또는 일괄 작업에 대한 타임라인 다시 계산](#recalculate-timelines-for-individual-projects-or-in-bulk)
* [프로젝트 편집 상자에서 타임라인을 일괄적으로 수동으로 다시 계산합니다.](#manually-recalculate-timelines-in-bulk-in-the-edit-projects-box)
* [시스템의 모든 프로젝트에 대한 타임라인 다시 계산(Workfront 관리자만 해당)](#recalculate-timelines-for-all-projects-in-the-system-workfront-administrators-only)

### 개별 프로젝트 또는 일괄 작업에 대한 타임라인 다시 계산 {#recalculate-timelines-for-individual-projects-or-in-bulk}

프로젝트 페이지나 프로젝트 목록 또는 보고서에서 Workfront에 있는 프로젝트의 타임라인을 다시 계산할 수 있습니다.

1. 타임라인을 재계산할 프로젝트로 이동하여 **자세히** 아이콘 ![](assets/qs-more-menu.png) 프로젝트 이름의 왼쪽에 있습니다.

   ![](assets/project-level-more-drop-down-expanded-nwe-350x516.png)

   또는

   프로젝트 목록 또는 보고서로 이동하고 하나 이상의 프로젝트를 선택한 다음 **자세히** 아이콘 ![](assets/qs-more-menu.png) 를 클릭합니다.

   ![](assets/recalculate-expressions-timeline-finances-drop-down-in-project-list-nwe.png)

   >[!TIP]
   >
   >프로젝트의 복잡성에 따라 타임라인을 일괄적으로 재계산할 때 많은 프로젝트를 선택하지 않도록 하여 최적의 성능을 보장합니다. 프로젝트를 너무 복잡하게 만들 수 있는 몇 가지 사항은 여러 종속성, 할당 또는 많은 사용자 지정 필드일 수 있습니다.

1. 클릭 **타임라인 다시 계산**.

   타임라인이 다시 계산되면 재계산이 성공했음을 나타내는 메시지가 표시됩니다.

   >[!TIP]
   >
   >타임라인 재계산이 완료되기 전에 일부 계획 또는 예상 날짜가 흐리게 표시될 수 있습니다. 즉, 재계산이 아직 완료되지 않고 날짜가 변경될 수 있습니다.

### 프로젝트 편집 상자에서 타임라인을 일괄적으로 수동으로 다시 계산합니다. {#manually-recalculate-timelines-in-bulk-in-the-edit-projects-box}

여러 프로젝트의 타임라인을 일괄적으로 편집하여 수동으로 다시 계산할 수 있습니다.
...
1. 클릭 **설정**&#x200B;를 선택하고 을 선택합니다. **타임라인 다시 계산**.

1. 클릭 **변경 내용 저장**.

### 시스템의 모든 프로젝트에 대한 타임라인 다시 계산(Workfront 관리자만 해당) {#recalculate-timelines-for-all-projects-in-the-system-workfront-administrators-only}

Workfront 관리자는 타임라인 재계산 진단 을 실행하여 Workfront 시스템의 모든 타임라인을 즉시 재계산할 수 있습니다. 이를 통해 모든 프로젝트 관리자는 계획 날짜와 예상 날짜 모두에 외부 변경 내용이 미치는 영향을 즉시 확인할 수 있습니다.

전체 Workfront 사이트의 타임라인 재계산에 대한 자세한 내용은 의 &quot;전체 Workfront 인스턴스에 대한 타임라인 다시 계산&quot; 섹션을 참조하십시오. [프로젝트에 대한 타임라인 재계산 구성](../../../administration-and-setup/set-up-workfront/configure-system-defaults/configure-timeline-recalculations-projects.md).
