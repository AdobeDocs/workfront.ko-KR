   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td> <p> <p>(을(를) 선택한 후에만 표시됩니다 <strong>수동</strong> 대상 <strong>조건 유형</strong>): 프로젝트 진행 방식을 나타내려면 조건 을 선택합니다. </p> <p>프로젝트 조건을 자동으로 또는 수동으로 설정하는 방법에 대한 자세한 내용은 <a href="../../../manage-work/projects/manage-projects/project-condition-and-condition-type.md" class="MCXref xref">프로젝트 조건 및 조건 유형 개요</a></p> </p> </td> 
     </tr> 
     <tr> 
...
액세스 수준 및 프로젝트 권한에 따라, 다음과 같은 시나리오가 있습니다.

* 프로젝트에 대한 사용자 지정 양식 편집 권한이 없는 경우 첨부된 사용자 지정 양식의 필드를 편집할 수 없습니다. 프로젝트에 첨부된 사용자 지정 양식의 필드만 볼 수 있습니다.
* 사용자 지정 양식의 섹션에 대한 보기(편집이 아님) 액세스 권한이 있는 경우 해당 섹션의 필드를 편집할 수 없습니다.
* 프로젝트에 첨부된 사용자 지정 양식 중 하나의 섹션에 액세스할 수 없는 경우 프로젝트 편집 상자에 섹션이 표시되지 않습니다.

두 개 이상의 프로젝트를 일괄 편집할 때 다음 시나리오가 있습니다.

* 선택한 프로젝트 중 하나 이상에 대한 사용자 지정 양식 편집 권한이 없는 경우 첨부된 사용자 지정 양식의 필드를 편집할 수 없습니다. 첨부된 사용자 지정 양식에서만 필드를 볼 수 있습니다
* 사용자 지정 양식의 섹션에 대한 보기(편집이 아님) 액세스 권한이 있는 경우 해당 섹션의 필드를 편집할 수 없습니다. 해당 섹션의 필드만 볼 수 있습니다.
* 하나 이상의 프로젝트에 첨부된 사용자 지정 양식 중 하나의 섹션에 액세스할 수 없는 경우 프로젝트 편집 상자에 섹션이 표시되지 않습니다.
* 모든 프로젝트에 첨부된 사용자 지정 양식에 필수 필드가 있고 실제로 편집하지 않고 한 필드를 선택하는 경우 선택한 프로젝트를 저장하려면 먼저 해당 필드의 변경 내용을 취소하거나 해당 필드에 정보를 추가해야 합니다. 필수 필드를 전혀 선택하지 않으면 필수 필드가 비어 있어도 선택한 프로젝트를 저장할 수 있습니다.

사용자 지정 양식 액세스에 대한 자세한 내용은 다음 문서를 참조하십시오.
...
1. 을(를) 클릭합니다. **사용자 지정 양식 추가** 상자를 열고 목록에서 양식을 선택하여 프로젝트에 첨부합니다. 기본적으로 처음 40개의 양식은 알파벳 순서로 표시됩니다. 목록에 양식이 표시되지 않으면 해당 이름을 입력한 다음 목록에 표시될 때 선택합니다.
...
* 프로젝트에 대한 재무 데이터 및 재무 보기 권한이 있는 경우 재무 섹션의 필드만 볼 수 있습니다. 이 섹션의 필드는 편집할 수 없습니다.
* 프로젝트에 대한 재무 데이터에 대한 편집 액세스 권한 및 재무 관리 권한이 있는 경우 이 섹션의 필드를 갱신할 수 있습니다.

둘 이상의 프로젝트를 일괄 편집할 경우 다음 시나리오가 있습니다.

* 재무 보기 권한이 있는 프로젝트를 하나 이상 선택하는 경우(재무 관리 대신) 선택한 모든 프로젝트에 대해 이 섹션의 필드만 볼 수 있습니다. 재무 섹션에서 필드를 벌크 편집할 수 없습니다.
* 재무 권한이 없는 프로젝트를 하나 이상 선택하는 경우 이 섹션이 전혀 표시되지 않습니다.
...
1. 다음 정보를 업데이트합니다.
   ...
   <p>사용자가 프로젝트에서 이러한 시간 유형 옵션을 보려면 개별 사용자 수준에서 동일한 시간 유형을 선택해야 합니다. 사용자 수준에서 시간 유형 정의에 대한 자세한 내용은 <a href="../../../timesheets/create-and-manage-timesheets/log-time.md" class="MCXref xref">로그 시간</a>. </p> </p> </td> 
      ...
      <tr data-mc-conditions=""> 
       ...
       <li> <p>시스템 전체 및 그룹별 승인 프로세스가 목록에 표시됩니다. 프로젝트 이외의 그룹과 연관된 승인 프로세스가 목록에 표시되지 않습니다.</p> <p>프로젝트와 연관된 그룹이 변경되면 그룹별 승인 프로세스는 단일 사용 승인 프로세스가 됩니다. 프로젝트 그룹의 변경 사항이나 승인 프로세스의 변경 사항이 승인 설정에 미치는 영향에 대한 자세한 내용은 <a href="../../../administration-and-setup/customize-workfront/configure-approval-milestone-processes/how-changes-affect-group-approvals.md" class="MCXref xref">그룹 및 승인 프로세스 변경이 지정된 승인 프로세스에 미치는 영향</a>. </p> </li> <!--(NOTE: this bullet stays here although the sections it might appear in are QS only, so we can use the snippet for both Qs and classic)-->
...
* &quot;복수 값&quot; 표시기 외에도 선택한 옵션이 선택한 프로젝트 중 적어도 하나에서 다른 경우 다음 방법 중 하나로 여러 옵션이 있는 필드가 표시됩니다.

   * 확인란 필드에는 선택한 모든 프로젝트에 대해 일부 선택되지만 일부 프로젝트에 대해서는 선택되어 있지 않은 옵션에 대한 확인란 대신 줄이 있습니다.

      ![](assets/multiple-values-indicator-check-boxes-bulk-edit-projects.png)

   * 토글 유형 필드는 흐리게 표시되어 일부 프로젝트에서는 사용할 수 있지만 선택한 모든 프로젝트에는 사용할 수 없는 옵션에 대한 중간 토글 기능을 사용할 수 있습니다.

   ![](assets/multiple-values-highlighted-bulk-edit-projects.png)

   * 일부 선택 사항은 선택했지만 모두 선택되지 않은 라디오 단추 유형 필드에 모든 라디오 단추가 비어 있는 상태로 표시됩니다.

      ![](assets/multiple-values-indicator-radio-buttons-bulk-edit-projects.png)


* 여러 옵션 필드(예: 전환 또는 확인란으로 표시되는 필드)에서 한 옵션을 업데이트하는 경우 다른 모든 옵션은 선택한 프로젝트 간에 일치해야 합니다.

   >[!IMPORTANT]
   >
   >예를 들어, 모든 프로젝트에 대해 3개의 확인란(옵션1, 옵션 2 및 옵션 3)과 옵션 1이 선택 취소되어 있고, 일부 프로젝트에 대해서는 옵션 2와 3이 선택되고 선택한 다른 프로젝트에 대해서는 선택 취소되어 있는 확인란 필드가 있을 수 있습니다. 모든 프로젝트에 대해 옵션 1을 선택하려면 선택한 모든 프로젝트에 대해 옵션 2와 3을 일치시켜야 변경 사항을 저장할 수 있으므로 선택한 모든 프로젝트에서 일치되도록 선택하거나 선택 취소해야 합니다. 옵션을 변경하지 않으면 필드를 그대로 저장할 수 있으며, 프로젝트에서는 모든 옵션에 대한 현재 선택을 유지합니다.
...
1. 을(를) 클릭합니다. **편집** 아이콘 ![](assets/edit-icon.png) 를 클릭합니다.
...
1. 클릭 **개요** 을 눌러 선택한 프로젝트에 대한 일반 정보를 편집합니다.  개요 영역 편집에 대한 자세한 내용은 섹션을 참조하십시오 [개요](#overview) 참조하십시오.

   >[!TIP]
   >
   >편집하는 필드는 밝은 자주색 배경으로 표시됩니다.

1. 클릭 **사용자 지정 Forms** 선택한 프로젝트와 연결된 사용자 지정 양식을 편집, 추가 또는 교체하려면 다음을 수행하십시오.

   선택한 모든 프로젝트에 첨부된 사용자 지정 양식이 **일반적인 사용자 지정 양식** 하위 섹션의 **사용자 지정 Forms** 영역.

   ![](assets/custom-forms-in-common-unshimmed.png)

   >[!TIP]
   >
   >   선택한 모든 프로젝트에 공통인 양식의 이름이 [프로젝트 편집] 상자의 왼쪽 패널에 표시됩니다.

1. 에 사용자 지정 양식 이름을 입력합니다. **사용자 지정 양식 추가** 필드.


   ![](assets/forms-already-attached-indication-in-bulk-editing-projects-unshimmed.png)

   선택한 프로젝트에 이미 첨부된 사용자 지정 양식이 **첨부된 양식** 하위 섹션의 **사용자 지정 양식 추가** 필드.
...
1. (선택 사항) **x** 사용자 지정 양식 이름의 오른쪽에 있는 아이콘을 클릭한 다음 **제거** 을 눌러 선택한 모든 프로젝트에서 제거합니다.
   ...
   >[!CAUTION]
   >
   >사용자 지정 양식을 제거하면 양식의 기존 사용자 지정 필드 정보가 모두 손실됩니다. 복구할 수 없습니다.

   사용자 지정 양식 편집에 대한 자세한 내용은 섹션을 참조하십시오 [사용자 지정 Forms](#custom-forms) 참조하십시오.

1. 클릭 **재무** 을 눌러 선택한 모든 프로젝트에 대한 재무 정보를 편집합니다.
재무 영역 편집에 대한 자세한 내용은 섹션을 참조하십시오 [재무](#finance) 참조하십시오.
1. 클릭 **프로젝트 설정** 을 눌러 선택한 모든 프로젝트에 대한 설정을 편집합니다.
프로젝트 설정 영역 편집에 대한 자세한 내용은 섹션을 참조하십시오 [프로젝트 설정](#project-settings) 참조하십시오.
1. 클릭 **작업 설정** 을 눌러 선택한 모든 프로젝트에 대한 작업 설정을 편집합니다.
작업 설정 영역 편집에 대한 자세한 내용은 섹션을 참조하십시오 [작업 설정](#task-settings) 참조하십시오.
1. 클릭 **문제 설정** 선택한 모든 프로젝트에 대한 문제 설정을 편집하려면 다음을 수행하십시오.
문제 설정 영역 편집에 대한 자세한 내용은 섹션을 참조하십시오 [문제 설정](#issue-settings) 참조하십시오.
1. 클릭 **액세스** 을 눌러 선택한 모든 프로젝트에 대한 액세스 설정을 편집합니다.
액세스 영역 편집에 대한 자세한 내용은 섹션을 참조하십시오 [액세스](#access) 참조하십시오.
1. (선택 사항) 프로젝트 편집 상자에서 추가한 정보를 제거하려면 편집된 필드를 마우스로 가리킨 다음 **x** 필드의 오른쪽 상단에 있는 아이콘을 삭제합니다.
