...
* 청구 요율을 관리하려면 재무 데이터(특히 청구 요금)에 대한 편집 액세스 권한이 있는 플랜 또는 표준 라이센스가 필요합니다.
...
요금 카드를 편집할 수 있는 액세스 권한이 있는 경우 위치, 그룹 또는 에이전시와 같은 속성을 기준으로 역할당 여러 청구 요금이 있는 요금 카드를 정의할 수 있습니다. 속성은 최대 5개 수준까지 구성할 수 있습니다.

프로젝트에 요금을 적용하려면 요금 카드를 첨부해야 합니다. 비율이 비율 카드에서 잠겨 있으면 프로젝트 수준에서 재정의할 수 없습니다.

비율 카드 비율은 태스크 수익 유형에 따라 비율을 결정하는 계층의 일부입니다.
...
요금 계층에 대한 자세한 내용은 [수익 및 비용 계층 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)를 참조하십시오.

![샘플 속도 카드](assets/sample-rate-card-march2026.png)
...
Adobe Workfront 관리자는 작업 역할을 생성할 때 요금 값 및 날짜를 지정하여 작업 역할을 일자 유효 청구 요금과 연결할 수 있습니다.

Workfront 시스템의 기본 통화 또는 다른 통화를 사용하여 작업 역할 청구 요금의 값을 정의할 수 있습니다. 시스템 환율에서도 추가 통화를 정의해야 합니다.
...
### 청구 요금 재정의 - 워크플로우 Ultimate 패키지

>[!IMPORTANT]
>
>프로젝트 수준에서 작업 역할 또는 사용자와 연관된 청구 요율을 대체할 수 있습니다. 고정 비율은 재정의할 수 없습니다.

프로젝트 수준에서 다음을 수행할 수 있습니다.

* 작업 역할에 대한 청구 단가 대체(위치, 그룹 또는 에이전시와 같은 속성이 적용된 경우).
* 해당 프로젝트의 특정 사용자에 대한 청구 요율을 재정의합니다.

청구 요금 무시는 일반적이지 않습니다. 예를 들어 &quot;Designer&quot;를 역할로 재정의하지는 않습니다. 대신, 관련 유효 기간 동안 &quot;Designer - 뉴욕 - 에이전시 X&quot;를 재정의합니다. 재정의는 청구 요금 계층을 준수하므로 시스템은 항상 우선순위 순서로 이를 적용합니다.
...
* 특정 프로젝트
  ...
  프로젝트에 대한 작업 역할 청구 요율을 만드는 방법에 대한 자세한 내용은 문서 [청구 요율 재정의 개요 및 프로젝트 수익 계산](/help/quicksilver/manage-work/projects/project-finances/override-role-billing-rates-and-calculate-project-revenue.md)을 참조하십시오.
...
기본적으로 모든 새 작업에 대한 매출 유형은 Workfront 또는 그룹 관리자가 지정한 작업 및 문제 환경 설정에 따라 설정됩니다.
...
프로젝트의 고정 수익 설정에 대한 자세한 내용은 [프로젝트 편집](../../../manage-work/projects/manage-projects/edit-projects.md) 문서를 참조하십시오.
작업의 매출 유형 설정에 대한 자세한 내용은 [작업 편집](../../../manage-work/tasks/manage-tasks/edit-tasks.md) 문서를 참조하십시오.

>[!NOTE]
>
><span class="preview">사용 가능한 사용자 및 역할 시간당 수익 유형을 사용하려면 Workflow Ultimate 패키지가 있어야 합니다.</span>
...
작업을 만들 때 기본 수익 유형입니다.</p> </td>
  ...
  <tr> 
   ...
   <td> <p><span class="preview">이 유형은 작업에만 사용할 수 있습니다.</span></p> <p><span class="preview">이 유형은 사용자 및 역할 정보를 모두 검사하여 적절한 비율을 결정합니다.</span></p></td> 
  </tr>
  <tr> 
  ...
  <tr> 
   <td> <p><span class="preview">시간별 사용자 및 역할(상한 포함)</span></p> </td> 
   <td> <p><span class="preview">이 유형은 작업에만 사용할 수 있습니다.</span></p> <p><span class="preview">작업은 시간별 사용자 및 역할과 같이 시간별로 청구되지만 지정할 수 있는 최대 상한이 있습니다.</span></p></td> 
  </tr>
  <tr> 
  ...
  <tr> 
   <td> <p><span class="preview">시간별 사용자 및 역할 (고정 포함)</span></p> </td> 
   <td> <p><span class="preview">이 유형은 작업에만 사용할 수 있습니다.</span></p> <p><span class="preview">작업은 시간별 사용자 및 역할에서와 같이 시간별로 청구되지만 요금에 추가할 수 있는 추가 고정 금액이 있습니다. 작업에 지정된 고정 금액은 프로젝트의 청구 기록에 포함될 수 있습니다. 고정 금액에 작업의 시간을 곱하지 않습니다.</span></p></td> 
  </tr>
  <tr> 
   <td> <p>고정 시간별</p> </td> 
...
* [프로젝트에 대한 수익 계산](#revenue-calculations-for-projects)
...
시스템은 계층을 사용하여 태스크 지정을 기준으로 수익 계산에 사용되는 비율을 결정합니다.
...
사용자 및 역할 시간당 수익 유형의 경우 프로젝트 수준과 할당 수준 모두에서 청구에 대한 작업 역할을 정의할 수 있습니다. 특정 사용자에 대한 프로젝트 수준에서 정의된 경우 해당 역할은 해당 역할을 적용한 유효 기간 동안 해당 사용자의 모든 할당으로 자동으로 전파됩니다. 필요한 경우 발령 레벨에서 이 비율을 대체할 수 있습니다. 예를 들어, 사용자의 기본 작업 역할은 Designer이지만 프로젝트에 대한 대금 청구에 대한 작업 역할을 8월 한 달 동안 선임 Designer으로 설정합니다. 8월에 할당된 모든 작업은 자동으로 선임 Designer 청구 요금을 사용하게 됩니다.

그러나 특정 작업에서는 청구 중인 작업을 반영하기 위해 해당 할당에 대해서만 역할을 재정의할 수 있습니다. 이러한 방식으로 시스템은 프로젝트 전체의 일관성과 할당 수준의 유연성을 모두 지원합니다. 자세한 내용은 [수익 및 비용 계층 구조 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md) 및 [고급 할당 만들기](/help/quicksilver/manage-work/tasks/assign-tasks/create-advanced-assignments.md)를 참조하십시오.
...
* **작업의 매출 유형은 시간별 사용자입니다**
  ...
  <table style="table-layout:auto"> 
   ...
   <tbody> 
    ...
    <tr> 
     ...
     <td>작업에 할당된 작업 역할의 시스템 청구 요금이 계획된 수익을 계산하는 데 사용됩니다. 프로젝트 수준에서 청구 요금이 재정의될 수 있습니다.</td> 
    </tr> 
    <tr> 
  ...
  <td>시간을 기록하는 사용자가 프로필에 청구 요금이 있는 경우 해당 요금이 사용됩니다. <br>그렇지 않으면 기본 작업 역할의 청구 요금이 사용됩니다. 사용자 또는 기본 역할과 연결된 청구 요금이 없는 경우 실제 매출은 $0.00입니다. <br><p><b>메모</b>
  ...
  <table style="table-layout:auto"> 
   ...
   <tbody> 
    ...
    <tr> 
     ...
     <td>작업에 할당된 작업 역할의 청구 요금은 계획된 수익을 계산하는 데 사용됩니다. 프로젝트 수준에서 청구 요금이 재정의될 수 있습니다.</td> 
    </tr> 
    <tr> 
     <td role="rowheader">실제 매출에 대한 시간당 청구 요금</td> 
     <td>Workfront은 시간을 기록하는 사용자의 기본 작업 역할에 대한 청구 요금을 사용합니다. <br>시간을 기록하는 사용자에게 연결된 작업 역할이 없거나 기본 작업 역할에 청구 요금이 없는 경우 실제 매출은 $0.00입니다. </td> 
     <td> 시간을 기록한 사용자가 작업에 할당된 경우 작업에 대한 사용자와 연결된 작업 역할의 청구 요금이 실제 수익을 계산하는 데 사용됩니다. <br>그렇지 않으면 기본 작업 역할의 청구 요금이 사용됩니다. 사용자에게 기본 작업 역할이 없거나 기본 작업 역할에 청구 요금이 없는 경우 실제 매출은 $0.00입니다. </td> 
...
* **작업의 매출 유형은 시간별 사용자 및 역할입니다**

| 시간당 청구 요금 | 할당 없음 | 사용자 할당 | 작업 역할 할당 |
| --- | --- | --- | --- |
| 계획된 수익에 대한 시간당 청구 요금 | US$0.00 | 사용자가 할당되면 시스템은 유지된 청구 요율부터 시작하여 지정된 주문에서 요금을 찾습니다. 다음은 잠긴 요율 카드 요율, 발령에 대한 수동 입력 요율, 발령에 대한 청구에 대한 작업 역할, 프로젝트 수준 사용자 청구 요율 대체, 프로젝트 수준의 청구에 대한 작업 역할, 사용자 시스템 요율 및 사용자의 기본 작업 역할 요율입니다. <p> 자세한 내용은 [수익 및 비용 계층 구조 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)를 참조하십시오. | 작업 역할이 할당되면 시스템은 먼저 보존된 청구 요율을 찾은 다음 할당 시 작업 역할에 대해 잠긴 요율 카드 요율을 찾습니다. 그런 다음 할당에 수동으로 추가된 작업 역할 비율을 찾습니다. 비율을 찾을 수 없는 경우 프로젝트 수준에서 작업 역할 비율을 찾습니다. 우선 비율 카드에서 찾은 후 시스템 비율에서 찾습니다. <p> 자세한 내용은 [수익 및 비용 계층 구조 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)를 참조하십시오. |
| 실제 매출에 대한 시간당 청구 요금 | 다른 사용자가 작업에 할당된 경우에도 시간을 기록한 사용자와 관련된 비율만 계산에 고려됩니다. <p> 시스템은 유지된 청구 요율부터 시작하여 지정된 주문에서 요금을 찾습니다. 다음은 잠긴 요금 카드 요금, 프로젝트에 대한 청구 요금 대체, 청구에 대한 작업 역할, 소유자의 사용자 프로필에 대한 시스템 수준 요금 및 소유자의 기본 작업 역할에 대한 청구 요금입니다. <p> 자세한 내용은 [수익 및 비용 계층 구조 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)를 참조하십시오. | 사용자가 할당되면 시스템은 유지된 청구 요율부터 시작하여 지정된 주문에서 요금을 찾습니다. 다음은 잠긴 요율 카드 요율, 발령에 대한 수동 입력 요율, 발령에 대한 청구에 대한 작업 역할, 프로젝트 수준 사용자 청구 요율 대체, 프로젝트 수준의 청구에 대한 작업 역할, 사용자 시스템 요율 및 사용자의 기본 작업 역할 요율입니다. <p> 자세한 내용은 [수익 및 비용 계층 구조 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)를 참조하십시오. | 작업 역할이 할당되면 시스템은 먼저 보존된 청구 요율을 찾은 다음 할당 시 작업 역할에 대해 잠긴 요율 카드 요율을 찾습니다. 그런 다음 할당에 수동으로 추가된 작업 역할 비율을 찾습니다. 비율을 찾을 수 없는 경우 프로젝트 수준에서 작업 역할 비율을 찾습니다. 우선 비율 카드에서 찾은 후 시스템 비율에서 찾습니다. 자세한 내용은 [수익 및 비용 계층 구조 개요](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)를 참조하십시오. |
