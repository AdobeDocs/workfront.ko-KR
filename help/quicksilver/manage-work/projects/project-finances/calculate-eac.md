...
<pre>EAC Labor = <em>IF</em> CPI Labor &lt;&gt; 0 다음 EAC Labor = 계획된 인건비 / CPI Labor</pre><pre><em>ELSE</em> EAC 인건비 = 계획된 인건비 + 실제 인건비</pre><pre>CPI 노무 = 실제 인건비 &lt;&gt; 0인 경우 CPI 노무 = 총 예산 비용작업수행 / 실제 인건비</pre><pre>ELSE CPI 인건비 = 1 </pre>EAC를 계산할 때 다음 필드를 고려합니다.
