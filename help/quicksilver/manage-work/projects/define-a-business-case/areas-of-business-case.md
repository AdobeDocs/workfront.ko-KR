...
비즈니스 사례 영역 활성화에 대한 자세한 내용은 문서 [시스템 전체 프로젝트 환경 설정 구성](../../../administration-and-setup/set-up-workfront/configure-system-defaults/set-project-preferences.md)의 비즈니스 사례 섹션을 참조하십시오.
...
모든 프로젝트에는 사전 설정된 필드가 있는 비즈니스 사례의 프로젝트 정보 영역이 있습니다. 즉, Workfront 관리자는 이 영역에 표시되는 필드를 구성할 수 없습니다.

다음 필드를 편집하는 것이 좋습니다.

* **설명**: 프로젝트에 대한 설명을 추가합니다.

* **프로젝트 소유자**: 기본적으로 프로젝트를 만드는 사용자도 프로젝트 소유자입니다. 이 필드를 편집하여 다른 활성 사용자를 프로젝트 소유자로 선택합니다.

* **프로젝트 스폰서**: 프로젝트 스폰서가 될 활성 사용자를 선택하십시오. 스폰서는 비즈니스 사례의 승인을 받습니다.

* **Portfolio**: 프로젝트의 Portfolio을 선택하십시오. 이 드롭다운 메뉴에서 Portfolio을 선택하려면 먼저 를 만들고 활성 상태에 두어야 합니다.
...
* **계획된 이익**: 이 프로젝트가 완료될 때 조직에 대해 계획된 금전적 이익을 예측합니다. 어떤 통화든 사용할 수 있으며 양의 값(예: $10,000)이어야 합니다.

* **상태**: 기본적으로 프로젝트 요청의 상태는 아이디어로 설정됩니다. 상태를 Idea 또는 Planning 이외의 값으로 변경하면 실행 버튼이 비즈니스 사례 요약 영역에서 사라지고 승인을 위해 더 이상 비즈니스 사례를 제출할 수 없습니다.
...
목표를 정의하는 것은 프로젝트가 Portfolio Optimizer에서 점수를 받는 데 선택 사항입니다. 이 섹션은 비즈니스 사례에서 유일한 선택적 섹션입니다. 비즈니스 사례의 다른 모든 섹션은 프로젝트가 Portfolio Optimizer에서 채점되기 전에 완료되어야 합니다. 목표를 생성할 때 목표에 대한 우선순위 수준을 나타낼 수 있습니다.

자세한 내용은 [비즈니스 사례 목표 만들기](../../../manage-work/projects/define-a-business-case/create-business-case-goals.md)를 참조하세요.

## 경비

경비는 프로젝트 수명 동안 발생할 수 있는 비인적 비용을 나타냅니다. 이 영역은 비즈니스 사례에서 기본적으로 활성화되어 있지만 Workfront 관리자는 이 영역을 표시하지 않도록 선택할 수 있습니다.

비즈니스 사례에 입력하는 모든 경비는 프로젝트의 경비 탭에 계획된 경비로도 입력됩니다.
...
비용에 대한 자세한 내용은 [프로젝트 비용 관리](../../../manage-work/projects/project-finances/manage-project-expenses.md)를 참조하십시오.
...
프로젝트 자원에 대해 예산이 책정된 시간은 비즈니스 사례의 자원 예산 책정 영역에 표시되어 프로젝트의 예산 책정된 인건비를 생성합니다. 이 영역은 기본적으로 활성화되어 있습니다.
...
![비즈니스 사례 리소스 예산 책정](assets/business-case-sp-selected-with-choose-button-350x121.png)
...
* 자원의 예산을 책정한 후 역할이 시간당 비용 요율과 연관된 경우 프로젝트의 예산 인건비가 자원 예산 편성 영역에 표시됩니다. 예산 인건비가 프로젝트 통화로 표시됩니다.
...
위험은 프로젝트가 정시 또는 예산으로 끝나지 않도록 하는 요소입니다. 이러한 요소를 정의하는 것은 Portfolio 관리자 또는 프로젝트 스폰서가 프로젝트의 승인에 대해 교육적인 결정을 내리는 데 중요합니다. 이 영역은 비즈니스 사례에서 기본적으로 활성화되어 있지만 Workfront 관리자는 이 영역을 표시하지 않도록 선택할 수 있습니다.

잠재적 비용을 발생 시 정의한 위험과 연관시킬 수 있습니다. 위험의 비용은 프로젝트의 순가치에 영향을 미친다.
...
위험 만들기에 대한 자세한 내용은 [프로젝트에 대한 위험 만들기 및 편집](../../../manage-work/projects/define-a-business-case/create-edit-risks-on-projects.md)을 참조하세요.
...
스코어카드는 프로젝트의 정렬을 측정합니다. 이 영역은 비즈니스 사례에서 기본적으로 활성화되어 있지만 Workfront 관리자는 이 영역을 표시하지 않도록 선택할 수 있습니다.

스코어카드를 적용하려면 Workfront 관리자가 먼저 스코어카드를 만들어야 합니다. 비즈니스 사례의 스코어카드 영역은 스코어카드가 만들어지지 않으면 표시되지 않습니다.
...
비즈니스 사례를 정의할 때 프로젝트에 사용자 지정 Forms을 첨부할 수 있습니다. 이 영역은 비즈니스 사례에서 기본적으로 활성화되지 않으며 Workfront 관리자가 비즈니스 사례에서 표시하도록 활성화해야 합니다.
...
비즈니스 사례의 오른쪽 상단에 있는 비즈니스 사례 요약 패널에서 주요 프로젝트 재무 및 프로젝트가 스코어카드와 정렬되었는지 여부에 대한 요약을 볼 수 있습니다.

비즈니스 사례 요약은 재무 필드 및 스코어카드와 관련되므로 프로젝트 상태를 빠르게 볼 수 있으며 편집할 수 없습니다.
...
비즈니스 사례를 인쇄하거나 보다 압축된 형식으로 이메일에 첨부해야 하는 경우 PDF 파일로 내보낼 수 있습니다.
