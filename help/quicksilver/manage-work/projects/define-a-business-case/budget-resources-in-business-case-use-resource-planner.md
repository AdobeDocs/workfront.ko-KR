...
1. 다음 중 하나를 수행하여 사용자, 역할 또는 프로젝트에 대한 시간, FTE 또는 비용 값으로 **BDG**(예산 시간) 필드를 업데이트합니다.
   ...
   * 프로젝트 또는 작업 역할에 대한 **옵션** 아이콘을 클릭하고 역할, 사용자 또는 프로젝트에 대한 시간을 자동으로 예산책정하는 옵션을 선택하십시오.
   ...
   >[!NOTE]
   >
   >프로젝트의 타임라인과 관계없이 리소스 예산 책정 영역에 표시되는 모든 일정에 대해 리소스의 시간, FTE 또는 비용을 책정할 수 있습니다. 예를 들어, 프로젝트의 타임라인(계획된 시간과 연관된 시간) 중에는 리소스를 사용할 수 없지만 다른 시간 중에는 사용할 수 있음을 나타내려는 경우, 계획된 시간이 0인 시간대에 예산을 편성하여 작업을 수행할 수 있습니다.

1. (선택 사항) 예산 시간, FTE 또는 비용을 다른 일정으로 이동할 수 있는지 확인하려면 **옵션** 아이콘을 클릭한 다음 **예산 책정 날짜 조정**&#x200B;을 클릭합니다.
